 ...
 *  <p>典型用法是在此bean上设置{@code  targetClass}和可选的{@code  bindingName}属性。 
 *  @author  Arjen Poutsma @自3.0起
 * @see  org.jibx.runtime.IMarshallingContext 
 * @see  org.jibx.runtime.IUnmarshallingContext @Spring Framework 5.1.5以来已弃用，因为JiBX上缺乏活动项目
 */
@Deprecated
public class JibxMarshaller extends AbstractMarshaller implements InitializingBean {
	...
	/**
	 * 设置此实例的目标类。 
	 * 需要设置此属性或{@link  #setTargetPackage（String）targetPackage}属性。 
	 *  <p>如果设置了此属性，则将忽略{@link  #setTargetPackage（String）targetPackage}。 
	 * 
	 */
	...
	/**
	 * 设置此实例的目标包。 
	 * 需要设置此属性或{@link  #setTargetClass（Class）targetClass}属性。 
	 *  <p>如果设置了{@link  #setTargetClass（Class）targetClass}，则将忽略此属性。 
	 * 
	 */
	...
	/**
	 * 设置编组时编写的DTD声明的根元素名称。 
	 * 默认情况下，这是{@code  null}（即未写入DTD声明）。 
	 *  <p>如果设置为值，则还需要设置系统ID或公共ID。 
	 *  
	 * @see  #setDocTypeSystemId（String）
	 * @see  #setDocTypePublicId（String）
	 */
	 ...
	 * 默认情况下，这是{@code  null}。 
	 * 仅在还设置了根元素时使用。 
	 *  <p>设置此属性或{@code  docTypePublicId}，但不要同时设置两者。 
	 *  
	 * @see  #setDocTypeRootElementName（String）
	 */
	 ...
	 * 默认情况下，这是{@code  null}。 
	 * 仅在还设置了根元素时使用。 
	 *  <p>设置此属性或{@code  docTypeSystemId}，而不要同时设置两者。 
	 *  
	 * @see  #setDocTypeRootElementName（String）
	 */
	 ...
	 */
	/**
	 * 创建一个新的{@code  IMarshallingContext}，并为其配置正确的缩进。 
	 *  
	 * @return 创建的编组上下文
	 * @throws  JiBXException发生错误
	 */
	 ...
	 *  <p>布尔值标志用于指示此异常是在编组还是在编组期间发生，因为JiBX本身不在其异常层次结构中进行区分。 
	 *  
	 * @param 发生在{
	 * @param>编组中的{@@code> JiBXException}表示在编组（{@code  true}）还是在取消编组（{@code  false}）<
	 * @return期间发生异常>相应的{@code  XmlMappingException}
	 */
