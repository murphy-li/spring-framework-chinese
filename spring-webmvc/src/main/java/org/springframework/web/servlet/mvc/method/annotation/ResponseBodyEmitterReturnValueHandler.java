 ...
 */
/**
 * 类型为{@link  ResponseBodyEmitter}的返回值和子类（例如{@link  SseEmitter}）的返回值的处理程序，包括用{@link  ResponseEntity}包装的相同类型。 
 *  <p>从5.0版本开始，对于在{@link  ReactiveAdapterRegistry}中已注册适配器的任何反应式库，也支持反应式返回值类型。 
...
public class ResponseBodyEmitterReturnValueHandler implements HandlerMethodReturnValueHandler {
	 ...
	 */
	/**
	 * 具有响应类型支持的简单构造函数，基于带有接受标头策略的默认实例{@link  ReactiveAdapterRegistry}，{<@link> org.springframework.core.task.SyncTaskExecutor}和{@link  ContentNegotiationManager}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 具有可插入"反应式"类型支持的完整构造函数。 
	 *  
	 * @param  messageConverters转换器使用
	 * @param 注册表写入发出的对象，以提供对反应性返回值类型的支持
	 * @param 执行程序，用于阻止I / O写入从反应性类型
	 * @param 管理器发出的项目，以检测流媒体类型@从5.0开始
	 */
	public ResponseBodyEmitterReturnValueHandler(List<HttpMessageConverter<?>> messageConverters,
	 ...
	 */
	/**
	 * 使用HttpMessageConverter编写的ResponseBodyEmitter.Handler。 
	 * 
	 */
	 ...
	 */
	/**
	 * 包装以静默忽略标头更改HttpMessageConverter，否则将导致HttpHeaders引发异常。 
	 * 
	 */
