 ...
 */
/**
 * {@code  <bind>}标签支持对特定bean或bean属性的绑定错误进行评估。 
 * 对Java表达式和JSP EL表达式都公开类型为{@link  org.springframework.web.servlet.support.BindStatus}的"状态"变量。 
 *  <p>可用于绑定到模型中的任何bean或bean属性。 
 * 指定的路径确定标记是公开Bean本身的状态（显示对象级错误），特定的Bean属性（显示字段错误）还是匹配的Bean属性集（显示所有对应的字段错误）。 
 *  <p>使用该标签绑定的{@link  org.springframework.validation.Errors}对象将暴露给协作标签以及该错误对象适用的bean属性。 
 * 嵌套标签（例如{@link  TransformTag}）可以访问那些公开的属性。 
 *  <table> <caption>属性摘要</ caption> <thead> <tr> <th class ="colFirst">属性</ th> <th class ="colOne">必需？</ th> <th class ="colOne">运行时表达式？</ th> <th class ="colLast">描述</ th> </ tr> </ thead> <tbody> <tr class ="altColor"> <td> <p> htmlEscape < / p> </ td> <td> <p> false </ p> </ td> <td> <p> true </ p> </ td> <td> <p>为此标签设置HTML转义，作为布尔值。 
 * 覆盖当前页面的默认HTML转义设置。 
 * </ p> </ td> </ tr> <tr class ="rowColor"> <td> <p> ignoreNestedPath </ p> </ td> <td> < p> false </ p> </ td> <td> <p> true </ p> </ td> <td> <p>设置是否忽略嵌套路径（如果有）。 
 * 默认设置为不忽略。 
 * </ p> </ td> </ tr> <tr class ="altColor"> <td> <p>路径</ p> </ td> <td> <p> true </ p> </ td> <td> <p> true </ p> </ td> <td> <p>绑定状态信息的bean或bean属性的路径。 
 * 例如account.name，company.address.zipCode或仅员工。 
 * 状态对象将导出到页面范围，专门针对该bean或bean属性</ p> </ td> </ tr> </ tbody> </ table> @author  Rod Johnson @author  Juergen Hoeller 
 * @see  #setPath
 */
...
public class BindTag extends HtmlEscapingAwareTag implements EditorAwareTag {
	...
	/**
	 * 设置此标签应应用的路径。 
	 * 可以是获取全局错误的bean（例如"person"），也可以是获取字段错误的bean属性（例如"person.name"）（还支持嵌套字段和"person.na"映射）。 
	 *  "person。 
	 * "将返回指定bean的所有错误，包括全局错误和字段错误。 
	 *  
	 * @see  org.springframework.validation.Errors＃getGlobalErrors 
	 * @see  org.springframework.validation.Errors＃getFieldErrors
	 */
	...
	/**
	 * 检索此标签当前绑定的属性； 
	 * 如果绑定到对象而不是特定属性，则获取{@code  null}。 
	 * 旨在配合嵌套标签。 
	 *  
	 * @return 该标签当前绑定的属性，如果没有，则为{@code  null}
	 */
	@Nullable
