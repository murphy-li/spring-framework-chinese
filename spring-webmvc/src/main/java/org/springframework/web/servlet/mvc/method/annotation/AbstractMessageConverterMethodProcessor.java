 ...
 */
/**
 * 通过使用{@link  HttpMessageConverter HttpMessageConverters}写入响应，扩展了{@link  AbstractMessageConverterMethodArgumentResolver}的能力，以处理方法返回值。 
 *  @author  Arjen Poutsma @author  Rossen Stoyanchev @author  Brian Clozel @author  Juergen Hoeller @3.1起
 */
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	 ...
	 */
	/**
	 * 带有转换器列表和ContentNegotiationManager的构造方法。 
	 * 
	 */
	protected AbstractMessageConverterMethodProcessor(List<HttpMessageConverter<?>> converters,
	 ...
	 */
	/**
	 * 构造函数，包括转换器和ContentNegotiationManager以及请求/响应正文建议实例的列表。 
	 * 
	 */
	protected AbstractMessageConverterMethodProcessor(List<HttpMessageConverter<?>> converters,
	...
	/**
	 * 将给定的返回值写入给定的Web请求。 
	 * 委托给{@link  #writeWithMessageConverters（Object，MethodParameter，ServletServerHttpRequest，ServletServerHttpResponse）}
	 */
	 ...
	 *  
	 * @param 值要写入输出消息的值
	 * @param  returnType值的类型
	 * @param  inputMessage输入消息。 
	 * 用于检查{@code  Accept}标头。 
	 *  
	 * @param  outputMessage在I / O错误的情况下写入
	 * @throws 引发的IOException的输出消息
	 * @throws  HttpMediaTypeNotAcceptableException当无法满足请求中由{@code  Accept}头指示的条件时抛出由消息转换器
	 * @throws 抛出：HttpMessageNotWritableException如果转换器不能编写给定消息，或者服务器选择的内容类型没有兼容的转换器，则抛出HttpMessageNotWritableException。 
	 * 
	 */
	...
	/**
	 * 返回要写入响应的值的类型。 
	 * 通常，这是通过getClass对该值进行的简单检查，但是如果该值为null，则需要检查返回类型，可能包括泛型类型确定（例如{@code  ResponseEntity <T>}）。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回{@code  return Type}的泛型类型（如果是{@link  HttpEntity}，则返回嵌套类型）。 
	 * 
	 */
	...
	/**
	 * 返回可以产生的媒体类型。 
	 * 产生的媒体类型为：<ul> <li>在请求映射中指定的可生产媒体类型，或<li>已配置的可以写入特定返回值的转换器的媒体类型，或<li> {<@link> MediaType＃ ALL} </ ul> @始于4.2
	 */
	 ...
	 */
	/**
	 * 检查路径是否具有文件扩展名，以及扩展名是否为{@link  #WHITELISTED_EXTENSIONS白名单}或显式{{@link> ContentNegotiationManager＃getAllFileExtensions（）已注册}。 
	 * 如果不是，并且状态在2xx范围内，则会添加带有安全附件文件名（"f.txt"）的"Content-Disposition"标头，以防止RFD攻击。 
	 * 
	 */
