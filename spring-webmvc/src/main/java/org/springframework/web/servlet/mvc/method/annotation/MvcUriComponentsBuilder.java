 ...
 */
/**
 * 通过指向Spring MVC控制器上的{@code  @RequestMapping}方法来创建{@link  org.springframework.web.util.UriComponentsBuilder}的实例。 
 *  <p>有几组方法：<ul> <li>静态{@code  fromXxx（...）}方法，用于使用当前请求中的信息来准备链接，该信息由对{@link 的调用确定org.springframework.web.servlet.support.ServletUriComponentsBuilder＃fromCurrentServletMapping（）}。 
 * 在请求上下文之外操作时，可以为<li>静态{@code  fromXxx（UriComponentsBuilder，...）}方法赋予baseUrl。 
 * 基于<li>基于实例的{@code  withXxx（...）}方法，其中通过{@link  #relativeTo（org.springframework.web.util.UriComponentsBuilder）}使用baseUrl创建MvcUriComponentsBuilder实例。 
 *  </ ul> <p> <strong>注意：</ strong>：此类使用"Forwarded"（<a href="https://tools.ietf.org/html/rfc7239"> RFC 7239 </ a >），"X-Forwarded-Host"，"X-Forwarded-Port"和"X-Forwarded-Proto"标头（如果存在），以反映客户端起源的协议和地址。 
 * 考虑使用{@code  ForwardedHeaderFilter}以便从中央位置选择是提取还是使用还是丢弃此类标头。 
 * 有关此过滤器的更多信息，请参见Spring Framework参考。 
 *  @author 奥利弗·吉尔克（Oliver Gierke）@author  Rossen Stoyanchev @author  Sam Brannen @since 4.0
 */
public class MvcUriComponentsBuilder {
	 ...
	 */
	/**
	 * Bean工厂中{@link  CompositeUriComponentsContributor}对象的知名名称。 
	 * 
	 */
	 ...
	 *  
	 * @see  #fromController（Class）
	 * @see  #fromMethodName（类，字符串，对象...）
	 * @see  #fromMethodCall（对象）
	 * @see  #fromMappingName（String）
	 * @see  #fromMethod（类，方法，对象...）
	 */
	...
	/**
	 * 创建具有基本URL的此类的实例。 
	 * 之后，使用Xxx（...}}方法对基于实例的{@code 之一进行调用将创建相对于给定基本URL的URL。 
	 * 
	 */
	 ...
	 */
	/**
	 * 从控制器类的映射和当前请求信息（包括Servlet映射）创建一个{@link  UriComponentsBuilder}。 
	 * 如果控制器包含多个映射，则仅使用第一个。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  
	 * @param 控制器键入控制器以为
	 * @return 一个UriComponentsBuilder实例构建URI（切勿使用{<@@code> null}）
	 */
	 ...
	 * 在处理请求的上下文之外使用MvcUriComponentsBuilder或应用与当前请求不匹配的自定义baseUrl时，此功能很有用。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  
	 * @param  builder基本URL的构建器； 
	 * 构建器将被克隆，因此不会被修改，并且可以重新用于以后的调用。 
	 *  
	 * @param 控制器键入控制器以为
	 * @return 一个UriComponentsBuilder实例构建URI（切勿使用{<@@code> null}）
	 */
	 ...
	 */
	/**
	 * 从控制器方法和方法参数值数组的映射创建一个{@link  UriComponentsBuilder}。 
	 * 此方法委托给{@link  #fromMethod（Class，Method，Object ...）}。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  
	 * @param  controllerType控制器
	 * @param  methodName方法名称
	 * @param  args参数值
	 * @return 一个UriComponentsBuilder实例，从不{@code  null} 
	 * @throws  IllegalArgumentException如果没有匹配项或如果有多个匹配方法
	 */
	 ...
	 */
	/**
	 * {@link  #fromMethodName（Class，String，Object ...）}的替代方法，它接受代表基本URL的{@code  UriComponentsBuilder}。 
	 * 在处理请求的上下文之外使用MvcUriComponentsBuilder或应用与当前请求不匹配的自定义baseUrl时，此功能很有用。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  
	 ...
	 * 从控制器方法和方法参数值数组的映射创建一个{@link  UriComponentsBuilder}。 
	 * 值的数组必须与控制器方法的签名匹配。 
	 *  {@code  @RequestParam}和{@code  @PathVariable}的值用于构建URI（通过{@link  org.springframework.web.method.support.UriComponentsContributor UriComponentsContributor}的实现）参数值将被忽略，并且可以为{@code  null}。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  
	 * @param  controllerType控制器类型
	 * @param 方法控制器方法
	 * @param 控制器方法
	 * @return 的args参数值UriComponentsBuilder实例，从不{{@@code> null} @自4.2起
	 */
	 ...
	 * 请参阅类级文档。 
	 *  
	 * @param  baseUrl基本URL的构建器； 
	 * 构建器将被克隆，因此不会被修改，并且可以重新用于以后的调用。 
	 *  
	 * @param  controllerType控制器类型
	 * @param 方法控制器方法
	 * @param 控制器方法
	 * @return 的args参数值UriComponentsBuilder实例（从不{<@@code> null}）@4.2起
	 */
	 ...
	 */
	/**
	 * 通过调用"模拟"控制器方法来创建{@link  UriComponentsBuilder}。 
	 * 然后，将控制器方法和提供的参数值用于委托给{@link  #fromMethod（Class，Method，Object ...）}。 
	 *  <p>例如，给定此控制器：<pre class ="code"> @RequestMapping（"/ people / {id} / addresses"）类AddressController {@RequestMapping（"/ {country}"）public HttpEntity <Void> getAddressesForCountry（@PathVariable字符串国家/地区）{...} @RequestMapping（value ="/"，method = RequestMethod.POST）public void addAddress（地址）{...}} </ pre>可以创建一个UriComponentsBuilder： <pre class ="code"> //内联样式，带有静态导入的"MvcUriComponentsBuilder.on"MvcUriComponentsBuilder.fromMethodCall（on（AddressController.class）.getAddressesForCountry（"US"））。 
	 * buildAndExpand（1）; //较长的形式对于重复调用（和void控制器方法）很有用。 
	 * AddressController controller = MvcUriComponentsBuilder.on（AddressController.class）; controller.addAddress（null）; builder = MvcUriComponentsBuilder.fromMethodCall（controller）; controller.getAddressesForCountry（"US"）builder = MvcUriComponentsBuilder.fromMethodCall（controller）; </ pre> <p> <strong>注意：</ strong>：此方法从"Forwarded"和"X-Forwarded-"标头中提取值（如果找到）。 
	 * 请参阅类级文档。 
	 *  
	 * @param 信息是从"模拟"控制器调用返回的值，或者是在调用之后从"模拟"控制器本身返回的值。 
	 * 
	 * @return 一个UriComponents实例
	 * @see  #on（Class）
	 * @see  #controller（Class ）
	 */
	 ...
	 * 构建器将被克隆，因此不会被修改，并且可以重新用于以后的调用。 
	 *  
	 * @param 信息是从"模拟"控制器调用返回的值，或者是在调用之后"模拟"控制器本身
	...
	/**
	 * 返回一个"模拟"控制器实例。 
	 * 调用控制器上的{@code  @RequestMapping}方法时，将记住提供的参数值，然后可通过{@link  #fromMethodCall（Object）将结果用于创建{@code  UriComponentsBuilder} ）}。 
	 *  <p>请注意，这是{@link  #controller（Class）}的简写版本，旨在用于内联使用（带有静态导入），例如：<pre class ="code"> MvcUriComponentsBuilder.fromMethodCall（on（ FooController.class）.getFoo（1）.build（）; </ pre> <p> <strong>注意：</ strong>：此方法从"Forwarded"和"X-Forwarded-"标头中提取值（如果找到）。 
	 * 请参阅类级文档。 
	 *  
	 * @param 控制器键入目标控制器
	 */
	 ...
	 * 返回一个"模拟"控制器实例。 
	 * 调用控制器上的{@code  @RequestMapping}方法时，将记住提供的参数值，然后可通过{@link  #fromMethodCall（Object）将结果用于创建{@code  UriComponentsBuilder} }。 
	 *  <p>这是{@link  #on（Class）}的较长版本。 
	 * 控制器方法也需要它返回void以便重复调用。 
	 *  <pre class ="code"> FooController fooController = controller（FooController.class）; fooController.saveFoo（1，null）; builder = MvcUriComponentsBuilder.fromMethodCall（fooController）; fooController.saveFoo（2，null）; builder = MvcUriComponentsBuilder.fromMethodCall（fooController）; </ pre> <p> <strong>注意：</ strong>：此方法从"Forwarded"和"X-Forwarded-"标头中提取值（如果找到）。 
	 * 请参阅类级文档。 
	 *  
	 * @param 控制器键入目标控制器
	 */
	 ...
	 */
	/**
	 * 从Spring MVC控制器方法的请求映射的名称创建一个URL。 
	 *  <p>已配置的{@link  org.springframework.web.servlet.handler.HandlerMethodMappingNamingStrategy HandlerMethodMappingNamingStrategy}确定启动时控制器方法请求映射的名称。 
	 * 默认情况下，将根据类名称的大写字母为所有映射分配一个名称，后跟"＃"作为分隔符，然后是方法名称。 
	 * 例如，使用方法getPerson的名为PersonController的类的"PC＃getPerson"。 
	 * 如果命名约定没有产生唯一的结果，则可以通过{@code  @RequestMapping}注解的name属性来指定显式名称。 
	 *  <p>主要用于视图渲染技术和EL表达式。 
	 *  Spring URL标记库将此方法注册为一个名为"mvcUrl"的函数。 
	 *  <p>例如，给定此控制器：<pre class ="code"> @RequestMapping（"/ people"）类PersonController {@RequestMapping（"/ {id}"）public HttpEntity <Void> getPerson（@PathVariable String id ）{...}} </ pre> JSP可以如下准备控制器方法的URL：<pre class ="code"> <％@taglib uri ="http://www.springframework.org/tags "prefix ="s"％> <a href="${s:mvcUrl('PC#getPerson').arg(0,"123").build()}">找人</a> </ pre > <p>请注意，不必指定所有参数。 
	 * 仅准备URL所需的那些，主要是{@code  @RequestParam}和{@code  @PathVariable}）。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  
	 * @param  mappingName映射名称
	 * @return 一个构建器，用于准备URI字符串
	 * @throws  IllegalArgumentException（如果未找到映射名称或自4.1起没有唯一匹配项）
	 */
	 ...
	 *  
	 * @param 命名映射名称
	 * @return 一个构建器以准备URI字符串
	 * @throws  IllegalArgumentException（如果未找到映射名称或自4.2起没有唯一匹配项）
	 */
	 ...
	 */
	/**
	 * {@link  #fromController（Class）}的替代方法，用于通过调用{@link  #relativeTo}创建的此类实例。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  @4.2起
	 */
	 ...
	 */
	/**
	 * {@link  #fromMethodName（Class，String，Object ...）}}的替代方法，用于通过{@link  #relativeTo}创建的此类的实例。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  @4.2起
	 */
	 ...
	 */
	/**
	 * {@link  #fromMethodCall（Object）}的替代方法，用于通过{@link  #relativeTo}创建的此类的实例。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  @4.2起
	 */
	 ...
	 */
	/**
	 * {@link  #fromMappingName（String）}的替代方法，用于通过{@link  #relativeTo}创建的此类的实例。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  @4.2起
	 */
	 ...
	 */
	/**
	 * {@link  #fromMethod（Class，Method，Object ...）}的替代方法，用于通过{@link  #relativeTo}创建的此类实例。 
	 *  <p> <strong>注意</ strong>：如果找到此方法，此方法将从"Forwarded"和"X-Forwarded-"标头中提取值。 
	 * 请参阅类级文档。 
	 *  @4.2起
	 */
	...
	public static class MethodArgumentBuilder {
		 ...
		 */
		/**
		 * 仅当需要通过引用具有保留含义的所有字符来对扩展的URI变量应用强编码时，才使用此方法。 
		 *  @5.0.8起
		 */
