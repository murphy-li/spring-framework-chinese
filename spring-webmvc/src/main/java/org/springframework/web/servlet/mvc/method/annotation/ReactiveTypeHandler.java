 ...
 */
/**
 * 私有帮助程序类，用于协助处理可通过{@link  ReactiveAdapterRegistry}适应于Reactive Streams {@link  Publisher}的"反应性"返回值类型。 
 *  <p>可以将此类返回值桥接到{@link  ResponseBodyEmitter}，以在存在流媒体类型或基于通用类型的情况下进行流传输。 
 *  <p>对于所有其他情况，将收集{@code  Publisher}输出，并将其桥接到{@link  DeferredResult}，以进行标准的异步请求处理。 
...
class ReactiveTypeHandler {
	 ...
	 */
	/**
	 * 该类型是否可以适应于Reactive Streams {@link  Publisher}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 处理给定的无功返回值，并确定是否使其适应{@link  ResponseBodyEmitter}或{@link  DeferredResult}。 
	 *  
	 * @return 用于流传输的发射器，如果在内部使用{@link  DeferredResult}处理，则为{@code  null}
	 */
	@Nullable
