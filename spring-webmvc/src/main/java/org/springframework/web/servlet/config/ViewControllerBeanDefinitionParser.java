 ...
 */
/**
 * {@link  org.springframework.beans.factory.xml.BeanDefinitionParser}解析以下MVC名称空间元素：<ul> <li> {<@code> <view-controller>} <li> {<@code> <redirect-view-controller>} <li> {<@code> <status-controller>} </ ul> <p>所有元素都会导致{@link  org.springframework.web.servlet的注册。 
 *  mvc.ParameterizableViewController ParameterizableViewController}，所有控制器在单个{@link  org.springframework.web.servlet.handler.SimpleUrlHandlerMapping SimpleUrlHandlerMapping}中进行映射。 
 *  @author 基思·唐纳德@author 克里斯蒂安·杜普依斯@author  Rossen Stoyanchev @since 3.0
 */
