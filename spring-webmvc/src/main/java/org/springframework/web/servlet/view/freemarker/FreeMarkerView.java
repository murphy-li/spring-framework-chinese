...
/**
 * 使用FreeMarker模板引擎查看。 
 *  <p>公开以下JavaBean属性：<ul> <li> <b> url </ b>：要包装的FreeMarker模板相对于FreeMarker模板上下文（目录）的位置。 
 *  <li> <b>编码</ b>（可选，默认由FreeMarker配置确定）：FreeMarker模板文件的编码</ ul> <p>取决于单个{@link  FreeMarkerConfig}对象，例如{@link  FreeMarkerConfigurer}可以在当前Web应用程序上下文中以任何bean名称进行访问。 
 * 另外，您可以将FreeMarker {@link  Configuration}对象设置为bean属性。 
 * 有关此方法的影响的更多详细信息，请参见{@link  #setConfiguration}。 
 *  <p>注意：Spring的FreeMarker支持需要FreeMarker 2.3或更高版本。 
 *  @author 达伦·戴维森（Darren Davison）@author  Juergen Hoeller @2004年3月3日起FreeMarkerConfigurer
 */
public class FreeMarkerView extends AbstractTemplateView {
	...
	/**
	 * 设置FreeMarker模板文件的编码。 
	 * 默认值由FreeMarker配置确定："ISO-8859-1"（如果未另行指定）。 
	 *  <p>如果所有模板共享相同的编码，请在FreeMarker配置中指定编码，而不是在每个模板中指定编码。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置该视图要使用的FreeMarker配置。 
	 *  <p>如果未设置此项，则将进行默认查找：在当前的Web应用程序上下文中，应使用任何bean名称使用单个{@link  FreeMarkerConfig}。 
	 *  <strong>注意：</ strong>：使用此方法将为每个{@link  FreeMarkerView}实例创建一个新的{@link  TaglibFactory}实例。 
	 * 就内存和初始CPU使用率而言，这可能会非常昂贵。 
	 * 在生产中，建议您使用{@link  FreeMarkerConfig}，它公开单个共享的{@link  TaglibFactory}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 获取FreeMarker配置以供实际使用。 
	 *  
	 * @return  FreeMarker配置（永不{<@@code> null}）
	 * @throws 如果自5.0以来未设置任何配置对象，则抛出IllegalStateException
	 */
	...
	/**
	 * 在启动时调用。 
	 * 查找单个FreeMarkerConfig bean，以查找该工厂的相关配置。 
	 *  <p>检查是否可以找到默认语言环境的模板：如果找不到特定于语言环境的模板，FreeMarker将检查非特定于语言环境的模板。 
	 *  
	 * @see  freemarker.cache.TemplateCache＃getTemplate
	 */
	@Override
	 ...
	 *  
	 * @return 用于FreeMarkerViews的配置实例
	 * @throws  BeansException（如果找不到配置实例）
	 * @see  #getApplicationContext 
	 * @see  #setConfiguration
	 */
	 ...
	 */
	/**
	 * 如果未指定，则返回配置的FreeMarker {@link  ObjectWrapper}，或返回{@link  ObjectWrapper＃DEFAULT_WRAPPER默认包装器}。 
	 *  
	 * @see  freemarker.template.Configuration＃getObjectWrapper（）
	 */
	 ...
	 */
	/**
	 * 检查用于该视图的FreeMarker模板是否存在并且有效。 
	 *  <p>可以重写以自定义行为，例如在将多个模板呈现到单个视图中的情况下。 
	 * 
	 */
	@Override
	 ...
	 */
	/**
	 * 通过与FreeMarker模板合并来处理模型图。 
	 * 输出定向到Servlet响应。 
	 *  <p>如果需要自定义行为，则可以重写此方法。 
	 * 
	 */
	@Override
	...
	/**
	 * 公开每个渲染操作特有的助手。 
	 * 这是必要的，以便不同的渲染操作不能覆盖彼此的格式等。 
	 * <p>由{@code  renderMergedTemplateModel}调用。 
	 * 默认实现为空。 
	 * 可以重写此方法以将自定义助手添加到模型。 
	 *  
	 * @param 对在合并时将传递给模板的模型进行建模
	 * @param 请求当前的HTTP请求
	 * @throws 在将信息添加到上下文时出现致命错误的异常
	 * @see  #renderMergedTemplateModel
	 */
	 ...
	 */
	/**
	 * 使用给定的模型映射（包含要使用的完整模板模型），将FreeMarker视图呈现给定的响应。 
	 *  <p>默认实现呈现由"url"bean属性指定的模板，该属性通过{@code  getTemplate}检索。 
	 * 它委派给{@code  processTemplate}方法，以将模板实例与给定的模板模型合并。 
	 *  <p>向模型添加标准的Freemarker哈希模型：请求参数，请求，会话和应用程序（ServletContext），以及JSP标记库哈希模型。 
	 * 可以覆盖<p>以自定义行为，例如将多个模板呈现到单个视图中。 
	 *  
	 * @param 对用于呈现的模型进行建模。 
	 * 请参阅> #setUrl <@请参阅org.springframework.web.servlet.support.RequestContextUtils＃getLocale <@请参阅#getTemplate（java.util.Locale）<@请参阅> #processTemplate <@请参阅freemarker.ext.servlet。 
	 *  FreemarkerServlet
	 */
	 ...
	 */
	/**
	 * 为给定的请求构建一个FreeMarker {@link  HttpSessionHashModel}，检测会话是否已经存在并做出相应的反应。 
	 *  
	 * @param 请求当前HTTP请求
	 * @param 响应当前servlet响应
	 ...
	 */
	/**
	 * 检索给定语言环境的FreeMarker模板，以通过此视图呈现。 
	 *  <p>默认情况下，将检索由"url"bean属性指定的模板。 
	 *  
	 * @param 语言环境当前语言环境
	 * @return  FreeMarker模板以呈现
	 * @throws  IOException如果无法检索模板文件
	 * @see  #setUrl 
	 * @see  #getTemplate（String，java.util.Locale ）
	 */
	 ...
	 */
	/**
	 * 使用由"encoding"bean属性指定的编码来检索由给定名称指定的FreeMarker模板。 
	 * 子类可以调用<p>来检索特定模板，例如将多个模板呈现到单个视图中。 
	 *  
	 * @param 命名所需模板的文件名
	 * @param 区域设置当前区域设置
	 * @return  FreeMarker模板
	 * @throws  IOException如果无法获取模板文件
	 */
	 ...
	 * @param 模板以处理模板
	 * @param 为模板的模型建模
	 * @param 响应servlet响应（用于获取OutputStream或Writer）
	 * @throws  IOException如果无法检索模板文件
	 * @throws  TemplateException如果由FreeMarker抛出
	 * @see  freemarker.template.Template＃process（Object，java.io.Writer）
	 */
	 ...
	 */
	/**
	 * {@link  ServletConfig}接口的内部实现，将传递给Servlet适配器。 
	 * 
	 */
