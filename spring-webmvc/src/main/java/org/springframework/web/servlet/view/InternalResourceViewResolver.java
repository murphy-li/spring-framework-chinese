 ...
 */
/**
 * {@link  UrlBasedViewResolver}的方便子类，它支持{@link  InternalResourceView}（即Servlet和JSP）以及子类，例如{@link  JstlView}。 
 *  <p>可以通过{@link  #setViewClass}指定此解析器生成的所有视图的视图类。 
 * 有关详细信息，请参见{@link  UrlBasedViewResolver}的javadoc。 
 * 默认值为{@link  InternalResourceView}，如果存在JSTL API，则默认为{@link  JstlView}。 
 *  <p>顺便说一句，优良作法是将仅用作视图的JSP文件放在WEB-INF下，以使其无法直接访问（例如，通过手动输入的URL）。 
 * 届时只有控制器可以访问它们。 
 *  <p> <b>注意：</ b>链接ViewResolvers时，InternalResourceViewResolver始终需要最后一个，因为它将尝试解析任何视图名称，无论基础资源是否实际存在。 
 *  @author  Juergen Hoeller @2003年2月17日起
 * @see  #setViewClass 
 * @see  #setPrefix 
 * @see  #setSuffix 
 * @see  #setRequestContextAttribute 
...
public class InternalResourceViewResolver extends UrlBasedViewResolver {
	 ...
	 */
	/**
	 * 将默认的{@link  #setViewClass视图类}设置为{@link  #requiredViewClass}：默认情况下为{@link  InternalResourceView}，如果存在JSTL API，则为{@link  JstlView}。 
	 * 
	 */
