...
public class GroovyMarkupView extends AbstractTemplateView {
	 ...
	 */
	/**
	 * 设置MarkupTemplateEngine以在此视图中使用。 
	 *  <p>如果未设置，则通过在Web应用程序上下文中查找单个{@link  GroovyMarkupConfig} bean并使用它获取已配置的{@code  MarkupTemplateEngine}实例来自动检测引擎。 
	 *  
	 * @see  GroovyMarkupConfig
	 */
	...
	/**
	 * 在启动时调用。 
	 * 如果没有手动设置{{@link> #setTemplateEngine（MarkupTemplateEngine）templateEngine}，则此方法按类型查找{@link  GroovyMarkupConfig} bean，并使用它获取Groovy标记模板引擎。 
	 *  
	 * @see  GroovyMarkupConfig 
	 * @see  #setTemplateEngine（groovy.text.markup.MarkupTemplateEngine）
	 */
	 ...
	 * 如果尚未手动配置MarkupTemplateEngine，则调用。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回由配置的Groovy标记模板引擎为给定视图URL编译的模板。 
	 * 
	 */
