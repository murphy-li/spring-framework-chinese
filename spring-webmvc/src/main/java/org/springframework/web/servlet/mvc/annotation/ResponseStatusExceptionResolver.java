 ...
 */
/**
 * 一个{@link  org.springframework.web.servlet.HandlerExceptionResolver HandlerExceptionResolver}，它使用{@link  ResponseStatus @ResponseStatus}注解将异常映射到HTTP状态代码。 
 *  <p>默认情况下，此异常解析器在{@link  org.springframework.web.servlet.DispatcherServlet DispatcherServlet}中启用，并且在MVC Java配置和MVC名称空间中启用。 
 *  <p>从4.2版本开始，此解析程序还将递归查找原因异常上出现的{@code  @ResponseStatus}，从4.2.2版本开始，此解析程序支持在自定义组合注释中为{@code  @ResponseStatus}属性覆盖。 
 *  <p>从5.0开始，此解析器还支持{@link  ResponseStatusException}。 
 *  @author  Arjen Poutsma @author  Rossen Stoyanchev @author  Sam Brannen @since 3.0 
 * @see  ResponseStatus 
 * @see  ResponseStatusException
 */
public class ResponseStatusExceptionResolver extends AbstractHandlerExceptionResolver implements MessageSourceAware {
	 ...
	 *  <p>默认实现将状态代码和注释中的原因委托给{@link  #applyStatusAndReason}。 
	 *  
	 * @param  responseStatus {@code  @ResponseStatus}注解
	 * @param 请求当前HTTP请求
	 * @param 响应当前HTTP响应
	 * @param 处理程序已执行的处理程序； 
	 * 如果没有，则{@code  null}在例外时选择，例如如果多部分解析失败
	 * @param 除异常
	 * @return 外为空的ModelAndView，即已解决异常
	 */
	 ...
	 *  <p>默认实现将状态代码和异常原因委托给{@link  #applyStatusAndReason}。 
	 *  
	 * @param 例外情况
	 * @param 请求当前HTTP请求
	 * @param 响应当前HTTP响应
	 * @param 处理程序已执行的处理程序； 
	 * 如果在发生异常时未选择，则为{@code  null}，例如如果多部分解析失败
	 * @return 一个空的ModelAndView，即从5.0开始解析异常
	 */
	...
	/**
	 * 将已解决的状态代码和原因应用于响应。 
	 *  <p>默认实现使用{@link  HttpServletResponse＃sendError（int）}或{@link  HttpServletResponse＃sendError（int，String）}发送响应错误（如果有原因），然后返回空的ModelAndView。 
	 *  
	 * @param  statusCode HTTP状态代码
	 * @param 原因关联的原因（可以为{@code  null}或为空）
	 * @param 响应当前HTTP响应@5.0
	 */
