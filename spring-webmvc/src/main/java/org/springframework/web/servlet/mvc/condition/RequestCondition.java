...
/**
 * 合同要求映射条件。 
 *  <p>请求条件可以通过{@link  #combine（Object）}进行组合，通过{@link  #getMatchingCondition（HttpServletRequest）}与请求进行匹配，并通过{@link ＃进行比较compareTo（Object，HttpServletRequest）}来确定哪个与给定请求更接近。 
 *  @author  Rossen Stoyanchev @author  Arjen Poutsma @since 3.1 
...
public interface RequestCondition<T> {
	 ...
	 * 此方法假定两个实例均已通过{@link  #getMatchingCondition（HttpServletRequest）}获取，以确保它们都具有仅与当前请求相关的内容。 
	 * 
	 */
