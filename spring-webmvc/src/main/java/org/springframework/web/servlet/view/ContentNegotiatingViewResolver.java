 ...
 */
/**
 * {@link  ViewResolver}的实现，该解析基于请求文件名或{@code  Accept}头解析视图。 
 *  <p> {<@code> ContentNegotiatingViewResolver}不会自行解析视图，而是委托给其他{@link  ViewResolver ViewResolvers}。 
 * 默认情况下，这些其他视图解析器会自动从应用程序上下文中拾取，尽管也可以使用{@link  #setViewResolvers viewResolvers}属性进行显式设置。 
 *  <strong>注意</ strong>，为了使此视图解析器正常工作，必须将{@link  #setOrder order}属性设置为比其他属性更高的优先级（默认为{@link 订购了#HIGHEST_PRECEDENCE}。 
 *  <p>此视图解析器使用请求的{@link  plain MediaType媒体类型}为请求选择合适的{@link 视图}。 
 * 通过配置的{@link  ContentNegotiationManager}确定请求的媒体类型。 
 * 确定请求的媒体类型后，此解析器将向每个委托视图解析器查询{@link 视图}，并确定请求的媒体类型是否与该视图的{@link  plain MediaType＃includes（MediaType）兼容} {@link 纯View＃getContentType（）内容类型}）。 
 * 返回最兼容的视图。 
 *  <p>此外，此视图解析器还公开了{@link  #setDefaultViews（List）defaultViews}属性，使您可以覆盖视图解析器提供的视图。 
 * 请注意，这些默认视图是作为候选提供的，仍然需要具有请求的内容类型（通过文件扩展名，参数或{@code  Accept}标头，如上所述）。 
 *  <p>例如，如果请求路径为{@code  /view.html}，则此视图解析器将查找具有{@code  text / html}内容类型（基于{@code  html}文件扩展名）。 
 * 带有{@code  text / html}请求{{@@code> Accept}的标头的{@code  / view}的请求具有相同的结果。 
...
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport
	 ...
	 *  <p>如果未设置，将使用ContentNegotiationManager的默认构造函数，并应用{@link  org.springframework.web.accept.HeaderContentNegotiationStrategy}。 
	 *  
	 * @see  ContentNegotiationManager＃ContentNegotiationManager（）
	 */
	 ...
	 */
	/**
	 * 返回{@link  ContentNegotiationManager}以用于确定请求的媒体类型。 
	 *  @自4.1.9起
	 */
	@Nullable
	 ...
	 */
	/**
	 * 指示如果找不到合适的视图，是否应该返回{@link  HttpServletResponse＃SC_NOT_ACCEPTABLE 406 Not Acceptable}状态代码。 
	 *  <p>默认值为{@code  false}，这意味着当找不到可接受的视图时，此视图解析器将为{@link  #resolveViewName（String，Locale）}返回{@code  null}。 
	 * 这将允许视图解析器链接。 
	 * 当此属性设置为{@code  true}时，{<@link> #resolveViewName（String，Locale）}将使用将响应状态设置为{@code  406 Not Acceptable}的视图进行响应。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置无法从{@link  ViewResolver}链获得更特定视图时使用的默认视图。 
	 * 
	 */
