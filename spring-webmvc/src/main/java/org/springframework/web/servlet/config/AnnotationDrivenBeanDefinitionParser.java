 ...
 */
/**
 * 一个{@link  BeanDefinitionParser}，它为{@code  <annotation-driven />} MVC命名空间元素提供配置。 
 ...
 *  </ ul> <p> <strong>注意：</ strong>：由于使用{@code  <view-controller>}或{@code  <resources>} MVC，可能会注册其他处理程序映射命名空间元素。 
 *  <p>此类注册以下{@link  HandlerAdapter HandlerAdapters}：<ul> <li> {<@link> RequestMappingHandlerAdapter}用于处理带有注释控制器方法的请求。 
 ...
 *  </ ul> <p>此类注册以下{@link  HandlerExceptionResolver HandlerExceptionResolvers}：<ul> <li> {<@link> ExceptionHandlerExceptionResolver}通过{@link  org.springframework.web.bind处理异常.annotation.ExceptionHandler}方法。 
 *  <li> {<@link> ResponseStatusExceptionResolver}表示使用{@link  org.springframework.web.bind.annotation.ResponseStatus}注释的异常。 
 *  <li> {<@link> DefaultHandlerExceptionResolver}用于解决已知的Spring异常类型</ ul> <p>此类注册一个{@link  org.springframework.util.AntPathMatcher}和一个{@link  org.springframework .web.util.UrlPathHelper}将由以下人员使用：<ul> <li> {<@link> RequestMappingHandlerMapping}，<li>用于ViewControllers <li>的{@link  HandlerMapping}和{@link 用于提供资源的HandlerMapping} </ ul>注意，可以使用{@code  path-matching} MVC命名空间元素配置这些bean。 
 *  <p> {<@link> RequestMappingHandlerAdapter}和{@link  ExceptionHandlerExceptionResolver}均默认使用以下实例进行配置：<ul> <li> A {@link  ContentNegotiationManager} <li> A { @link  DefaultFormattingConversionService} <li>如果类路径上有JSR-303实现，则{{@link> org.springframework.validation.beanvalidation.LocalValidatorFactoryBean}} <li>范围为{@link  HttpMessageConverter HttpMessageConverters}取决于在类路径上可用的第三方库。 
 *  </ ul> @author 基思·唐纳德@author  Juergen Hoeller @author  Arjen Poutsma @author  Rossen Stoyanchev @author  Brian Clozel @author  Agim Emruli @since 3.0
 */
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 */
	/**
	 * 全面初始化后，获取在RequestMappingHandlerAdapter中配置的HandlerMethodArgumentResolver的CompositeUriComponentsContributor的FactoryBean。 
	 * 
	 */
