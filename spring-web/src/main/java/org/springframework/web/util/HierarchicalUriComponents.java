 ...
 */
/**
 * {@link  UriComponents}的扩展用于分层URI。 
 *  @author  Arjen Poutsma @author  Juergen Hoeller @author  Rossen Stoyanchev @author  Phillip Webb @since 3.1.3 
 * @see  <a href ="https://tools.ietf.org/html/ rfc3986＃section-1.2.3">分层URI </a>
 */
...
final class HierarchicalUriComponents extends UriComponents {
	 ...
	 * 此外，{@link  #variableEncoder}也会使用给定的字符集进行初始化，以供以后在URI变量扩展时使用。 
	 * 
	 */
	 ...
	 */
	/**
	 * 使用给定组件指定的规则和给定选项，将给定源编码为编码的String。 
	 *  
	 * @param 源源字符串
	 * @param 编码源字符串的编码
	 * @param 输入源URI的组成部分
	 * @return 编码的URI 
	 * @throws  IllegalArgumentException当给定值无效时URI组件
	 */
	 ...
	 * @param 键入源的URI组件
	 * @return 编码的URI 
	 * @throws  IllegalArgumentException（当给定值无效时） URI组件
	 */
	 ...
	 * 检查任何URI组件是否包含任何非法字符。 
	 *  
	 * @throws  IllegalArgumentException如果任何组件具有非法字符
	 */
	private void verify() {
	...
	private enum EncodeState {
		 ...
		 */
		/**
		 * URI变量首先扩展，然后通过仅引用该URI组件内的非法字符来对每个URI组件进行编码。 
		 * 
		 */
		FULLY_ENCODED,
		 ...
		 * illegal chars and chars with reserved meaning.
 		 */
		/**
		 * URI模板首先通过仅引用非法字符来编码，然后在扩展时通过引用非法字符和具有保留含义的字符来更严格地编码URI vars。 
		 * 
		 */
		TEMPLATE_ENCODED;
