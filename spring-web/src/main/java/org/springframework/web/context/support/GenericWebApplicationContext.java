 ...
 *  <p>实现{@link  org.springframework.web.context.ConfigurableWebApplicationContext}，但不适用于{@code  web.xml}中的声明性设置。 
 * 相反，它是专为编程设置而设计的，例如用于构建嵌套上下文或在{@link  org.springframework.web.WebApplicationInitializer WebApplicationInitializers}中使用。 
 *  <p> <b>如果打算实现从配置文件读取Bean定义的WebApplicationContext，请考虑从AbstractRefreshableWebApplicationContext派生，并在{@code  loadBeanDefinitions}方法的实现中读取Bean定义。 
 * </ b> <p >将资源路径解释为Servlet上下文资源，即，作为Web应用程序根目录下的路径。 
 * 绝对路径，例如对于Web应用程序根目录之外的文件，可以通过AbstractApplicationContext实现的"文件："URL访问。 
 *  <p>除了{@link  org.springframework.context.support.AbstractApplicationContext}检测到的特殊bean外，此类还在上下文中检测到主题源bean，名称为"themeSource"。 
 *  @author  Juergen Hoeller @author 克里斯·比姆斯（Chris Beams）自1.2起
 */
public class GenericWebApplicationContext extends GenericApplicationContext
	 ...
	 *  
	 * @param  servletContext在
	 * @see  #registerBeanDefinition 
	 * @see  #refresh中运行的ServletContext
	 */
	 ...
	 *  
	 * @param  beanFactory用于此上下文的DefaultListableBeanFactory实例
	 * @param  servletContext ServletContext在
	 * @see  #registerBeanDefinition 
	 * @see  #refresh中运行
	 */
