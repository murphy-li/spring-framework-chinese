 ...
 */
/**
 * {@code  @RequestScope}是{@link  Scope @Scope}的特化，其组件的生命周期绑定到当前Web请求。 
 *  <p>具体来说，{<@code> @RequestScope}是由<em>组成的注释</ em>，它用作{@code  @Scope（"request"）}}的快捷方式，默认为{<@link > #proxyMode}设置为{@link  ScopedProxyMode＃TARGET_CLASS TARGET_CLASS}。 
 *  <p> {<@code> @RequestScope}可用作创建自定义组成的注释的元注释。 
