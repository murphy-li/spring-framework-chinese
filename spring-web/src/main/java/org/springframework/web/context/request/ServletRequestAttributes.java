...
public class ServletRequestAttributes extends AbstractRequestAttributes {
	 ...
	 */
	/**
	 * 常数，标识在销毁回调名称存储在{@link  HttpSession}中时，前缀为销毁回调名称的{@link  String}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 公开我们要包装的本地{@link  HttpServletResponse}（如果有）。 
	 * 
	 */
	@Nullable
	 ...
	 */
	/**
	 * 公开我们要包装的{@link  HttpSession}。 
	 *  
	 * @param  allowCreate如果尚不存在，则是否允许创建新会话
	 */
	@Nullable
	 ...
	 */
	/**
	 * 确定是否将给定值视为不可变的会话属性，也就是说，不必通过{@code  session.setAttribute}重置给定值，因为其值无法在内部有意义地更改。 
	 *  <p>默认实现为{@code 字符串}，{<@code>字符}，{<@code>布尔值}和标准{@code  Number}值返回{@code  true}。 
	 *  
	 * @param 命名属性的名称
	 * @param 值对应的值以检查
	 * @return  {@code  true}是否出于会话属性管理的目的将该值视为不可变的； 
	 *  {@code  false}否则为
	 * @see  #updateAccessedSessionAttributes（）
	 */
	...
	/**
	 * 注册给定的回调以在会话终止后执行。 
	 *  <p>注意：回调对象应该是可序列化的，以便在Web应用程序重启后不受影响。 
	 *  
	 * @param 命名要注册回调的属性的名称
	 * @param 回调要销毁的回调
	 */
