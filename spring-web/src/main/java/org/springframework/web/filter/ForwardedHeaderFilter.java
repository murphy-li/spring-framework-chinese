 ...
 */
/**
 * 从"Forwarded"和"X-Forwarded-"头中提取值，包装请求和响应，并通过以下方法使其反映客户端发起的协议和地址：<ul> <li> {<@link> HttpServletRequest＃ getServerName（）getServerName（）} <li> {<@link> HttpServletRequest＃getServerPort（）getServerPort（）} <li> {<@link> HttpServletRequest＃getScheme（）getScheme（）} <li> {<@link> HttpServletRequest #isSecure（）isSecure（）} <li> {<@link> HttpServletResponse＃sendRedirect（String）sendRedirect（String）}。 
 *  </ ul> <p>此过滤器也可以在{@link  #setRemoveOnly removeOnly}模式下使用，在该模式下，将不使用"Forwarded"和"X-Forwarded-"头，并且不使用这些头。 
 *  @author  Rossen Stoyanchev @author EddúMeléndez@author  Rob Winch @始于4.3 
 * @see  <a href="https://tools.ietf.org/html/rfc7239"> https：// tools .ietf.org / html / rfc7239 </a>
 */
public class ForwardedHeaderFilter extends OncePerRequestFilter {
	 ...
	 */
	/**
	 * 启用其中仅删除任何"Forwarded"或"X-Forwarded-"标题并且忽略其中的信息的模式。 
	 *  
	 * @param  removeOnly是否放弃并忽略转发的标头@自4.3.9起
	 */
	 ...
	 */
	/**
	 * 使用此属性来启用相对重定向，如{@link  RelativeRedirectFilter}中所述，并使用与该过滤器相同的响应包装，或者，如果同时配置了两者，则仅包装一个。 
	 *  <p>默认情况下，如果将此属性设置为false，在这种情况下，将覆盖对{@link  HttpServletResponse＃sendRedirect（String）}的调用，以将其转换为绝对URL，同时还要考虑转发的标头。 
	 *  
	 * @param  relativeRedirects自4.3.10起是否使用相对重定向
	 */
	 ...
	 */
	/**
	 * 提取并使用"Forwarded"或"X-Forwarded-"标题。 
	 * 
	 */
	 ...
	 */
	/**
	 * 负责contextPath，requestURI和requestURL，同时牢记转发的标头，还考虑到对基础委托请求的路径的更改（例如，在Servlet FORWARD上）。 
	 * 
	 */
	private static class ForwardedPrefixExtractor {
		...
		/**
		 * 具有所需信息的构造函数。 
		 * 当前的{@link  HttpServletRequestWrapper＃getRequest（）委托请求}的
		 * @param 委托请求提供者，该转发可能在转发过程中更改（例如Tomcat。 
		 * 
		 * @param  pathHelper路径助手实例
		 * @param  baseUrl主机，方案，以及基于转发的标头的端口
		 */
