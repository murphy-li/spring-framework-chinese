 ...
 */
/**
 * 特殊的{@link  org.springframework.validation.DataBinder}用于执行从Web请求参数到JavaBeans的数据绑定，包括对多部分文件的支持。 
 *  <p>请参阅DataBinder / WebDataBinder超类以获取自定义选项，这些自定义选项包括指定允许/必需的字段以及注册自定义属性编辑器。 
 *  <p>也可以用于在基于Spring的{@link  org.springframework.web.context.request.WebRequest}抽象的自定义Web控制器或拦截器中进行手动数据绑定：在{@link  org.springframework.web.context.request.WebRequestInterceptor}实现中。 
 * 只需为每个绑定过程实例化一个WebRequestDataBinder，然后使用当前WebRequest作为参数调用{@code  bind}：<pre class ="code"> MyBean myBean = new MyBean（）;。 
 *  //将活页夹应用于自定义目标对象WebRequestDataBinder活页夹= new WebRequestDataBinder（myBean）; //注册自定义编辑器，如果需要的话，binder.registerCustomEditor（...）; //触发请求参数的实际绑定bind.bind（request）; //可选地评估绑定错误错误错误= binder.getErrors（）; ... </ pre> @author  Juergen Hoeller @author  Brian Clozel @since 2.5.2 
 * @see  #bind（org.springframework.web.context.request.WebRequest）
 * @see  #registerCustomEditor 
 * @see  #setAllowedFields 
 * @see  #setRequiredFields 
...
public class WebRequestDataBinder extends WebDataBinder {
	 ...
	 */
	/**
	 * 使用默认的对象名称创建一个新的WebRequestDataBinder实例。 
	 *  
	 ...
	 *  <p>多部分文件的目标属性的类型可以是Part，MultipartFile，byte []或String。 
	 * 后两个接收上载文件的内容。 
	 * 在这种情况下，所有的元数据（例如原始文件名，内容类型等）都将丢失。 
	 *  
	 * @param 具有绑定参数的请求请求（可以是多部分）
	 * @see  org.springframework.web.multipart.MultipartRequest 
	 * @see  org.springframework.web.multipart.MultipartFile 
	 * @see  javax.servlet.http .Part 
	 * @see  #bind（org.springframework.beans.PropertyValues）
	 ...
	 */
	/**
	 * 检查请求是否为多部分请求（通过检查其Content-Type标头）。 
	 *  
	 * @param 请求请求，带有要绑定的参数
	 */
