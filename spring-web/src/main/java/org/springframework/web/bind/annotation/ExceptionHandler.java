 ...
 * 带有此注解的<p>处理程序方法允许具有非常灵活的签名。 
 * 它们可能具有以下类型的参数，并且可以按任意顺序排列：<ul> <li>异常参数：声明为常规Exception或更具体的异常。 
 * 如果注释本身没有通过其{@link  #value（）}缩小异常类型，则这也可用作映射提示。 
 *  <li>请求和/或响应对象（通常来自Servlet API）。 
 * 您可以选择任何特定的请求/响应类型，例如{@link  javax.servlet.ServletRequest} / {@link  javax.servlet.http.HttpServletRequest}。 
 *  <li>会话对象：通常为{@link  javax.servlet.http.HttpSession}。 
 * 这种类型的参数将强制相应会话的存在。 
 * 结果，这样的参数将永远不会是{@code  null}。 
 *  <i>请注意，会话访问可能不是线程安全的，尤其是在Servlet环境中：考虑切换{@link  org.springframework.web.servlet.mvc.method.annotation.RequestMappingHandlerAdapter＃setSynchronizeOnSession"synchronizeOnSession"}如果允许多个请求同时访问会话，则将其标记为"true"。 
 * </ i> <li> {<@link> org.springframework.web.context.request.WebRequest}或{@link  org.springframework。 
 *  web.context.request.NativeWebRequest}。 
 * 允许通用请求参数访问以及请求/会话属性访问，而无需与本机Servlet API绑定。 
 * 当前请求语言环境的<li> {<@link> java.util.Locale}（由可用的最特定的语言环境解析器确定，即，在Servlet中配置的{@link  org.springframework.web.servlet.LocaleResolver}环境）。 
 *  <li> {<@link> java.io.InputStream} / {@link  java.io.Reader}用于访问请求的内容。 
 * 这将是Servlet API公开的原始InputStream / Reader。 
 *  <li> {<@link> java.io.OutputStream} / {@link  java.io.Writer}用于生成响应的内容。 
 * 这将是Servlet API公开的原始OutputStream / Writer。 
 *  <li> {<@link> org.springframework.ui.Model}作为从处理程序方法返回模型映射的替代方法。 
 * 请注意，提供的模型未预先填充有常规模型属性，因此始终为空，以方便为特定于异常的视图准备模型。 
 *  </ ul> <p>处理程序方法支持以下返回类型：<ul> <li>一个{@code  ModelAndView}对象（来自Servlet MVC）。 
 *  <li>一个{@link  org.springframework.ui.Model}对象，其视图名称是通过{@link  org.springframework.web.servlet.RequestToViewNameTranslator}隐式确定的。 
 *  <li>用于公开模型的{@link  java.util.Map}对象，其视图名称是通过{@link  org.springframework.web.servlet.RequestToViewNameTranslator}隐式确定的。 
 *  <li>一个{@link  org.springframework.web.servlet.View}对象。 
 *  <li>一个{@link 字符串}值，它被解释为视图名称。 
 *  <li> {<@link> ResponseBody @ResponseBody}带注释的方法（仅用于Servlet）设置响应内容。 
 * 将使用{@link  plain org.springframework.http.converter.HttpMessageConverter消息转换器}将返回值转换为响应流。 
 *  <li>一个{@link  org.springframework.http.HttpEntity HttpEntity <？>}或{@link  org.springframework.http.ResponseEntity ResponseEntity <？>}对象（仅限Servlet），用于设置响应标头和内容。 
 * 将使用{@link  plain org.springframework.http.converter.HttpMessageConverter消息转换器}转换ResponseEntity主体并将其写入响应流。 
 *  <li> {<@code> void}，如果方法本身处理响应（通过直接编写响应内容，声明类型为{@link  javax.servlet.ServletResponse} / {@link  javax.servlet .http.HttpServletResponse}或如果视图名称应通过{@link  org.springframework.web.servlet.RequestToViewNameTranslator}隐式确定（未在处理程序方法签名中声明响应参数）。 
 *  </ ul> <p>对于特定的HTTP错误状态，可以将{@code  ExceptionHandler}注释与{@link  ResponseStatus @ResponseStatus}组合在一起。 
