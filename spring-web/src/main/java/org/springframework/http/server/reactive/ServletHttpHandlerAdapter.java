 ...
 */
/**
 * 使用Servlet异步支持和Servlet 3.1非阻塞I / O将{@link  HttpHandler}调整为{@link  HttpServlet}。 
...
public class ServletHttpHandlerAdapter implements Servlet {
	 ...
	 */
	/**
	 * 通过从{@link  #init（ServletConfig）}检查Servlet注册，返回部署Servlet的Servlet路径。 
	 *  
	 * @return 路径； 
	 * 如果在部署Servlet时没有前缀（即"/"或""），则为空字符串； 
	 * 如果在{@link ＃之前调用此方法，则为{@code  null} init（ServletConfig）} Servlet容器回调。 
	 * 
	 */
	@Nullable
	 ...
	 */
/**
 * "））{字符串路径= mapping.substring（0，mapping.length（）-2）; if（！ 
 * path.isEmpty（）&& logger.isDebugEnabled（））{logger.debug（"找到的servlet映射前缀'"+ path +"'代表'"+名称+"'"）;}返回路径;}}抛出新的IllegalArgumentException（"期望单个Servlet映射："+"默认的Servlet映射（即'/'），"+"或基于路径的映射（例如''，'/ foo'）。 
 * "+"实际映射："+ Servlet的映射+"+名称+"'"））} @Override public void service（ServletRequest request，ServletResponse响应）抛出ServletException，IOException {//首先检查现有的错误属性是否（DispatcherType.ASYNC.equals（request.getDispatcherType（）））{Throwable ex =（Throwable）request.getAttribute（WRITE_ERROR_ATTRIBUTE_NAME）;抛出新的ServletException（"Failed创建响应内容"，ex）;} //在Read / WriteListener注册之前启动异步AsyncContext asyncContext = request.startAsync（）; asyncContext.setTimeout（-1）; ServletServerHttpRequest htt pRequest;尝试{httpRequest = createRequest（（（（HttpServletRequest）request），asyncContext）; } catch（URISyntaxException ex）{如果（logger.isDebugEnabled（））{logger.debug（"无法获取请求URL："+ ex.getMessage（））; }（（HttpServletResponse）响应）.setStatus（400）; asyncContext.complete（）;返回; } ServerHttpResponse httpResponse = createResponse（（（（（HttpServletResponse）response），asyncContext，httpRequest）;如果（httpRequest.getMethod（）== HttpMethod.HEAD）{httpResponse = new HttpHeadResponseDecorator（httpResponse）; } AtomicBoolean isCompleted = new AtomicBoolean（）; HandlerResultAsyncListener侦听器=新的HandlerResultAsyncListener（isCompleted，httpRequest）; asyncContext.addListener（listener）; HandlerResultSubscriber订阅者=新的HandlerResultSubscriber（asyncContext，isCompleted，httpRequest）; this.httpHandler.handle（httpRequest，httpResponse）.subscribe（subscriber）; }受保护的ServletServerHttpRequest createRequest（HttpServletRequest请求，AsyncContext上下文）引发IOException，URISyntaxException {Assert.notNull（this.servletPath，"Servlet路径未初始化"）;返回新的ServletServerHttpRequest（请求，上下文，this.servletPath，getDataBufferFactory（），getBufferSize（））; }受保护的ServletServerHttpResponse createResponse（HttpServletResponse响应，AsyncContext上下文，ServletServerHttpRequest请求）引发IOException {返回新的ServletServerHttpResponse（响应，上下文，getDataBufferFactory（），getBufferSize（），request）； 
 *  } @Override public String getServletInfo（）{return""; } @Override @Nullable public ServletConfig getServletConfig（）{返回null； 
 *  } @Override public void destroy（）{}由于以下原因，我们无法将ERROR_LISTENER和HandlerResultSubscriber结合使用：https://issues.jboss.org/browse/WFLY-8515。 
 * 
 */
