 ...
 */
/**
 * {@link  org.springframework.http.converter.HttpMessageConverter HttpMessageConverter}的实现，可以使用<a href ="https://github.com/FasterXML/jackson-读写读写微笑数据格式（"二进制JSON"） dataformats-binary / tree / master / smile">专用的Jackson 2.x扩展名</a>。 
...
public class MappingJackson2SmileHttpMessageConverter extends AbstractJackson2HttpMessageConverter {
	 ...
	 */
	/**
	 * 使用{@code  Jackson2ObjectMapperBuilder}提供的默认配置，构造一个新的{@code  MappingJackson2SmileHttpMessageConverter}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 使用自定义{@link  ObjectMapper}（必须使用{@code  SmileFactory}实例配置）构造一个新的{@code  MappingJackson2SmileHttpMessageConverter}。 
	 ...
	 */
	/**
	 * {@inheritDoc} {@code  ObjectMapper}必须配置一个{@code  SmileFactory}实例。 
	 * 
	 */
	@Override
