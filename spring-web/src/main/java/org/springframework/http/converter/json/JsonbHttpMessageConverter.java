 ...
 */
/**
 * 实现{@link  org.springframework.http.converter.HttpMessageConverter}，可以使用<a href="http://json-b.net/"> JSON绑定API </a>读写JSON。 
 *  <p>此转换器可用于绑定到键入的bean或未键入的{@code  HashMap}。 
 * 默认情况下，它支持{@code  UTF-8}字符集的{@code  application / json}和{@code  application + json}。 
...
public class JsonbHttpMessageConverter extends AbstractJsonHttpMessageConverter {
	 ...
	 */
	/**
	 * 使用默认配置构造一个新的{@code  JsonbHttpMessageConverter}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 使用给定的配置构造一个新的{@code  JsonbHttpMessageConverter}。 
	 *  
	 * @param 为基础委托配置{@code  JsonbConfig}
	 */
	 ...
	 */
	/**
	 * 使用给定的委托构造一个新的{@code  JsonbHttpMessageConverter}。 
	 *  
	 * @param  jsonb要使用的Jsonb实例
	 */
	 ...
	 * 如果未设置，将创建默认的{@code  Jsonb}实例。 
	 *  <p>设置自定义配置的{@code  Jsonb}是进一步控制JSON序列化过程的一种方法。 
	 *  
	 * @see  #JsonbHttpMessageConverter（Jsonb）
	 * @see  #JsonbHttpMessageConverter（JsonbConfig）
	 * @see  JsonbBuilder
