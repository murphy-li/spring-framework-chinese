 ...
 */
/**
 * {@link  org.springframework.http.converter.HttpMessageConverter HttpMessageConverter}的实现，可以使用<ahref读写<ahref="https://cbor.io/"> CBOR </a>数据格式="https://github.com/FasterXML/jackson-dataformats-binary/tree/master/cbor">专用的Jackson 2.x扩展</a>。 
 *  <p>默认情况下，此转换器支持{@link  MediaType＃APPLICATION_CBOR_VALUE}媒体类型。 
 * 可以通过设置{@link  #setSupportedMediaTypessupportedMediaTypes}属性来覆盖此属性。 
 *  <p>默认构造函数使用{@link  Jackson2ObjectMapperBuilder}提供的默认配置。 
 *  <p>与Jackson 2.9及更高版本兼容。 
...
public class MappingJackson2CborHttpMessageConverter extends AbstractJackson2HttpMessageConverter {
	 ...
	 */
	/**
	 * 使用{@code  Jackson2ObjectMapperBuilder}提供的默认配置构造一个新的{@code  MappingJackson2CborHttpMessageConverter}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 使用自定义{@link  ObjectMapper}（必须使用{@code  CBORFactory}实例配置）构造一个新的{@code  MappingJackson2CborHttpMessageConverter}。 
	 *  <p>您可以使用{@link  Jackson2ObjectMapperBuilder}轻松构建它。 
	 *  
	 * @see  Jackson2ObjectMapperBuilder＃cbor（）
	 */
	 ...
	 */
	/**
	 * {@inheritDoc}必须使用{@code  CBORFactory}实例配置{@code  ObjectMapper}。 
	 * 
	 */
	@Override
