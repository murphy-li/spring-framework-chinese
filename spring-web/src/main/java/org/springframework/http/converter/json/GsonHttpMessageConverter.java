 ...
 */
/**
 * 实现{@link  org.springframework.http.converter.HttpMessageConverter}，可以使用<a href="https://code.google.com/p/google-gson/"> Google Gson读写JSON </a>库。 
 *  <p>此转换器可用于绑定到键入的bean或未键入的{@code  HashMap}。 
 * 默认情况下，它支持{@code  UTF-8}字符集的{@code  application / json}和{@code  application + json}。 
 *  <p>经过Gson 2.8测试； 
 * 与Gson 2.0及更高版本兼容。 
 *  @author 罗伊·克拉克森@author  Juergen Hoeller @since 4.1起
 * @see  com.google.gson.Gson 
 * @see  com.google.gson.GsonBuilder 
 * @see  #setGson
 */
public class GsonHttpMessageConverter extends AbstractJsonHttpMessageConverter {
	 ...
	 */
	/**
	 * 使用默认配置构造一个新的{@code  GsonHttpMessageConverter}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 使用给定的委托构造一个新的{@code  GsonHttpMessageConverter}。 
	 *  
	 * @param  gson Gson实例使用@since 5.0
	 */
