 ...
 */
/**
 * 包装并委托给{@link 编码器}的{@code  HttpMessageWriter}。 
 *  <p>还有一个{@code  HttpMessageWriter}，它可以从服务器端可用的额外信息（例如请求或控制器方法注释）中预先解析编码提示。 
 *  @author  Arjen Poutsma @author  Sebastien Deleuze @author  Rossen Stoyanchev @author  Brian Clozel @author  Sam Brannen @since 5.0 
 * @param  <T>输入流中对象的类型
 */
public class EncoderHttpMessageWriter<T> implements HttpMessageWriter<T> {
	 ...
	 */
	/**
	 * 获取有关编码的其他提示，例如，基于服务器请求或来自控制器方法参数的注释。 
	 * 默认情况下，如果它是{@link  HttpMessageEncoder}的实例，则委托给编码器。 
	 * 
	 */
