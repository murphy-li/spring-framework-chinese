 ...
 */
/**
 * {@code  HttpMessageReader}，用于使用Synchronoss NIO Multipart库将{@code "multipart / form-data"}请求解析为{@link  Part}的流。 
 *  <p>可以将此阅读器提供给{@link  MultipartHttpMessageReader}，以便将所有部分聚合到一个Map中。 
 *  @author 塞巴斯蒂安·德勒兹（Sebastien Deleuze）@author  Rossen Stoyanchev @author  Arjen Poutsma @author  Brian Clozel @since 5.0起
 * @see  <a href ="https://github.com/synchronoss/nio-multipart"> Synchronoss NIO Multipart </a> 
 * @see  MultipartHttpMessageReader
 */
public class SynchronossPartHttpMessageReader extends LoggingCodecSupport implements HttpMessageReader<Part> {
	...
	/**
	 * 配置每个部件允许使用的最大内存量。 
	 * 当超出限制时：<ul> <li>文件部分将被写入临时文件。 
	 *  <li>非文件部分将被{@link  DataBufferLimitException}拒绝。 
	 *  </ ul> <p>默认情况下，此设置为256K。 
	 *  
	 * @param  byte以字节为单位计算内存限制； 
	 * 如果设置为-1，则不会强制执行此限制，并且所有部分都可以写入磁盘，并且仅受{@link  #setMaxDiskUsagePerPart（long）maxDiskUsagePerPart}属性的限制。 
	 *  @自5.1.11起
	 */
	 ...
	 */
	/**
	 * 获取{@link  #setMaxInMemorySize已配置}最大内存大小。 
	 *  @自5.1.11起
	 */
	 ...
	 *  <p>默认情况下，它设置为-1。 
	 *  
	 * @param  maxDiskUsagePerPart磁盘限制（以字节为单位），或-1表示不受限制@@5.1.11起
	 */
	 ...
	 */
	/**
	 * 获取{@link  #setMaxDiskUsagePerPart配置}的最大磁盘使用率。 
	 *  @自5.1.11起
	 */
	 ...
	 */
	/**
	 * 订阅输入流并馈入Synchronoss解析器。 
	 * 然后侦听解析器输出，创建零件，并将其推入FluxSink。 
	 * 
	 */
	 ...
	 */
	/**
	 * 侦听解析器输出，并适应{@code  Flux <Sink <Part >>}。 
	 * 
	 */
