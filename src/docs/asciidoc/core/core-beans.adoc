...
The preceding `bean.AppConfig` class is equivalent to the following Spring `<beans/>` XML:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new AnnotationConfigApplicationContext(bean.AppConfig.class);
...
----
	...
	fun main() {
		val ctx = AnnotationConfigApplicationContext(bean.AppConfig::class.java)
...
----
	public static void main(String[] args) {
		...
		ctx.register(bean.AppConfig.class, OtherConfig.class);
...
----
	...
	fun main() {
		...
		ctx.register(bean.AppConfig::class.java, OtherConfig::class.java)
...
assuming that `bean.AppConfig` is declared within the `com.acme` package (or any package
...
----
	<web-app>
		...
		<context-param>
			...
			<param-value>com.acme.bean.AppConfig</param-value>
		</context-param>
...
container. In this series examples, we create a `@Configuration` class named `bean.AppConfig` and
...
`@Configuration` annotation and processes the `@Bean` methods declared in `bean.AppConfig`
properly.
...
----
	<beans>
		...
		<bean class="com.acme.bean.AppConfig"/>
...
NOTE: In `system-test-config.xml` file, the `bean.AppConfig` `<bean/>` does not declare an `id`
...
----
	<beans>
		<!-- picks up and registers bean.AppConfig as a bean definition -->
		<context:component-scan base-package="com.acme"/>
		<context:property-placeholder location="classpath:/com/acme/jdbc.properties"/>
