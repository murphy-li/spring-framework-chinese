 ...
 */
/**
 * 执行与Spring特定注释有关的bean查找的便捷方法，例如Spring的{@link  Qualifier @Qualifier}注释。 
 *  @author  Juergen Hoeller @author 克里斯·比姆斯（Chris Beams）自3.1.2起
 * @see  BeanFactoryUtils
 */
public abstract class BeanFactoryAnnotationUtils {
	 ...
	 */
	/**
	 * 从给定的{@code  BeanFactory}中检索类型为{@code  T}的所有bean，声明与给定匹配的限定符（例如，通过{@code  <qualifier>}或{@code  @Qualifier}）限定符，或具有与给定限定符匹配的Bean名称。 
	 *  
	 * @param  beanFactory用于从中获取目标bean的工厂（也在搜索祖先）
	 * @param  beanType要检索的bean类型
	 * @param 限定符在所有类型匹配项中选择的限定符
	 * @return 匹配的Bean如果无法创建任何匹配的bean，则键入{@code  T} 
	 * @throws  BeansException（从5.1.1开始）
	 ...
	 */
	/**
	 * 从给定的{@code  BeanFactory}获取类型为{@code  T}的bean，声明与给定匹配的限定符（例如，通过{@code  <qualifier>}或{@code  @Qualifier}）限定符，或具有与给定限定符匹配的Bean名称。 
	 *  
	 * @param  beanFactory从中获取目标bean的工厂（也在搜索祖先）
	 * @param  beanType要检索的bean类型
	 * @param 限定符在多个bean匹配之间选择的限定符
	 * @return 匹配的bean类型{@code  T}（从不{<@@code> null}）
	 * @throws 如果找到了多个匹配类型为{@code  T}的bean，则为UniUniqueBeanDefinitionException 
	 * @throws 如果没有匹配类型为{<的匹配豆，则为NoSuchBeanDefinitionException @code> T}如果无法创建Bean，则发现
	 * @throws  BeansException 
	 * @see  BeanFactoryUtils＃beanOfTypeIn includedAncestors（ListableBeanFactory，Class）
	 */
	 ...
	 */
	/**
	 * 从给定的{@code  BeanFactory}获取类型为{@code  T}的bean，声明与给定的限定符匹配的限定符（例如{@code  <qualifier>}或{@code  @Qualifier}） ）。 
	 *  
	 * @param 从工厂获取
	 * @param  bean的类型输入要检索的bean类型
	 * @param 限定符在多个bean匹配之间选择的限定符
	 * @return 类型{<@code > T}（从不{@code  null}）
	 */
	 ...
	 */
	/**
	 * 检查命名的bean是否声明给定名称的限定符。 
	 *  
	 * @param 限定符要匹配的限定符
	 * @param  beanName候选bean的名称
	 * @param  beanFactory从中检索命名bean的工厂
	 * @return  {@code  true}，如果其中一个bean定义（在XML情况下）或Bean的工厂方法（在{@code  @Bean}情况下）定义匹配的限定符值（通过{@code  <qualifier>}或{@code  @Qualifier}） @5.0起
	 */
