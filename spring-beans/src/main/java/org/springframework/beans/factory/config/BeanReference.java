 ...
 * 该接口不一定意味着对实际bean实例的引用。 
 * 它只是表示对bean名称的逻辑引用。 
 *  <p>用作由任何类型的bean引用持有者实现的通用接口，例如{@link  RuntimeBeanReference RuntimeBeanReference}和{@link  RuntimeBeanNameReference RuntimeBeanNameReference}。 
