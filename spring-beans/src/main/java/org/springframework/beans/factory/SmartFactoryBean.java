 ...
 * 实现可能会指示它们是否始终返回独立实例，对于其{@link  #isSingleton（）}实现返回{@code  false}的情况并未明确指示独立实例的情况。 
 * 如果未实现此扩展接口的<p>纯{@link  FactoryBean}实现，只要它们的{@link  #isSingleton（）}实现返回{@code  false}，就假定它们总是返回独立实例。 
 * 暴露的对象仅按需访问。 
 *  <p> <b>注意：</ b>此接口是一个专用接口，主要供框架内部和协作框架内部使用。 
 * 通常，应用程序提供的FactoryBeans应该仅实现简单的{@link  FactoryBean}接口。 
 * 即使在点发行版中，新方法也可能会添加到此扩展接口中。 
...
public interface SmartFactoryBean<T> extends FactoryBean<T> {
	 ...
	 */
	/**
	 * 该工厂管理的对象是否是原型？也就是说，{<@link> #getObject（）}是否总是返回一个独立的实例？ <p> FactoryBean本身的原型状态通常由拥有的{@link  BeanFactory}提供； 
	 * 通常，它必须在那里定义为单例。 
	 *  <p>该方法应该严格检查独立实例； 
	 * 对于范围对象或其他类型的非单一，非独立对象，它不应返回{@code  true}。 
	 * 因此，这不只是{@link  #isSingleton（）}的倒置形式。 
	 *  <p>默认实现返回{@code  false}。 
	 *  
	 * @return 所暴露的对象是否为原型
	 * @see  #getObject（）
	 ...
	 */
	/**
	 * 此FactoryBean是否期望进行急切的初始化，即急于初始化自身以及期望对其单例对象（如果有）进行急切的初始化？ <p>不希望标准的FactoryBean急于初始化：其{@link  #getObject（）}仅在实际访问时被调用，即使是单例对象也是如此。 
	 * 从此方法返回{@code  true}表示应该急切调用{@link  #getObject（）}，同时也急切地应用后处理器。 
	 * 在{@link  #isSingleton（）单例}对象的情况下，这可能是有道理的，特别是如果后处理器希望在启动时应用。 
	 *  <p>默认实现返回{@code  false}。 
	 *  
	 * @return 是否进行急切的初始化
	 * @see  org.springframework.beans.factory.config.ConfigurableListableBeanFactory＃preInstantiateSingletons（）
