 ...
 */
/**
 * 尝试通过{@link  org.springframework.core.io.ResourceLoader}（通常，相对于{@code  ApplicationContext}的资源库）解析实体引用的{@code  EntityResolver}实现，如果适用。 
 * 扩展{@link  DelegatingEntityResolver}以提供DTD和XSD查找。 
 *  <p>允许使用标准XML实体将XML代码片段包含到应用程序上下文定义中，例如，将大型XML文件拆分为各种模块。 
 * 包含路径可以照常相对于应用程序上下文的资源库，而不是相对于JVM工作目录（XML解析器的默认值）。 
 *  <p>注意：除了相对路径之外，在当前系统根目录中指定文件的每个URL（即JVM工作目录）也将相对于应用程序上下文进行解释。 
 *  @author  Juergen Hoeller @2003年7月31日起
 * @see  org.springframework.core.io.ResourceLoader 
 * @see  org.springframework.context.ApplicationContext
 */
public class ResourceEntityResolver extends DelegatingEntityResolver {
	 ...
	 *  
	 * @param  resourceLoader用于加载XML实体的ResourceLoader（或ApplicationContext）包括
	 */
