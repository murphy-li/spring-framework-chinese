 ...
 *  <p>此类加载DOM文档并将BeanDefinitionDocumentReader应用于该文档。 
 * 文档阅读器将向给定的bean工厂注册每个bean定义，并讨论后者的{@link  org.springframework.beans.factory.support.BeanDefinitionRegistry}接口的实现。 
 *  @author  Juergen Hoeller @author  Rob Harrop @author  Chris Beams @2003年11月26日
 * @see  #setDocumentReaderClass 
 * @see  BeanDefinitionDocumentReader 
 * @see  DefaultBeanDefinitionDocumentReader 
 * @see  BeanDefinitionRegistry 
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * 设置是否使用XML验证。 
	 * 默认值为{@code  true}。 
	 *  <p>如果关闭验证，此方法将启用名称空间意识，以便在这种情况下仍能正确处理架构名称空间。 
	 *  
	 * @see  #setValidationMode 
	 * @see  #setNamespaceAware
	 */
	 ...
	 * 默认为{@link  #VALIDATION_AUTO}。 
	 *  <p>请注意，这只会激活或停用验证本身。 
	 * 如果要关闭模式文件的验证，则可能需要显式激活模式名称空间支持：请参见{@link  #setNamespaceAware}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置XML解析器是否应支持XML名称空间。 
	 * 默认值为"false"。 
	 *  <p>当模式验证处于活动状态时，通常不需要这样做。 
	 * 但是，未经验证，必须将其切换为"true"才能正确处理模式名称空间。 
	 * 
	 */
	 ...
	 *  <p>默认实现是{@link  org.springframework.beans.factory.parsing.FailFastProblemReporter}，它表现出快速失败的行为。 
	 * 外部工具可以提供替代实施，以整理错误和警告以显示在工具UI中。 
	 * 
	 */
	 ...
	 *  <p>默认实现是EmptyReaderEventListener，它将丢弃每个事件通知。 
	 * 外部工具可以提供替代实现，以监视在BeanFactory中注册的组件。 
	 * 
	 */
	 ...
	 *  <p>默认实现是{@link  NullSourceExtractor}，它仅返回{@code  null}作为源对象。 
	 * 这意味着-在正常运行时执行期间-不会将其他源元数据附加到Bean配置元数据。 
	 * 
	 */
	 ...
	 *  <p>如果未指定任何内容，将通过{@link  #createDefaultNamespaceHandlerResolver（）}创建默认实例。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回要使用的EntityResolver，如果未指定，则构建默认的解析器。 
	 * 
	 */
	 ...
	 *  <p>如果未设置，则使用默认的SimpleSaxErrorHandler，它仅使用视图类的记录器实例记录警告，并重新抛出错误以中止XML转换。 
	 *  
	 * @see  SimpleSaxErrorHandler
	 */
	 ...
	 */
	/**
	 * 指定要使用的{@link  BeanDefinitionDocumentReader}实现，负责XML Bean定义文档的实际读取。 
	 ...
	 * 从指定的XML文件加载bean定义。 
	 *  
	 * @param 编码为XML文件提供资源描述符，允许指定用于解析文件的编码
	 * @return 找到的bean定义的数量
	 ...
	 * 从指定的XML文件加载bean定义。 
	 *  
	 * @param  inputSource SAX InputSource从
	 * @return 读取找到的bean定义的数量
	 ...
	 *  
	 * @param  inputSource从
	 * @param  resource读取的SAX InputSource资源描述对资源的描述（可以为{@code  null}或为空）
	 * @return 找到的bean定义数
	 * @throws  BeanDefinitionStoreException加载或解析错误的情况
	 */
	 ...
	 *  
	 * @param  inputSource SAX InputSource从
	 * @param 资源读取XML文件的资源描述符
	 * @return 找到的bean定义数
	 * @throws  BeanDefinitionStoreException，如果加载或解析错误，请参见
	 * @see ＃ doLoadDocument 
	 * @see  #registerBeanDefinitions
	 */
	...
	/**
	 * 确定指定的{@link 资源}的验证模式。 
	 * 如果未配置任何显式验证模式，则验证模式将从给定资源中获取{@link  #detectValidationMode}。 
	 *  <p>即使您设置了{@link  #VALIDATION_AUTO}以外的其他内容，也希望完全控制验证模式，请重写此方法。 
	 *  
	 * @see  #detectValidationMode
	 */
	 ...
	 */
	/**
	 * 检测对提供的{@link 资源}标识的XML文件执行哪种验证。 
	 * 如果文件具有{@code  DOCTYPE}定义，则使用DTD验证，否则采用XSD验证。 
	 *  <p>如果您要自定义{@link  #VALIDATION_AUTO}模式的分辨率，请重写此方法。 
	 * 
	 */
	 ...
	 * @return 找到的bean定义数
	 * @throws 发生错误时BeanDefinitionStoreException 
	 * @see  #loadBeanDefinitions <
	 * @see > #setDocumentReaderClass 
	 * @see  BeanDefinitionDocumentReader＃registerBeanDefinitions
	 */
	 ...
	 */
	/**
	 * 创建{@link  BeanDefinitionDocumentReader}，以用于实际从XML文档读取bean定义。 
