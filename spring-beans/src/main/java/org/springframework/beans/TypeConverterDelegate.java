 ...
 *  {@link  BeanWrapperImpl}和{@link  SimpleTypeConverter}用作委托。 
 *  @author  Juergen Hoeller @author  Rob Harrop @author  Dave Syer @since 2.0起
 * @see  BeanWrapperImpl 
 * @see  SimpleTypeConverter
 */
class TypeConverterDelegate {
	 ...
	 *  
	 * @param  propertyEditorRegistry要使用的编辑器注册表
	 */
	 ...
	 *  
	 * @param  propertyEditorRegistry编辑编辑器注册表以使用
	 * @param  targetObject作为目标对象（作为可以传递给编辑器的上下文）
	 */
	 ...
	 * @param  oldValue先前的值（如果可用）（可能为{@code  null}）
	 * @param  newValue建议的新值
	 * @param  requiredType必须转换的类型到（或{@code  null}，如果未知，例如在集合元素的情况下）<
	 * @return>新值，可能是类型转换的结果
	 * @throws  IllegalArgumentException如果类型转换失败
	 */
	@Nullable
	 ...
	 */
	/**
	 * 将值转换为指定属性的所需类型（必要时从String转换）。 
	 *  
	 * @param  propertyName属性名称
	 * @param  oldValue先前的值（如果可用）（可能为{@code  null}）
	 * @param  newValue建议的新值
	 * @param  requiredType必须转换的类型到（或{@code  null}，如果未知，例如在集合元素的情况下）
	 * @param  typeDescriptor目标属性或字段的描述符
	 * @return 新值，可能是类型转换的结果
	 * @throws  IllegalArgumentException如果类型转换失败
	 */
	 ...
	 * 查找给定类型的默认编辑器。 
	 *  
	 * @param  requiredType键入类型以为
	 * @return 查找相应的编辑器找到一个编辑器； 
	 * 如果没有，则为{@code  null}
	 */
	@Nullable
	 ...
	 */
	/**
	 * 使用给定的属性编辑器将值转换为所需的类型（如果需要，则从字符串）。 
	 *  
	 * @param  oldValue先前的值（如果可用）（可能为{@code  null}）
	 * @param  newValue建议的新值
	 * @param  requiredType我们必须转换为的类型（或{@code  null }（如果未知），例如在集合元素的情况下）
	 * @param 编辑器PropertyEditor使用
	 * @return 新值，可能是类型转换的结果
	 * @throws  IllegalArgumentException如果类型转换失败
	 */
	@Nullable
