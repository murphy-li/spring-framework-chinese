 ...
 */
/**
 * 一个基本的{@link  ConfigurablePropertyAccessor}，它为所有典型的用例提供了必要的基础结构。 
 *  <p>如有必要，此访问器会将集合和数组值转换为相应的目标集合或数组。 
 * 可以通过PropertyEditor的{@code  setValue}来编写处理集合或数组的自定义属性编辑器，也可以通过{@code  setAsText}来对逗号分隔的字符串进行编写，因为如果将字符串数组转换为这种格式，数组本身不可分配。 
 *  @author  Juergen Hoeller @author  Stephane Nicoll @author  Rod Johnson @author  Rob Harrop @since 4.2起
 * @see  #registerCustomEditor 
 * @see  #setPropertyValues 
 * @see  #setPropertyValue 
 * @see ＃ getPropertyValue 
...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	 ...
	 * 被包装的实例之后需要设置。 
	 *  
	 * @param  registerDefaultEditors是否注册默认编辑器（如果访问器不需要任何类型转换，则可以取消显示）
	 * @see  #setWrappedInstance
	 */
	 ...
	 * @param 该访问器包装的对象对象
	 * @param  nestedPath对象
	 * @param 的嵌套路径父级（包含访问者）（必须不是{@code  null}）
	 */
	 ...
	 */
	/**
	 * 切换目标对象，仅在新对象的类与替换对象的类不同时才替换缓存的自省结果。 
	 *  
	 * @param 对象新的目标对象
	 */
	 ...
	 */
	/**
	 * 为指定的{@code  propertyName}返回{@link  PropertyHandler}，如有必要，进行导航。 
	 * 如果未找到，则返回{@code  null}，而不是引发异常。 
	 *  
	 * @param  propertyName属性以获得要获取的描述符的属性
	 * @return 指定属性的属性描述符，如果找不到，则返回{@code  null} 
	 * @throws  BeansException如果自省失败
	 */
	@Nullable
	 ...
	 */
	/**
	 * 返回指定本地{@code  propertyName}的{@link  PropertyHandler}。 
	 * 仅用于访问当前上下文中可用的属性。 
	 *  
	 * @param  propertyName本地属性的名称
	 * @return 该属性的处理程序； 
	 * 如果未找到，则为{@code  null}
	 */
	@Nullable
	 ...
	 *  
	 * @param 对象与此PropertyAccessor包装的对象
	 * @param  nestedPath对象的嵌套路径
	 ...
	 *  
	 * @param  pa属性访问器可用于
	 * @param  nestedPath属性路径，我们知道该路径是嵌套的
	 * @return 路径的最后一部分（目标bean上的属性）
	 */
	 ...
	 * 检索给定嵌套属性的属性访问器。 
	 * 如果在缓存中找不到，请创建一个新的。 
	 *  <p>注意：现在必须缓存嵌套的PropertyAccessors，以保留注册的嵌套属性的自定义编辑器。 
	 *  
	 * @param  nestedProperty属性可为
	 * @return  PropertyAccessor实例（缓存或新创建）创建PropertyAccessor
	 */
