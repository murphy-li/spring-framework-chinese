 ...
 */
/**
 * {@code  java.net.URI}的编辑器，以直接填充URI属性而不是使用String属性作为桥。 
 *  <p>支持Spring样式的URI表示法：任何完全限定的标准URI（"文件："，"http："等）和Spring的特殊"类路径："伪URL，它将解析为相应的URI。 
 *  <p>默认情况下，此编辑器会将字符串编码为URI。 
 * 例如，空格将被编码为{@code ％20}。 
 * 可以通过调用{@link  #URIEditor（boolean）}构造函数来更改此行为。 
 *  <p>注意：URI比URL更宽松，因为它不需要指定有效的协议。 
 * 即使未注册匹配的协议处理程序，也允许使用有效URI语法内的任何方案。 
...
public class URIEditor extends PropertyEditorSupport {
	 ...
	 */
	/**
	 * 创建一个新的编码URIEditor，将"classpath："位置转换为标准URI（而不是尝试将其解析为物理资源）。 
	 * 
	 */
	public URIEditor() {
		this(true);
	 ...
	 */
	/**
	 * 创建一个新的URIEditor，将"classpath："位置转换为标准URI（而不是尝试将其解析为物理资源）。 
	 *  
	 * @param  encode表示是否将对字符串进行编码，自3.0起
	 */
	 ...
	 */
	/**
	 * 使用给定的ClassLoader创建一个新的URIEditor，以将"classpath："位置解析为物理资源URL。 
	 *  
	 * @param  classLoader用于解析"classpath："位置的ClassLoader（可以为{@code  null}，以指示默认的ClassLoader）
	 */
	 ...
	 * 使用给定的ClassLoader创建一个新的URIEditor，以将"classpath："位置解析为物理资源URL。 
	 *  
	 * @param  classLoader用于解析"classpath："位置的ClassLoader（可能为{@code  null}表示默认的ClassLoader）
	 * @param  encode表示是否将对字符串进行编码，从3.0开始
	 */
	 ...
	 * @param 值将转换为URI实例的值
	 * @return  URI实例
	 * @throws  java.net.URISyntaxException如果URI转换失败
	 */
