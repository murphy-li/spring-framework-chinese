 ...
 */
/**
 * 适用于BEA WebLogic（9.0及更高版本）的特殊{@link  JtaTransactionManager}变体。 
 * 支持WebLogic事务协调器上的Spring事务定义的全部功能，超越标准JTA </ i>：事务名称，每个事务的隔离级别以及在所有情况下正确恢复事务。 
 *  <p>使用WebLogic的特殊{@code  begin（name）}方法启动JTA事务，以使<b> Spring驱动的事务在WebLogic的事务监视器中可见</ b>。 
 * 如果使用Spring的声明式事务，则默认情况下，公开的名称将为完全限定的类名称+""。 
 *  +方法名称。 
 *  <p>通过WebLogic的相应JTA事务属性"ISOLATION LEVEL"支持<b>每个事务隔离级别</ b>。 
 * 这会将指定的隔离级别（例如ISOLATION_SERIALIZABLE）应用于所有参与给定事务的JDBC连接。 
 *  <p>如果标准JTA恢复失败，则调用WebLogic的特殊{@code  forceResume}方法； 
 * 如果目标事务被标记为仅回滚，则<b>也恢复。 
 * 如果您首先不依赖于事务挂起的此功能，那么Spring的标准JtaTransactionManager也将正常运行。 
 *  <p>默认情况下，直接从WebLogic的{@code  TransactionHelper}获取JTA UserTransaction和TransactionManager句柄。 
 * 可以通过指定"userTransaction"/"userTransactionName"和"transactionManager"/"transactionManagerName"，传入现有句柄或指定要查找的相应JNDI位置来覆盖此设置。 
 *  <p> <b>注意：此JtaTransactionManager旨在改进Spring方面的特定事务划分行为。 
 * 它将与您的持久性提供程序中独立配置的WebLogic事务策略愉快地共存，而无需以任何方式专门连接这些设置。 
 * </ b> @author  Juergen Hoeller @since 1.1起
 * @see  org.springframework。 
 *  transaction.TransactionDefinition＃getName 
 * @see  org.springframework.transaction.TransactionDefinition＃getIsolationLevel 
 * @see  weblogic.transaction.UserTransaction＃begin（String）
 * @see  weblogic.transaction.Transaction＃setProperty 
 * @see  weblogic.transaction .TransactionManager＃forceResume 
		...
		if (this.weblogicUserTransactionAvailable && definition.getName() != null) {
			try {
				if (timeout > TransactionDefinition.TIMEOUT_DEFAULT) {
					...
					*/
					/**
					 * weblogic.transaction.UserTransaction wut =（weblogic.transaction.UserTransaction）ut; wut.begin（definition.getName（），超时）； 
					 * 
					 */
		...
		if (this.weblogicTransactionManagerAvailable) {
			if (definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT) {
				try {
					...
					*/
					/**
					 * weblogic.transaction.Transaction wtx =（weblogic.transaction.Transaction）tx; wtx.setProperty（ISOLATION_LEVEL_KEY，隔离级别）;
					 */
