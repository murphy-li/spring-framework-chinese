 ...
 */
/**
 * {@link  org.springframework.beans.factory.xml.BeanDefinitionParser BeanDefinitionParser}实现使用户可以轻松配置启用注释驱动的事务划分所需的所有基础结构bean。 
 *  <p>默认情况下，所有代理均创建为JDK代理。 
 * 如果将对象作为具体的类而不是接口注入，则可能会引起一些问题。 
 * 要克服此限制，您可以将'{@code  proxy-target-class}'属性设置为'{@code  true}'，这将导致创建基于类的代理。 
 *  @author  Juergen Hoeller @author  Rob Harrop @author 克里斯·比姆斯（Chris Beams）@author  Stephane Nicoll @since 2.0
 */
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	 ...
	 */
	/**
	 * 解析{@code  <tx：annotation-driven />}标记。 
