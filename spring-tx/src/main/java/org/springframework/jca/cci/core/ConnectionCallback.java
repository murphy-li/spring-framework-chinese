...
/**
 * 在CCI连接上运行的代码的通用回调接口。 
 * 允许使用任何类型和数量的交互在单个Connection上执行任何数量的操作。 
 *  <p>这对于委派现有的数据访问代码特别有用，该代码希望Connection可以继续工作并抛出ResourceException。 
...
public interface ConnectionCallback<T> {
	 ...
	 */
	/**
	 * 由具有活动CCI连接的{@code  CciTemplate.execute}调用。 
	 * 无需关心激活或关闭连接或处理事务。 
	 *  <p>如果在没有线程绑定CCI事务（由CciLocalTransactionManager发起）的情况下调用，则代码将简单地以其事务语义在CCI连接上执行。 
	 * 如果将CciTemplate配置为使用支持JTA的ConnectionFactory，则在JTA事务处于活动状态时，CCI连接以及回调代码将是事务性的。 
	 ...
	 * @return 一个结果对象，如果没有，则返回{@code  null} <@如果是CCI方法抛出的，则throws> ResourceException，如果被ResultSet方法抛出，则将被自动转换为DataAccessException 
