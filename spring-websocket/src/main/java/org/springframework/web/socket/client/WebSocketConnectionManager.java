 ...
 */
/**
 * 赋予了URI，{<@link> WebSocketClient}和{@link  WebSocketHandler}的WebSocket连接管理器通过{@link  #start（）}和{<@link > #stop（）}方法。 
 * 如果{@link  #setAutoStartup（boolean）}设置为{@code  true}，这将在刷新Spring ApplicationContext时自动完成。 
...
public class WebSocketConnectionManager extends ConnectionManagerSupport {
	 ...
	 *  <p>默认情况下，添加了{@link  LoggingWebSocketHandlerDecorator}。 
	 * 
	 */
	...
	/**
	 * 设置要使用的子协议。 
	 * 如果已配置，则将通过{@code  Sec-WebSocket-Protocol}标头在握手中请求指定的子协议。 
	 * 生成的WebSocket会话将包含服务器接受的协议（如果有）。 
	 * 
	 */
