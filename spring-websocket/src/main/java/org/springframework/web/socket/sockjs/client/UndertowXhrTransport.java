 ...
 * 从Spring Framework 5.0开始，需要Undertow 1.3或1.4，包括XNIO。 
 *  <p>当用于测试目的（例如负载测试）或用于特定用例（例如HTTPS配置）时，应提供自定义OptionMap：<pre class ="code"> OptionMap optionMap = OptionMap.builder（）.set（ Options.WORKER_IO_THREADS，8）.set（Options.TCP_NODELAY，true）.set（Options.KEEP_ALIVE，true）.set（Options.WORKER_NAME，"SockJSClient"）.getMap（）; UndertowXhrTransport transport =新的UndertowXhrTransport（optionMap）; </ pre> @author  Brian Clozel @author  Rossen Stoyanchev @4.1.2起
 * @see  org.xnio.Options
 */
public class UndertowXhrTransport extends AbstractXhrTransport {
	 ...
	 */
	/**
	 * 返回{@link  org.xnio.XnioWorker}，以支持Undertow的HTTP客户端的I / O操作。 
	 *  
	 * @see  org.xnio.Xnio
	 */
