 ...
 * WebSocket扩展将协议功能添加到WebSocket协议中。 
 * 会话中使用的扩展在握手阶段进行协商，如下所示：<ul> <li>客户端可以在HTTP握手请求中要求特定的扩展</ li> <li>服务器以最终的扩展列表作为响应在当前会话中使用</ li> </ ul> <p> WebSocket扩展HTTP标头可能包含参数，并遵循<a href="https://tools.ietf.org/html/rfc7230#section-3.2"> RFC 7230第3.2节</a> </ p> <p>请注意，HTTP标头中的扩展顺序定义了它们的执行顺序，例如扩展名"foo，bar"将被执行为"bar（foo（message））"。 
 * </ p> @author  Brian Clozel @author  Juergen Hoeller @since 4.0 
 * @see  <a href ="https： //tools.ietf.org/html/rfc6455#section-9">WebSocket协议扩展，RFC 6455-第9节</a>
 */
public class WebSocketExtension {
	 ...
	 */
	/**
	 * 使用给定的名称和参数创建一个WebSocketExtension。 
	 *  
	 * @param 名称扩展名
	 * @param 参数参数
	 */
	 ...
	 */
	/**
	 * 将给定的逗号分隔的字符串解析为{@code  WebSocketExtension}对象的列表。 
	 *  <p>此方法可用于解析"Sec-WebSocket-Extension"标头。 
	 *  
	 * @param 扩展要解析的字符串
	 * @return 扩展列表
	 * @throws  IllegalArgumentException如果无法解析字符串
	 */
