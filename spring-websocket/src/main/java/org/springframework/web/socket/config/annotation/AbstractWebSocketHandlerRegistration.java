 ...
 */
/**
 * {@link  WebSocketHandlerRegistration WebSocketHandlerRegistrations}的基类收集所有配置选项，但允许子类将实际的HTTP请求映射放在一起。 
 *  @author  Rossen Stoyanchev @author 塞巴斯蒂安·德勒兹（Sebastien Deleuze）@从4.0起
 * @param  <M>映射类型
 */
public abstract class AbstractWebSocketHandlerRegistration<M> implements WebSocketHandlerRegistration {
	 ...
	 */
	/**
	 * 公开{@code  SockJsServiceRegistration}（如果启用了SockJS； 
	 * 否则，公开{@code  null}），以便在应用程序不提供时，可以使用TaskScheduler对其进行配置。 
	 * 这应该在调用{@link  #getMappings（）}之前完成。 
	 * 
	 */
	@Nullable
