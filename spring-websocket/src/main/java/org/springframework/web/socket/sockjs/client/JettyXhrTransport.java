 ...
 *  <p>用于测试目的（例如，负载测试）时，必须将{@code  HttpClient}属性设置为允许比平时更多的连接和线程数。 
 * 例如：<pre class ="code"> HttpClient httpClient = new HttpClient（）; httpClient.setMaxConnectionsPerDestination（1000）; httpClient.setExecutor（new QueuedThreadPool（500））; </ pre> @author  Rossen Stoyanchev @始于4.1
 */
public class JettyXhrTransport extends AbstractXhrTransport implements Lifecycle {
	 ...
	 */
	/**
	 * Jetty客户端{@link  org.eclipse.jetty.client.api.Response.Listener响应侦听器}将响应的主体拆分为SockJS框架，并将它们委托给{@link  XhrClientSockJsSession}。 
	 * 
	 */
