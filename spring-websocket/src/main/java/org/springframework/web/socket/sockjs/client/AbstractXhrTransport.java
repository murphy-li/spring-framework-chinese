...
public abstract class AbstractXhrTransport implements XhrTransport {
	 ...
	 */
	/**
	 * {@code  XhrTransport}可以同时支持SockJS服务器的"xhr_streaming"和"xhr"传输。 
	 * 从客户角度看，实现上没有区别。 
	 *  <p>通常，首先将{@code  XhrTransport}用作"XHR流"，然后，如果失败，则用作"XHR"。 
	 * 但是，在某些情况下，抑制XHR流可能会有所帮助，以便仅尝试XHR。 
	 *  <p>默认情况下，此属性设置为{@code  false}，这意味着"XHR流"和"XHR"均适用。 
	 * 
	 */
