 ...
 */
/**
 * 一个{@link  org.springframework.beans.factory.xml.BeanDefinitionParser}，它为{@code  <websocket：message-broker />} XML名称空间元素提供配置。 
 *  <p>使用订单1注册一个Spring MVC {@link  org.springframework.web.servlet.HandlerMapping}，以映射来自STOMP / WebSocket客户端的HTTP WebSocket握手请求。 
 *  <p>注册以下{@link  org.springframework.messaging.MessageChannel MessageChannels}：<ul> <li>"clientInboundChannel"，用于接收来自客户端（例如WebSocket客户端）的消息<li>"clientOutboundChannel"，用于将消息发送到客户端（例如WebSocket客户端）<li>"brokerChannel"，用于将消息从应用程序内发送到消息代理</ ul> <p>根据所选的消息代理选项注册以下项之一：<ul> <li> a {@link  SimpleBrokerMessageHandler}（如果使用<simple-broker /> <li>）{{@@link> StompBrokerRelayMessageHandler}（如果使用<stomp-broker-relay />）</ ul> <p>注册{ @link  UserDestinationMessageHandler}用于处理用户目的地。 
 *  @author  Brian Clozel @author  Rossen Stoyanchev @从4.0开始
 */
