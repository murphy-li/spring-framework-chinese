 ...
 *  <p>此外，此类可以{@@link> plain #toHttpHandler（RouterFunction）将{@code  RouterFunction}转换为{@code  HttpHandler}，可以在Servlet 3.1 +，Reactor，或Undertow。 
...
public abstract class RouterFunctions {
	 ...
	 */
	/**
	 * 包含{@link  ServerRequest}的{@link  ServerWebExchange}属性的名称。 
	 * 
	 */
	 ...
	 */
	/**
	 * {@link  ServerWebExchange}属性的名称，该属性包含URI模板映射，将变量名称映射为值。 
	 * 
	 */
	 ...
	 */
	/**
	 * 包含匹配模式的{@link  ServerWebExchange＃getAttributes（）属性}的名称，作为{@link  org.springframework.web.util.pattern.PathPattern}。 
	 * 
	 */
	 ...
	 * 返回的函数可以是{@link  plain Function＃andThen（Function）composition}，例如，当查找函数不匹配时，返回默认资源：<pre class ="code"> Mono <Resource> defaultResource = Mono.just（new ClassPathResource（"index.html"））; Function <ServerRequest，Mono <Resource >> lookupFunction = RouterFunctions.resourceLookupFunction（"/ resources"，new FileSystemResource（"public-resources /"））.andThen（resourceMono-> resourceMono.switchIfEmpty（defaultResource））; RouterFunction <ServerResponse>资源= RouterFunctions.resources（lookupFunction）; </ pre> 
	 * @param 模式化模式以匹配
	 ...
	 */
	/**
	 * 将给定的{@link  plain RouterFunction路由器函数}转换为{@link  HttpHandler}。 
	 * 此转换使用{@link  plain HandlerStrategies＃builder（）默认策略}。 
	 *  <p>使用{@link  org.springframework.http.server.reactive.ServletHttpHandlerAdapter}，</ li> <li>使用Reactor的返回器，可以适应在<ul> <li> Servlet 3.1+中运行。 
	 *  {@link  org.springframework.http.server.reactive.ReactorHttpHandlerAdapter}，</ li> <li>使用{@link  org.springframework.http.server.reactive.UndertowHttpHandlerAdapter}的Undertow。 
	 * </ li > </ ul> <p>请注意，{<@code> HttpWebHandlerAdapter}还实现了{@link  WebHandler}，从而允许通过{@link  WebHttpHandlerBuilder}进行其他过滤器和异常处理程序注册。 
	 *  
	 * @param  routerfunction路由器功能，用于转换
	 * @return 使用给定路由器功能处理HTTP请求的http处理程序
	 */
	 ...
	 */
	/**
	 * 使用给定的策略将给定的{@link  plain RouterFunction路由器函数}转换为{@link  HttpHandler}。 
	 *  <p>返回的{@code  HttpHandler}可以使用{@link  org.springframework.http.server.reactive.ServletHttpHandlerAdapter}，</ li>修改为在<ul> <li> Servlet 3.1+中运行。 
	 *  > <li> Reactor使用{@link  org.springframework.http.server.reactive.ReactorHttpHandlerAdapter}，</ li> <li> Undertow使用{@link  org.springframework.http.server.reactive。 
	 *  UndertowHttpHandlerAdapter}。 
	 * </ li> </ ul> 
	 * @param 路由器功能转换
	 * @param 策略的路由器功能策略使用
	 * @return 使用给定路由器功能处理HTTP请求的http处理程序的策略
	 */
	 ...
	 */
	/**
	 * 将给定的{@link  plain RouterFunction路由器功能}转换为{@link  WebHandler}。 
	 * 此转换使用{@link  plain HandlerStrategies＃builder（）默认策略}。 
	 *  
	 * @param  routerfunction路由器功能，用于转换
	 * @return 使用给定路由器功能处理Web请求的Web处理程序
	 */
	 ...
	 */
	/**
	 * 使用给定的策略将给定的{@link  plain RouterFunction路由器功能}转换为{@link  WebHandler}。 
	 *  
	 * @param  routerFunction用于转换
	 * @param 策略的路由器功能策略使用
	 * @return 使用给定路由器功能处理Web请求的Web处理程序的策略
	 */
	 ...
	 */
	/**
	 * 如果此路由具有{@link  plain Mono＃empty（）no result}，则该组合路由功能将首先调用一个功能，然后调用另一个功能（具有相同的响应类型{@code  T}）。 
	 *  
	 * @param  <T>服务器响应类型
	 */
	 ...
	 */
	/**
	 * 如果此路由具有{@link  plain Mono＃empty（）no result}，则该组合路由功能将首先调用一个功能，然后调用另一个功能（响应类型不同）。 
	 * 
	 */
