 ...
 */
/**
 * 使用FreeMarker模板引擎的{@code  View}实现。 
 *  <p>取决于可在应用程序上下文中访问的单个{@link  FreeMarkerConfig}对象，例如{@link  FreeMarkerConfigurer}。 
 * 或者，可以通过{@link  #setConfiguration}在此类上直接设置FreeMarker {@link 配置}。 
 *  <p> {<@link> #setUrl（String）url}属性是FreeMarker模板相对于FreeMarkerConfigurer的{@link  FreeMarkerConfigurer＃setTemplateLoaderPath templateLoaderPath}的位置。 
...
public class FreeMarkerView extends AbstractUrlBasedView {
	 ...
	 */
	/**
	 * 模板模型中{@link  RequestContext}实例的属性名称，可用于Spring的宏-例如，用于创建{@link  org.springframework.web.reactive.result.view.BindStatus BindStatus}对象。 
	 *  @since 5.2 
	 * @see  #setExposeSpringMacroHelpers（boolean）
	 */
	 ...
	 */
	/**
	 * 设置此视图使用的FreeMarker {@link 配置}。 
	 *  <p>通常不直接设置此属性。 
	 * 取而代之的是，在Spring应用程序上下文中应使用单个{@link  FreeMarkerConfig}来获取FreeMarker配置。 
	 * 
	 */
	 ...
	 */
	/**
	 * 获取FreeMarker {@link 配置}进行实际使用。 
	 *  
	 * @return  FreeMarker配置（永不{@code  null}）
	 * @throws 在没有{{@code> Configuration}对象集的情况下IllegalStateException 
	 * @see  #getConfiguration（）
	 */
	...
	/**
	 * 设置FreeMarker模板文件的编码。 
	 *  <p>默认情况下，{<@link> FreeMarkerConfigurer}将FreeMarker配置中的默认编码设置为"UTF-8"。 
	 * 如果所有模板共享一个公共编码，建议您在FreeMarker {@link  Configuration}中指定编码，而不是每个模板。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置是否以{@value #SPRING_MACRO_REQUEST_CONTEXT_ATTRIBUTE}的名称公开{@link  RequestContext}供Spring的宏库使用。 
	 *  <p>默认值为{@code  true}。 
	 *  <p> Spring的FreeMarker默认宏所需要。 
	 ...
	 */
	/**
	 * 在{@code  ApplicationContext}中自动检测一个{@link  FreeMarkerConfig}对象。 
	 *  
	 * @return 用于此视图的{@code  FreeMarkerConfig}实例
	 * @throws  BeansException如果找不到{{@code> FreeMarkerConfig}实例
	 * @see  #setConfiguration
	 */
	 ...
	 */
	/**
	 * 通过潜在地暴露供Spring FreeMarker宏使用的{@link  RequestContext}，然后委托给该方法的继承实现，来准备用于呈现的模型。 
	 *  @since 5.2 
	 * @see  #setExposeSpringMacroHelpers（boolean）
	 * @see  org.springframework.web.reactive.result.view.AbstractView＃getModelAttributes（Map，ServerWebExchange）
	 */
	 ...
	 */
	/**
	 * 如果未指定，则获取配置的FreeMarker {@link  ObjectWrapper}，或获取{@link  plain ObjectWrapper＃DEFAULT_WRAPPER默认包装器}。 
	 *  
	 * @see  freemarker.template.Configuration＃getObjectWrapper（）
	 */
	 ...
	 */
	/**
	 * 获取给定语言环境的FreeMarker模板，此视图将呈现该模板。 
