 ...
 */
/**
 * 用于通过使用{@link  HttpMessageReader}读取请求主体来解析方法参数的参数解析器的抽象基类。 
 *  <p>如果方法参数用{@code  @javax.validation.Valid}或{@link  org.springframework.validation.annotation.Validated}注释，则应用验证。 
...
public abstract class AbstractMessageReaderArgumentResolver extends HandlerMethodArgumentResolverSupport {
	 ...
	 */
	/**
	 * 具有{@link  HttpMessageReader}和{@link  Validator}的构造方法。 
	 *  
	 * @param 读者读者从请求主体转换
	 */
	 ...
	 */
	/**
	 * 构造函数，它也接受{@link  ReactiveAdapterRegistry}。 
	 *  
	 * @param  messageReaders读者可以从请求正文中进行转换
	 * @param  adapterRegistry以适应Flux和Mono的其他反应类型
	 */
	 ...
	 */
	/**
	 * 使用{@link  HttpMessageReader}从方法参数读取正文。 
	 *  
	 * @param  bodyParameter {{@link> MethodParameter}读取
	 * @param  isBodyRequired如果需要正文，则为true 
	 * @param  bindingContext要使用
	 * @param 交换的绑定上下文当前交换
	 * @return 正文
	 * @see  #readBody（MethodParameter，MethodParameter，boolean，BindingContext，ServerWebExchange）
	 */
	 ...
	 * 使用{@link  HttpMessageReader}从方法参数读取正文。 
	 *  
	 * @param  bodyParam表示主体的元素类型。 
	 * 
	 * @param  actualParam是实际方法的参数类型； 
	 * 可能与{@code  bodyParam}不同，例如对于{@code  HttpEntity}自变量
	 * @param  isBodyRequired如果需要正文，则为true 
	 * @param  bindingContext要使用的绑定上下文
	 * @param 交换当前交换
	 * @return 一个具有要使用值的Mono对于方法参数@since 5.0.2
	 */
	 ...
	 */
	/**
	 * 检查给定的MethodParameter是否需要验证，如果需要，则返回一个带有验证提示的Object []（可能为空）。 
	 * 返回值{@code  null}表示不需要验证。 
	 * 
	 */
	@Nullable
