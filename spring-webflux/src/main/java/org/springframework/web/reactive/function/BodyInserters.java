 ...
 */
/**
 * {@link  BodyInserter}实现的静态工厂方法。 
 *  @author  Arjen Poutsma @author  Rossen Stoyanchev @author 塞巴斯蒂安·德勒兹@5.0
 */
public abstract class BodyInserters {
	...
	/**
	 * 插入程序写入给定值。 
	 *  <p>或者，考虑在{@link  org.springframework.web.reactive.function.client.WebClient WebClient}和{@link  org.springframework}上使用{@code  bodyValue（Object）}快捷方式。 
	 *  web.reactive.function.server.ServerResponse ServerResponse}。 
	 *  
	 * @param 正文要写入的值
	 * @param  <T>正文的类型
	 * @return 插入要写入单个值的插入器
	 * @throws 如果{@code  body}是{<@link> Publisher}或{@link  ReactiveAdapterRegistry＃getSharedInstance（）}支持的类型的实例，对于该实例，{@link  #fromPublisher（Publisher，Class）}或{@link  #fromProducer（Object，类）}。 
	 *  
	 * @see  #fromPublisher（发布者，类）
	 * @see  #fromProducer（对象，类）
	 */
	 ...
	 * @param  <T>正文的类型
	 * @return 插入单个对象的插入器
	 * @throws  IllegalArgumentException如果{@code  body}是{@link  Publisher}或{@link  ReactiveAdapterRegistry＃getSharedInstance（）}支持的类型的实例，为此，{<@link> #fromPublisher（Publisher，Class）}或{@link  #fromProducer （对象，类）}。 
	 *  
	 * @see  #fromPublisher（Publisher，Class）
	 * @see  #fromProducer（Object，Class）@不推荐使用从Spring Framework 5.2开始，推荐使用{@link  #fromValue（Object）}
	 */
	@Deprecated
	 ...
	 */
	/**
	 * 插入程序，用于编写给定值的生产者，该生产者必须是{@link  Publisher}或通过{@link  ReactiveAdapterRegistry}可以适应{@code  Publisher}的其他生产者。 
	 *  <p>或者，考虑在{@link  org.springframework.web.reactive.function.client.WebClient WebClient}和{@link  org.springframework.web.reactive}上使用{@code  body}快捷方式.function.server.ServerResponse ServerResponse}。 
	 *  
	 * @param  <T>正文的类型
	 * @param 生产者正文值的来源。 
	 *  
	 * @param  elementClass要生成的值的类
	 * @return 插入程序以生成生产者@5.2起
	 */
	 ...
	 */
	/**
	 * 插入程序可编写给定的{@link  Publisher}。 
	 *  <p>或者，考虑在{@link  org.springframework.web.reactive.function.client.WebClient WebClient}和{@link  org.springframework.web.reactive}上使用{@code  body}快捷方式.function.server.ServerResponse ServerResponse}。 
	 *  
	 * @param 发布者发布者使用
	 * @param  elementClass编写发布者中元素的类
	 * @param  <T>发布者中包含的元素的类型
	 * @param  <P> { > Publisher}，键入
	 * @return 插入程序以编写{@code  Publisher}
	 */
	 ...
	 *  
	 * @param 发布者发布者用
	 * @param 编写的elementTypeRef发布者中包含的元素的类型
	 * @param  <T>发布者中包含的元素的类型
	 * @param  <P> {代码>发布者}键入
	 * @return 插入程序以编写{@code 发布者}
	 */
	public static <T, P extends Publisher<T>> BodyInserter<P, ReactiveHttpOutputMessage> fromPublisher(
	 ...
	 */
	/**
	 * 插入程序以编写给定的{@code  Resource}。 
	 *  <p>如果可以将资源解析为{@link  plain Resource＃getFile（）文件}，则将使用<a href ="https://en.wikipedia.org/wiki/Zero-copy ">零复制</a>。 
	 *  
	 * @param 资源写入输出消息的资源
	 * @param  <T> {@code 资源的类型} 
	 * @return 插入器以编写{@code  Publisher}
	 */
	 ...
	 */
	/**
	 * 插入程序，用于编写给定的{@code  ServerSentEvent}发布者。 
	 *  <p>或者，您可以通过{@link  #fromPublisher（Publisher，Class）}或{@link  #fromProducer（Object，Class）}提供事件数据对象，并将"Content-Type"设置为{ @link  MediaType＃TEXT_EVENT_STREAM文本/事件流}。 
	 *  
	 * @param  eventspublisher {{@@code> ServerSentEvent}发布者，将其写入响应主体
	 * @param  <T> {@link  ServerSentEvent}中数据元素的类型
	 * @return 插入器{@code  ServerSentEvent}发布者
	 ...
	 */
	/**
	 * 返回{@link  FormInserter}以将给定的{@code  MultiValueMap}编写为URL编码的表单数据。 
	 * 返回的插入器允许通过{@link  FormInserter＃with（String，Object）}添加其他条目。 
	 *  <p>请注意，您还可以在{@code  WebClient}和{@code  WebTestClient}的请求构建器中使用{@code  bodyValue（Object）}方法。 
	 * 在那种情况下，也不需要设置请求内容类型，只需确保该映射仅包含String值，否则将其解释为一个多部分请求。 
	 *  
	 * @param  formData要写入输出消息的表单数据
	 ...
	 */
	/**
	 * 返回{@link  FormInserter}，将给定的键值对写为URL编码的表单数据。 
	 * 返回的插入器允许通过{@link  FormInserter＃with（String，Object）}添加其他条目。 
	 *  
	 * @param 命名要添加到表单的键
	 * @param 值要添加到表单的值
	 ...
	 */
	/**
	 * 返回{@link  MultipartInserter}以将给定的{@code  MultiValueMap}写入为多部分数据。 
	 * 映射中的值可以是Object或{@link  HttpEntity}。 
	 *  <p>请注意，您还可以使用{@link  MultipartBodyBuilder}在外部构建多部分数据，并将生成的映射直接传递到{@code  WebClient}中的{@code  bodyValue（Object）}快捷方法。 
	 *  。 
	 *  
	 * @param  multipartData将表单数据写入输出消息
	 * @return 允许添加更多部分的插入器
	 * @see  MultipartBodyBuilder
	 */
	 ...
	 */
	/**
	 * 返回{@link  MultipartInserter}以将给定的部分作为多部分数据写入。 
	 * 映射中的值可以是Object或{@link  HttpEntity}。 
	 *  <p>请注意，您还可以使用{@link  MultipartBodyBuilder}在外部构建多部分数据，并将生成的映射直接传递到{@code  WebClient}中的{@code  bodyValue（Object）}快捷方法。 
	 *  。 
	 *  
	 * @param 命名零件名称
	 * @param 值零件值，一个Object或{@code  HttpEntity} 
	 * @return 允许添加更多零件的插入器
	 */
	 ...
	 */
	/**
	 * 返回{@link  MultipartInserter}以将给定的异步部分作为多部分数据写入。 
	 *  <p>请注意，您还可以使用{@link  MultipartBodyBuilder}在外部构建多部分数据，并将生成的映射直接传递到{@code  WebClient}中的{@code  bodyValue（Object）}快捷方法。 
	 *  。 
	 *  
	 * @param 命名零件名称
	 * @param 发布者形成零件值的发布者
	 * @param  elementClass {@code 发布者}中包含的类
	 * @return 允许添加更多零件的插入器
	 */
	 ...
	 */
	/**
	 * {@link  #fromMultipartAsyncData（String，Publisher，Class）}的变体，它接受元素类型的{@link  ParameterizedTypeReference}，它允许指定通用类型信息。 
	 *  <p>请注意，您还可以使用{@link  MultipartBodyBuilder}在外部构建多部分数据，并将生成的映射直接传递到{@code  WebClient}中的{@code  bodyValue（Object）}快捷方法。 
	 *  。 
	 *  
	 * @param 为零件名称命名
	 ...
	 */
	/**
	 * 插入程序将给定的{@code  Publisher <DataBuffer>}写入主体。 
	 *  
	 * @param 发布者数据缓冲区发布者编写
	 * @param  <T>发布者的类型
	 * @return 插入程序直接写到正文
	...
	public interface MultipartInserter extends FormInserter<Object> {
		 ...
		 */
		/**
		 * 添加具有基于{@link  Publisher}的内容的异步部分。 
		 *  
		 * @param 命名要添加的零件的名称
		 * @param 发布者的零件内容
		 * @param  elementClass发布者中包含的元素的类型
		 * @return 此插入器用于添加更多零件
		 */
		 ...
		 */
		/**
		 * {@link  #withPublisher（String，Publisher，Class）}的变体，它接受元素类型的{@link  ParameterizedTypeReference}，它允许指定泛型类型信息。 
		 *  
		 * @param 命名要添加的密钥
		 * @param 发布者要添加的发布者作为值
