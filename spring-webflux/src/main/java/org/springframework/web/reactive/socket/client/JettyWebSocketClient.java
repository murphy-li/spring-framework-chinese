 ...
 */
/**
 * 与Jetty {@link  org.eclipse.jetty.websocket.client.WebSocketClient}一起使用的{@link  WebSocketClient}实现。 
 *  <p> <strong>注意：</ strong>码头{@code  WebSocketClient}需要生命周期管理，并且必须启动和停止。 
 * 当此类被声明为Spring bean并使用默认构造函数创建时，将自动对其进行管理。 
 * 有关更多详细信息，请参见构造函数注释。 
...
public class JettyWebSocketClient implements WebSocketClient, Lifecycle {
	 ...
	 */
	/**
	 * 创建和管理Jetty {@link  org.eclipse.jetty.websocket.client.WebSocketClient WebSocketClient}的实例的默认构造函数。 
	 * 可以使用{@link  #getJettyClient（）}获取实例，以进行进一步配置。 
	 *  <p> <strong>注意：</ strong>：使用此构造函数时，此类的{@link 生命周期}方法将委派给Jetty {@code  WebSocketClient}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 接受Jetty {@link  org.eclipse.jetty.websocket.client.WebSocketClient WebSocketClient}的现有实例的构造方法。 
	 *  <p> <strong>注意：</ strong>使用此构造函数意味着Jetty {@code  WebSocketClient}是由外部管理的，因此此类的{@link  Lifecycle}方法没有委托给它。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回基础Jetty {@code  WebSocketClient}。 
	 * 
	 */
