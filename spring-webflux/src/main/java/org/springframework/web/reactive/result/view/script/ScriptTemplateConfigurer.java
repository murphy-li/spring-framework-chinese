 ...
 */
/**
 * Spring WebFlux {@link  ScriptTemplateConfig}的实现，用于创建在Web应用程序中使用的{@code  ScriptEngine}。 
...
public class ScriptTemplateConfigurer implements ScriptTemplateConfig {
	 ...
	 */
	/**
	 * 当设置为{@code  false}时，将为每个请求创建一个新的{@link  ScriptEngine}实例，否则将重复使用同一实例。 
	 ...
	 * 无法使用{@link  #setEngine（ScriptEngine）}，因为需要为每个请求创建脚本引擎的多个实例。 
	 *  
	 ...
	 *  <p>将使用以下参数调用此函数：<ol> <li> {<@code>字符串模板}：模板内容</ li> <li> {<@code>地图模型}：视图模型</ li> <li> {<@code> RenderingContext context}：呈现上下文</ li> </ ol> 
	 * @see  RenderingContext
	 */
