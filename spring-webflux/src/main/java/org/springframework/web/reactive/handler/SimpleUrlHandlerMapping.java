 ...
 */
/**
 * {@link  org.springframework.web.reactive.HandlerMapping}接口的实现，用于从URL映射到请求处理程序bean。 
 * 支持映射到Bean实例和映射到Bean名称； 
 * 非单事件处理程序则需要后者。 
 *  <p>"urlMap"属性适用于用bean实例填充处理程序映射。 
 * 可以通过{mappings"属性以{@code  java.util.Properties}类接受的形式设置对bean名称的映射，如下所示：<pre class ="code"> /welcome.html=ticketController /show.html=ticketController </ pre> <p>语法为{@code  PATH = HANDLER_BEAN_NAME}。 
 * 如果路径不是以斜杠开头，则在前面加上一个斜杠。 
 *  <p>支持直接匹配，例如已注册的"/ test"匹配"/ test"，并且各种Ant样式的模式匹配，例如注册的"/ t"模式匹配"/ test"和"/ team"，"/ test"匹配"/ test"下的所有路径，"/ test"匹配"/ test"下的所有路径。 
...
public class SimpleUrlHandlerMapping extends AbstractUrlHandlerMapping {
	 ...
	 * 有关语法的详细信息，请参见{@link  org.springframework.web.util.pattern.PathPattern} javadoc。 
	 *  
