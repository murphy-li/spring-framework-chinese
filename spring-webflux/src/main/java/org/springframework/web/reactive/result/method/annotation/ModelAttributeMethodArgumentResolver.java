 ...
 */
/**
 * 解决带有注释的方法参数{@code  @ModelAttribute}。 
 *  <p>模型属性源自模型，或使用默认构造函数创建，然后添加到模型。 
 * 创建属性后，将通过数据绑定到请求（表单数据，查询参数）来填充属性。 
 * 如果参数用{@code  @javax.validation.Valid}或Spring自己的{@code  @org.springframework.validation.annotation.Validated}注释，则也可以应用验证。 
 *  <p>使用{@code  useDefaultResolution = true}创建此处理程序时，无论是否存在{@code  @ModelAttribute}，任何非简单类型参数和返回值都将被视为模型属性。 
...
public class ModelAttributeMethodArgumentResolver extends HandlerMethodArgumentResolverSupport {
	 ...
	 * 具有默认解析模式标志的类构造函数。 
	 *  
	 * @param  adapterRegistry用于适应Mono和Mono之间的其他反应类型
	 * @param  useDefaultResolution如果"true"，非简单方法参数和返回值被视为带有或不带有{@code  @ModelAttribute}注解的模型属性当下。 
	 * 
	 */
