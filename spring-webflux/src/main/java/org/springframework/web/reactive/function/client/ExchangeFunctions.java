 ...
 */
/**
 * 用于创建{@link  ExchangeFunction}的静态工厂方法。 
...
public abstract class ExchangeFunctions {
	 ...
	 */
	/**
	 * 使用给定的{@code  ClientHttpConnector}创建一个{@code  ExchangeFunction}。 
	 * 这与调用{@link  #create（ClientHttpConnector，ExchangeStrategies）}并传递{@link  ExchangeStrategies＃withDefaults（）}相同。 
	 *  
	 * @param 连接器用于连接服务器的连接器
	 * @return 创建的{@code  ExchangeFunction}
	 */
	 ...
	 */
	/**
	 * 使用给定的{@code  ClientHttpConnector}和{@code  ExchangeStrategies}创建一个{@code  ExchangeFunction}。 
	 *  
	 * @param 连接器用于连接服务器的连接器
	 * @param 策略{@code  ExchangeStrategies}使用
	 * @return 创建的{@code  ExchangeFunction}
	 */
