 ...
 *  <p>此类知道用于提供静态资源的Spring WebFlux处理程序映射，并使用已配置的{@code  ResourceHttpRequestHandler}的{@code  ResourceResolver}链来做出决定。 
...
public class ResourceUrlProvider implements ApplicationListener<ContextRefreshedEvent> {
	...
	/**
	 * 获取给定URI字符串的公共资源URL。 
	 *  <p> URI字符串应为路径，并且如果包含查询或片段，则将其保留在结果公共资源URL中。 
	 *  
	 * @param  uriString转换的URI字符串
	 * @param 交换当前交换
	 * @return 解析的公共资源URL路径； 
	 * 如果未解析，则为空
	 */
