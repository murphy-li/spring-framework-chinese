 ...
 */
/**
 * 允许使用[协程路由器Kotlin DSL] [CoRouterFunctionDsl]轻松创建WebFlux.fn [RouterFunction]。 
 * 示例：```@Configuration类RouterConfiguration {@Bean fun mainRouter（userHandler：UserHandler）= coRouter {accept（TEXT_HTML）.nest {（GET（"/ user /"）或GET（"/ users /"））。 
 ...
 * invoke（userHandler :: findAll）POST（"/ api / users /"，userHandler :: create）}}}```@author 塞巴斯蒂安·德勒兹
 * @see 路由器@since 5.2
 */
 ...
 */
/**
 * 提供一个WebFlux.fn [RouterFunction]由[`coRouter {}] [coRouter]创建的协程Kotlin DSL，以便能够编写惯用的Kotlin代码。 
