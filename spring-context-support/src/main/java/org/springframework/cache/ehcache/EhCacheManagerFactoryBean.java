 ...
 *  <p>如果未指定配置位置，则将从类路径根目录中的"ehcache.xml"配置CacheManager（即，将应用EhCache文档中定义的默认EhCache初始化）。 
 *  <p>使用EhCacheFactoryBean时，也建议设置一个单独的EhCacheManagerFactoryBean，因为它提供（默认情况下）独立的CacheManager实例，并注意正确关闭CacheManager。 
 * 从非默认配置位置加载EhCache配置时，也需要EhCacheManagerFactoryBean。 
...
public class EhCacheManagerFactoryBean implements FactoryBean<CacheManager>, InitializingBean, DisposableBean {
	...
	/**
	 * 设置EhCache配置文件的位置。 
	 * 典型值为"/WEB-INF/ehcache.xml"。 
	 *  <p>默认为类路径的根目录中的"ehcache.xml"，如果找不到，则为EhCache jar中的"ehcache-failsafe.xml"（默认EhCache初始化）。 
	 *  
	 * @see  net.sf.ehcache.CacheManager＃create（java.io.InputStream）
	 * @see  net.sf.ehcache.CacheManager＃CacheManager（java.io.InputStream）
	 */
	 ...
	 */
	/**
	 * 设置此EhCacheManagerFactoryBean安装是否接受具有相同名称的现有EhCache CacheManager。 
	 * 默认值为"false"。 
	 *  <p>通常与{@link  #setCacheManagerName"cacheManagerName"}结合使用，但如果未指定默认值，将仅使用默认的CacheManager名称。 
	 * 在相同的ClassLoader空间中对相同CacheManager名称（或相同默认值）的所有引用将共享指定的CacheManager。 
	 *  
	 ...
	 */
	/**
	 * 设置EhCache CacheManager是应该共享（作为ClassLoader级别的单例）还是独立（通常在应用程序内部）。 
	 * 默认值为"false"，创建一个独立的本地实例。 
	 *  <p> <b>注意：</ b>此功能允许在同一ClassLoader空间中与调用<code> CacheManager.create（）</ code>的任何代码共享此EhCacheManagerFactoryBean的CacheManager，而无需就特定的CacheManager名称。 
	 * 但是，它仅支持所涉及的单个EhCacheManagerFactoryBean，它将控制基础CacheManager的生命周期（尤其是其关闭）。 
	 *  <p>如果同时设置了两个标志，则此标志将覆盖{@link  #setAcceptExisting"acceptExisting"}，因为它表示共享的"更强"模式。 
	 *  
	 * @see  #setCacheManagerName 
	 * @see  #setAcceptExisting 
	 * @see  net.sf.ehcache.CacheManager＃create（）
