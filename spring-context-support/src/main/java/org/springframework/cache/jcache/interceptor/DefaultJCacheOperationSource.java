 ...
 */
/**
 * 默认的{@link  JCacheOperationSource}实现将默认操作委派给可配置的服务，这些服务具有不存在的默认值。 
...
public class DefaultJCacheOperationSource extends AnnotationJCacheOperationSource
	 ...
	 */
	/**
	 * 使用给定的缓存管理器，缓存解析器和密钥生成器供应商构造一个新的{@code  DefaultJCacheOperationSource}，如果供应商不可解析，则应用相应的默认值。 
	 *  @5.1起
	 */
	public DefaultJCacheOperationSource(
	 ...
	 * 仅在未设置{@link  plain CacheResolver缓存解析器}的情况下才是必需的。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置{@link  CacheResolver}可以解析常规缓存。 
	 * 如果未设置，则将使用使用指定的缓存管理器的默认实现。 
	 * 
	 */
	 ...
	 * 如果未设置，则将使用遵守JSR-107 {@link  javax.cache.annotation.CacheKey}和{@link  javax.cache.annotation.CacheValue}的{@link  SimpleKeyGenerator}。 
	 * 
	 */
	 ...
	 *  <p>非JSR-107设置需要使用{@link  CacheManager}或{@link  CacheResolver}。 
	 * 如果仅指定后者，则无法从自定义{@code  CacheResolver}实现中提取默认异常{@code  CacheResolver}，因此我们不得不依靠{@code  CacheManager}。 
	 *  <p>这给出了一个非常有效的配置的怪异情况，该配置突然中断了，因为启用了JCache支持。 
	 * 为了避免这种情况，我们尽可能晚地解决默认异常{@code  CacheResolver}，以避免在其他情况下出现此类硬性要求。 
	 * 
	 */
