 ...
 */
/**
 * ---------------------- <p> <strong>注意：</ strong>：此类是从{@code  org.springframework.http复制而来的。 
 *  server.reactive.ChannelSendOperator}，并且与此相同。 
 * 它用于同一目的，即，如果输出发布者以错误开头，则可以通过带注释的异常处理程序方法切换到备用处理。 
 *  <p> ---------------------- <br> <p>给出一个接受源{@code  Publisher <T>}的写函数使用write并返回{@code  Publisher <Void>}作为结果，此运算符有助于推迟对write函数的调用，直到我们知道源发布者是否将开始发布而不会出现错误。 
 * 如果第一个发射是错误，则绕过写函数，并且错误直接通过结果发布者发送。 
 * 否则，将调用写入功能。 
 *  @author  Rossen Stoyanchev @author 斯蒂芬·马尔蒂尼（Stephane Maldini）@从5.2起
 * @param  <T>用信号表示的元素类型
 */
