 ...
 */
/**
 * {@link  MessageHandler}的抽象基类，用于将消息代理到注册的订户。 
...
public abstract class AbstractBrokerMessageHandler
	 ...
	 *  
	 * @param  reservePublishOrder是否按顺序从5.1开始发布
	 */
	...
	/**
	 * 检查此消息处理程序当前是否正在运行。 
	 *  <p>请注意，即使此消息处理程序正在运行，{{@link> #isBrokerAvailable（）}标志仍然可以根据具体的子类实现独立地在打开和关闭之间交替。 
	 * 
	 */
	@Override
	...
	/**
	 * 消息代理当前是否可用并且能够处理消息。 
	 *  <p>请注意，这是{@link  #isRunning（）}标志的补充，该标志指示此消息处理程序是否正在运行。 
	 * 换句话说，消息处理程序必须首先运行，然后{@code  #isBrokerAvailable（）}标志仍然可以根据具体的子类实现在打开和关闭之间独立地交替。 
	 *  <p>应用程序组件可以实现{@code  org.springframework.context.ApplicationListener <BrokerAvailabilityEvent>}，以在代理可用和不可用时接收通知。 
	 * 
	 */
	 ...
	 */
	/**
	 * 获取要用于向客户端发送消息的MessageChannel，当{@code  preservePublishOrder = true}时，可以按会话包装。 
	 *  @5.1起
	 */
