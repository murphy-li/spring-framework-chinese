 ...
 */
/**
 * 根据解码的STOMP帧创建{@code  Message}或将{@code  Message}编码为STOMP帧时使用的{{@code> MessageHeaderAccessor}。 
 *  <p>从STOMP帧内容创建时，实际的STOMP标头存储在父类{@link  org.springframework.messaging.support.NativeMessageHeaderAccessor}管理的本机头子映射中，而父类{<@link> SimpMessageHeaderAccessor}管理常见的处理头，其中一些基于STOMP头（例如，目标，内容类型等）。 
 *  <p>也可以通过包装现有的{@code  Message}来创建此类的实例。 
 * 该消息可能是使用更通用的{@link  org.springframework.messaging.simp.SimpMessageHeaderAccessor}创建的，在这种情况下，STOMP头是从通用处理头创建的。 
 * 在这种情况下，如果发送消息，并且取决于消息是发送给客户端还是消息代理，还必须调用{@link  #updateStompCommandAsClientMessage（）}或{@link  #updateStompCommandAsServerMessage（）} 。 
...
public class StompHeaderAccessor extends SimpMessageHeaderAccessor {
	...
	/**
	 * 用于访问和修改现有消息头的构造函数。 
	 * 请注意，消息标头可能不是从STOMP框架创建的，而可能源于使用更通用的{@link  org.springframework.messaging.simp.SimpMessageHeaderAccessor}。 
	 * 
	 */
	...
	/**
	 * 创建心跳的标头。 
	 * 尽管STOMP心跳帧没有标头，但至少需要会话ID才能进行处理。 
	 * 
	 */
	 ...
	 */
	/**
	 * 从给定的标头返回STOMP命令，如果未设置，则返回{@code  null}。 
	 * 
	 */
	@Nullable
