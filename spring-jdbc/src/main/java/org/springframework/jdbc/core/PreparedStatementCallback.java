 ...
 * 允许在单个PreparedStatement上执行任意数量的操作，例如单个{@code  executeUpdate}调用或具有不同参数的重复{@code  executeUpdate}调用。 
 *  <p>由JdbcTemplate内部使用，但对应用程序代码也很有用。 
 * 请注意，传入的PreparedStatement可以由框架或自定义的PreparedStatementCreator创建。 
 * 但是，几乎不需要后者，因为大多数自定义回调操作将执行更新，在这种情况下，可以使用标准PreparedStatement。 
 * 自定义操作将始终始终自行设置参数值，因此也不需要PreparedStatementCreator功能。 
...
public interface PreparedStatementCallback<T> {
	 ...
	 *  
	 * @param  ps活动的JDBC PreparedStatement 
