 ...
 */
/**
 * SimpleJdbcInsert是一个多线程可重用对象，为表提供了轻松的插入功能。 
 * 它提供元数据处理，以简化构造基本插入语句所需的代码。 
 * 您只需要提供表名和一个包含列名和列值的Map。 
 *  <p>元数据处理基于JDBC驱动程序提供的DatabaseMetaData。 
 * 只要JDBC驱动程序可以提供指定表的列名，我们就可以依靠此自动检测功能。 
 * 如果不是这种情况，则必须显式指定列名称。 
 *  <p>实际插入使用Spring的{@link  JdbcTemplate}处理。 
 *  <p>许多配置方法返回SimpleJdbcInsert的当前实例，以提供以"流畅"界面样式将多个链接在一起的功能。 
...
public class SimpleJdbcInsert extends AbstractJdbcInsert implements SimpleJdbcInsertOperations {
	 ...
	 */
	/**
	 * 创建JdbcTemplate时使用带有JDBC数据源一个参数的构造方法。 
	 *  
