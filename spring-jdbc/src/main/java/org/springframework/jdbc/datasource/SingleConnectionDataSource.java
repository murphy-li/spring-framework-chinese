 ...
 */
/**
 * {@link  SmartDataSource}的实现包装单个JDBC连接，该连接在使用后不会关闭。 
 * 显然，这不是多线程功能。 
 *  <p>请注意，在关闭时，应该通过{@code  close（）}方法关闭基础连接。 
 * 如果客户端代码支持SmartDataSource（例如，使用{@code  DataSourceUtils.releaseConnection}），则永远不会在Connection句柄上调用close。 
 *  <p>如果在共享池的假设下客户端代码将调用{@code  close（）}，例如使用持久性工具时，请将"suppressClose"设置为"true"。 
 * 这将返回一个关闭抑制的代理而不是物理连接。 
 *  <p>这主要用于测试。 
 * 例如，它使您可以轻松地在应用服务器上进行测试，以测试期望在数据源上运行的代码。 
 * 与{@link  DriverManagerDataSource}相比，它始终重用同一连接，从而避免了过多的物理连接创建。 
...
public class SingleConnectionDataSource extends DriverManagerDataSource implements SmartDataSource, DisposableBean {
	 ...
	 * 如果返回的Connection应该是一个关闭抑制的代理或物理Connection，则使用<Close>关闭。 
	 * 
	 * @see  java.sql.DriverManager＃getConnection（String，String，串）
	 */
	 ...
	 */
	/**
	 * 使用给定的连接创建一个新的SingleConnectionDataSource。 
	 *  
	 * @param 目标作为目标连接的基础
	 * @param 抑制连接，如果该连接应使用抑制{@code  close（）}调用的连接来包装（以允许正常使用{@code  close（）}期望池化连接但不知道我们的SmartDataSource接口的应用程序）
	 */
	 ...
	 */
	/**
	 * 设置返回的Connection应该是封闭代理还是物理Connection。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回返回的Connection是封闭代理还是物理Connection。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回是否应重写返回的Connection的"autoCommit"设置。 
	 *  
	 * @return "autoCommit"值； 
	 * 如果不应用任何值，则返回{@code  null}
	 */
	@Nullable
	 ...
	 */
	/**
	 * 仅使用一个Connection指定自定义用户名和密码是没有意义的。 
	 * 如果给定相同的用户名和密码，则返回单个Connection； 
	 * 否则抛出SQLException。 
	 * 
	 */
	@Override
