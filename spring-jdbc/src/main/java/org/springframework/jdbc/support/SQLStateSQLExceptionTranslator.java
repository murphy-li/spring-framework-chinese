 ...
 */
/**
 * {@link  SQLExceptionTranslator}实现，该实现基于前两位数字（SQL状态"类"）来分析{@link  SQLException}中的SQL状态。 
 * 检测标准SQL状态值和知名的特定于供应商的SQL状态。 
 *  <p>无法诊断所有问题，但是可以在数据库之间移植并且不需要特殊的初始化（无需检测数据库供应商等）。 
 * 为了获得更精确的翻译，请考虑使用{@link  SQLErrorCodeSQLExceptionTranslator}。 
...
public class SQLStateSQLExceptionTranslator extends AbstractFallbackSQLExceptionTranslator {
	 ...
	 */
	/**
	 * 从提供的{@link  SQLException异常}获取SQL状态代码。 
	 *  <p>某些JDBC驱动程序从批处理更新中嵌套实际的异常，因此我们可能需要深入研究嵌套的异常。 
	 *  
	 * @param ，例如从中提取{@link  SQLException＃getSQLState（）SQL state}的异常
	 * @return  SQL状态代码
	 */
	@Nullable
