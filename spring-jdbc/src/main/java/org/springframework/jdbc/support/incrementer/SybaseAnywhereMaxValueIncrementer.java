 ...
 *  <p>该类旨在与Sybase Anywhere一起使用。 
 *  <p>序列保存在表格中。 
 * 每个表应该有一个需要自动生成键的序列表。 
 *  <p>示例：<pre class ="code">创建表标签（id int不为null的主键，文本varchar（100））创建表tab_sequence（id bigint身份）插入到tab_sequence值中（默认值）</ pre>设置"cacheSize"后，将在不查询数据库的情况下提供中间值。 
 * 如果服务器或应用程序停止，崩溃或回滚事务，则永远不会提供未使用的值。 
 * 因此，编号中的最大孔大小是cacheSize的值。 
 *  <b>提示：</ b>由于Sybase Anywhere支持JDBC 3.0 {@code  getGeneratedKeys}方法，因此建议直接在表中使用IDENTITY列，然后再使用{@link  org.springframework.jdbc .core.simple.SimpleJdbcInsert}或在使用{<@@链接> org.springframework.jdbc.core.JdbcTemplate}。 
 *  <p>感谢Tarald Saxi Stormark的建议！ 
 *  @author 托马斯·里斯伯格（Thomas Risberg）@3.0.5起
 */
