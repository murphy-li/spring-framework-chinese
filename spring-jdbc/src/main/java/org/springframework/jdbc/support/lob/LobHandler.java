 ...
 */
/**
 * 无论是表示为简单类型还是表示大型对象，用于处理特定数据库中的大型二进制字段和大型文本字段的抽象。 
 *  <p>提供BLOB和CLOB的访问器方法，并充当LobCreator实例的工厂，用作创建BLOB或CLOB的会话。 
 * 通常会为每个语句执行或每个事务实例化LobCreator。 
 * 它们不是线程安全的，因为它们可能会跟踪分配的数据库资源，以便在执行后释放它们。 
 *  <p>大多数数据库/驱动程序应该能够与{@link  DefaultLobHandler}一起使用，默认情况下，它会委托JDBC的直接访问器方法，从而避免使用{@code  java.sql.Blob}和{@code 完全是Java.sql.Clob} API。 
 * 也可以通过设置{@link  DefaultLobHandler＃setWrapAsLob"wrapAsLob"}属性，将{@link  DefaultLobHandler}配置为使用{@code  PreparedStatement.setBlob / setClob}（例如，对于PostgreSQL）访问LOB。 
 *  <p>当然，您需要为每个数据库声明不同的字段类型。 
 * 在Oracle中，任何二进制内容都需要放入BLOB，而超过4000字节的所有字符内容都需要放入CLOB。 
 * 在MySQL中，没有CLOB类型的概念，而是表现得像VARCHAR的LONGTEXT类型的概念。 
 * 为了实现完全的可移植性，请使用LobHandler来处理由于字段大小而通常在某些数据库上需要LOB的字段（以Oracle的数字为准）。 
 *  <p> <b>总结推荐的选项（用于实际的LOB字段）：</ b> <ul> <li> <b> JDBC 4.0驱动程序（包括Oracle 11g驱动程序）：</ b>使用{@link  DefaultLobHandler}，如果数据库驱动程序在填充LOB字段时需要该提示，则可能带有{@code  streamAsLob = true}。 
 * 如果在您的（Oracle）数据库设置中碰巧遇到LOB大小限制，请使用{@code  createTemporaryLob = true}。 
 *  <li> <b> Oracle 10g驱动程序：</ b>使用{@link  DefaultLobHandler}进行标准设置。 
 * 在Oracle 10.1上，设置"SetBigStringTryClob"连接属性； 
 * 从Oracle 10.2开始，DefaultLobHandler应该开箱即用地进行标准设置。 
 *  <li> <b> PostgreSQL：</ b>使用{@code  wrapAsLob = true}配置{@link  DefaultLobHandler}，并使用该LobHandler访问数据库表中的OID列（但不访问BYTEA）。 
 *  <li>对于所有其他数据库驱动程序（以及对于可能在某些数据库上转换为LOB的非LOB字段）：只需使用普通的{@link  DefaultLobHandler}。 
 *  </ ul> @author  Juergen Hoeller @2003年12月23日起
 * @see  DefaultLobHandler 
 * @see  java.sql.ResultSet＃getBlob 
 * @see  java.sql.ResultSet＃getClob 
 * @see  java.sql。 
...
public interface LobHandler {
	 ...
	 */
	/**
	 * 从给定的ResultSet中检索给定的列作为字节。 
	 * 可能仅调用{@code  ResultSet.getBytes}或使用{@code  ResultSet.getBlob}，具体取决于数据库和驱动程序。 
	 *  
	 * @param 为ResultSet以从
	 * @param  columnName检索内容名称列名以将
	 * @return 内容用作字节数组，如果SQL NULL为
	 * @throws ，则为{@code  null}如果由JDBC方法抛出
	 * @see  java.sql.ResultSet＃getBytes
	 */
	@Nullable
	 ...
	 * @throws  SQLException，则为{@code  null}如果由JDBC方法抛出
	 * @see  java.sql.ResultSet＃getBinaryStream
	 */
	@Nullable
	 ...
	 * @return 内容用作二进制流，如果SQL NULL 
	 * @throws  SQLException，则使用{@code  null}如果由JDBC方法抛出
	 * @see  java.sql.ResultSet＃getBinaryStream
	 */
	@Nullable
	 ...
	 * 可能仅调用{@code  ResultSet.getString}或使用{@code  ResultSet.getClob}，具体取决于数据库和驱动程序。 
	 *  
	 * @param 为ResultSet以从
	 * @param  columnName检索内容，将列名使用
	 * @return 内容作为字符串，如果为SQL NULL，则为{@code  null}，如果为SQL，则为
	 * @throws  SQLException JDBC方法抛出的异常@
	 * @see java.sql.ResultSet＃getString
	 */
	@Nullable
	 ...
	 */
	/**
	 * 从给定的ResultSet中检索给定的列作为ASCII流。 
	 * 可以简单地调用{@code  ResultSet.getAsciiStream}或使用{@code  ResultSet.getClob}，具体取决于数据库和驱动程序。 
	 *  
	 * @param 或ResultSet以从
	 * @param  columnName检索内容名称列名以将
	 * @return 内容用作ASCII流，如果SQL NULL 
	 * @throws  SQLException，则为{@code  null}如果由JDBC方法抛出
	 * @see  java.sql.ResultSet＃getAsciiStream
	 */
	@Nullable
	 ...
	 * @param 或ResultSet从
	 * @param  columnIndex检索内容索引列索引以将
	 * @return 内容用作ASCII流，或者在SQL NULL 
	 * @throws 的情况下使用{@code  null}如果由JDBC方法抛出
	 * @see  java.sql.ResultSet＃getAsciiStream
	 */
	@Nullable
	 ...
	 * 可能仅调用{@code  ResultSet.getCharacterStream}或使用{@code  ResultSet.getClob}，具体取决于数据库和驱动程序。 
	 *  
	 * @param 或ResultSet以从
	 * @param  columnName检索内容名称列名以将
	 * @return 内容用作字符流
	 ...
	 */
	/**
	 * 创建一个新的{@link  LobCreator}实例，即用于创建BLOB和CLOB的会话。 
	 * 在不再需要创建的LOB之后需要关闭-通常在语句执行或事务完成之后。 
	 *  
