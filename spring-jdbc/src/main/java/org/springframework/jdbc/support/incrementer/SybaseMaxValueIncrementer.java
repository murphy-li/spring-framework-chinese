 ...
 */
/**
 * {@link  DataFieldMaxValueIncrementer}，它以给定的Sybase表的最大值递增一个值，该值等于一个自动递增列。 
 * 注意：如果使用此类，则表键列应<i> NOT </ i>定义为IDENTITY列，因为序列表可以完成此工作。 
 *  <p>此类旨在与Sybase Adaptive Server一起使用。 
 *  <p>序列保存在表格中。 
 * 每个表应该有一个需要自动生成键的序列表。 
 *  <p>示例：<pre class ="code">创建表标签（id int不为null的主键，文本varchar（100））创建表tab_sequence（id bigint身份）插入到tab_sequence values（）</ pre>中，如果"设置了"cacheSize"后，无需查询数据库即可提供中间值。 
 * 如果服务器或应用程序停止，崩溃或回滚事务，则永远不会提供未使用的值。 
 * 因此，编号中的最大孔大小是cacheSize的值。 
 *  <b>提示：</ b>由于Sybase Adaptive Server支持JDBC 3.0 {@code  getGeneratedKeys}方法，因此建议直接在表中使用IDENTITY列，然后再使用{@link  org.springframework。 
 *  jdbc.core.simple.SimpleJdbcInsert}或通过{@code  update（PreparedStatementCreator psc，KeyHolder generateKeyHolder）}方法调用时，利用{@link  org.springframework.jdbc.support.KeyHolder} @link> org.springframework.jdbc.core.JdbcTemplate}。 
 *  <p>感谢刘银薇的建议！ 
