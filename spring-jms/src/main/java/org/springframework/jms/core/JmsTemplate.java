 ...
 * 对于其他操作，这不是必需的。 
 * 点对点（队列）是默认域。 
 *  <p> JMS会话的默认设置为"未交易"和"自动确认"。 
 * 根据Java EE规范的定义，当在活动事务中创建JMS会话时，无论JTA事务还是Spring管理的事务，都将忽略事务和确认参数。 
 * 要为本地JMS使用配置它们，请为bean属性的"sessionTransacted"和"sessionAcknowledgeMode"指定适当的值。 
 *  <p>此模板使用{@link  org.springframework.jms.support.destination.DynamicDestinationResolver}和{@link  org.springframework.jms.support.converter.SimpleMessageConverter}作为解决目标的默认策略分别命名或转换消息。 
 * 可以通过"destinationResolver"和"messageConverter"bean属性来覆盖这些默认值。 
 *  <p> <b>注意：此模板使用的{@code  ConnectionFactory}应该返回池化的Connections（或单个共享Connection）以及池化的Session和MessageProducers。 
 * 否则，临时JMS操作的性能将受到损害。 
 * </ b>最简单的选择是使用Spring提供的{@link  org.springframework.jms.connection.SingleConnectionFactory}作为目标{ @code  ConnectionFactory}，以线程安全的方式重用单个JMS Connection； 
 * 对于通过此模板发送消息的目的而言，这通常通常已经足够好。 
 * 在Java EE环境中，请确保通过JNDI从应用程序的环境命名上下文中获取{@code  ConnectionFactory}。 
 * 应用服务器通常在此处公开池中的，具有事务意识的工厂。 
 *  @author 马克·波拉克@author 于尔根·霍勒@author  Stephane Nicoll @since 1.1 .jms.MessageProducer 
 * @see  javax.jms.MessageConsumer
 */
public class JmsTemplate extends JmsDestinationAccessor implements JmsOperations {
	...
	/**
	 * 内部ResourceFactory适配器，用于与ConnectionFactoryUtils进行交互。 
	 * 
	 */
	 ...
	 */
	/**
	 * 给定一个ConnectionFactory，创建一个新的JmsTemplate。 
	 *  
	 ...
	 */
	/**
	 * 初始化模板策略的默认实现：DynamicDestinationResolver和SimpleMessageConverter。 
	 *  
	 * @see  #setDestinationResolver 
	 * @see  #setMessageConverter 
	 * @see  org.springframework.jms.support.destination.DynamicDestinationResolver 
	 ...
	 */
	/**
	 * 设置要用于没有目标参数的发送/接收操作的目标。 
	 *  <p>或者，指定要通过DestinationResolver动态解析的"defaultDestinationName"。 
	 *  
	 * @see  #send（MessageCreator）
	 * @see  #convertAndSend（Object）
	 * @see  #convertAndSend（Object，MessagePostProcessor）
	...
	/**
	 * 为此模板设置消息转换器。 
	 * 用于解析对象参数以转换为convertAndSend方法，并接收来自ReceiveAndConvert方法的对象结果。 
	 *  <p>默认转换器是SimpleMessageConverter，它能够处理BytesMessages，TextMessages和ObjectMessages。 
	 *  
	 * @see  #convertAndSend 
	 * @see  #receiveAndConvert 
	 * @see  org.springframework.jms.support.converter.SimpleMessageConverter
	...
	/**
	 * 设置用于接收呼叫的超时（以毫秒为单位）。 
	 *  <p>默认值为{@link  #RECEIVE_TIMEOUT_INDEFINITE_WAIT}，表示没有超时的阻止接收。 
	 ...
	 */
	/**
	 * 设置用于发送呼叫的传递延迟（以毫秒为单位）。 
	 *  <p>默认值为-1（没有传递延迟传递给代理）。 
	 * 请注意，此功能需要JMS 2.0。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置是否应将QOS值（deliveryMode，priority，timeToLive）用于发送消息。 
	 *  
	 * @see  #setDeliveryMode 
	 * @see  #setPriority 
	 * @see  #setTimeToLive
	 ...
	 */
	/**
	 * 如果为"true"，则在发送消息时将使用deliveryMode，priority和timeToLive的值。 
	 * 否则，将使用可以通过管理方式设置的默认值。 
	 *  
	 * @return 如果覆盖QOS参数的默认值（deliveryMode，优先级和timeToLive），则为true。 
	 * 
	 * @see  #setDeliveryMode 
	 * @see  #setPriority 
	 * @see  #setTimeToLive
	 ...
	 */
	/**
	 * 设置{@link  QosSettings}以在发送消息时使用。 
	 *  
	 * @param 设置使用@since 5.0起的deliveryMode，优先级和timeToLive设置
	 * @see  #setExplicitQosEnabled（boolean）
	 * @see  #setDeliveryMode（int）
	 * @see  #setPriority（int）
	 * @see ＃ setTimeToLive（long）
	 ...
	 */
	/**
	 * 设置消息传递是持久的还是非持久的，指定为布尔值（"true"或"false"）。 
	 * 这将相应地将传递模式设置为"PERSISTENT"（2）或"NON_PERSISTENT"（1）。 
	 *  <p>默认为"true"，也称为投放方式"PERSISTENT"。 
	 *  
	 * @see  #setDeliveryMode（int）
	 * @see  javax.jms.DeliveryMode＃PERSISTENT 
	 * @see  javax.jms.DeliveryMode＃NON_PERSISTENT
	 ...
	 *  <p>由于可以通过管理方式定义默认值，因此仅在"isExplicitQosEnabled"等于"true"时使用。 
	 *  
	 * @param  timeToLive消息的生存时间（以毫秒为单位）
	 * @see  #isExplicitQosEnabled 
	 * @see  javax.jms.Message＃DEFAULT_TIME_TO_LIVE 
	 * @see  javax.jms.MessageProducer＃send（javax.jms.Message，int，int ， 长）
	 */
	 ...
	 *  {@code  execute（SessionCallback）}的通用版本，允许即时启动JMS连接。 
	 *  <p>一般情况下，请使用{@code  execute（SessionCallback）}。 
	 * 启动JMS连接只是接收消息所必需的，最好通过{@code  receive}方法来实现。 
	 *  
	 * @param 操作回调对象，用于公开会话
	 * @param  startConnection是否启动连接
	 * @return 结果对象与会话一起工作
	 * @throws  JmsException如果存在任何问题，请参见
	 * @see  #execute（ SessionCallback）
	 * @see  #receive
	 */
	@Nullable
	 ...
	 * @param 会话JMS会话在
	 * @param 目标上运行JMS目标发送到
	 * @param  messageCreator回调以创建JMS消息
	 ...
	 * 实际发送给定的JMS消息。 
	 *  
	 * @param 生产者JMS MessageProducer与
	 * @param 消息一起发送JMS消息要发送
	 * @throws  JMSException（如果由JMS API方法抛出）
	 */
	 ...
	 * @param 会话JMS会话在
	 * @param 目标上运行，JMS会话从
	 * @param  messageSelector接收到此使用者的消息选择器（可以为{@code  null}）
	 * @return  JMS收到消息，如果没有消息，则返回{@code  null} 
	 * @throws  JMSException，如果由JMS API方法抛出
	 */
	@Nullable
	 ...
	 *  
	 * @param 会话JMS会话在
	 * @param 使用者上运行，JMS MessageConsumer通过
	 * @return 接收到的JMS消息接收，如果没有，则返回{@code  null} 
	 * @throws  JMSException（如果由JMS抛出） API方法
	 */
	@Nullable
	 ...
	 * 从给定的JMS消息中提取内容。 
	 *  
	 * @param 消息要转换的JMS消息（可以为{@code  null}）
	 * @return 消息的内容，如果没有，则为{@code  null}
	 */
	@Nullable
	 ...
	 */
	/**
	 * 将请求消息发送到给定的{@link  Destination}并阻塞，直到在即时创建的临时队列上收到答复为止。 
	 *  <p>返回响应消息； 
	 * 如果没有消息具有
	 * @throws ，则返回{@code  null}； 
	 * 如果由JMS API方法抛出，则返回JMSException
	 */
	@Nullable
	 ...
	 */
	/**
	 * {@link  #execute（SessionCallback，boolean）}的变体，显式创建非事务性的{@link  Session}。 
	 ...
	 */
	/**
	 * 为给定的会话和目标创建一个JMS MessageProducer，将其配置为禁用消息ID和/或时间戳（如有必要）。 
	 *  <p>委派到{@link  #doCreateProducer}，以创建原始JMS MessageProducer。 
	 *  
	 * @param 会话JMS会话为
	 * @param 目标创建MessageProducer，JMS目的地为
	 * @return 新JMS MessageProducer 
	 * @throws 创建JMSException如果被JMS API方法抛出，则为JMSException。 
	 *  setMessageIdEnabled 
	 * @see  #setMessageTimestampEnabled
	 */
	 ...
	 */
	/**
	 * 为给定的会话和目标创建一个原始的JMS MessageProducer。 
	 *  <p>此实现使用JMS 1.1 API。 
	 *  
	 * @param 会话JMS会话为
	 * @param 目标创建MessageProducer，JMS目的地为
	 * @return 新JMS MessageProducer 
	 * @throws 创建JMSException JMSException（如果由JMS API方法抛出）
	 */
	 ...
	 * @param  messageSelector创建MessageConsumer messageSelector此使用者的消息选择器（可以为{@code  null}）<@如果由JMS API方法抛出，则返回>新的JMS MessageConsumer 
	 * @throws  JMSException
	 */
	 ...
	 *  
	 * @param 会话JMS会话为
	 * @param 创建一个QueueBrowser队列JMS队列为
	 * @param 创建一个QueueBrowser的messageSelector此使用者的消息选择器（可以为{@code  null}）<@返回>新的JMS QueueBrowser 
	 * @throws  JMSException（如果由JMS API方法抛出）
	 * @see  #setMessageIdEnabled 
	 * @see  #setMessageTimestampEnabled
	 */
	 ...
	 */
	/**
	 * 委托给此模板的受保护回调方法的ResourceFactory实现。 
	 * 
	 */
