 ...
 */
/**
 * 单个JPA {@link  javax.persistence.EntityManagerFactory}的{@link  org.springframework.transaction.PlatformTransactionManager}实现。 
 * 将指定工厂的JPA EntityManager绑定到线程，可能每个工厂允许一个线程绑定的EntityManager。 
 *  {@link  SharedEntityManagerCreator}和{@code  @PersistenceContext}知道线程绑定的实体管理器，并自动参与此类事务。 
 * 支持该事务管理机制的JPA访问代码需要使用其中一种。 
 *  <p>此事务管理器适用于使用单个JPA EntityManagerFactory进行事务数据访问的应用程序。 
 *  JTA（通常通过{@link  org.springframework.transaction.jta.JtaTransactionManager}）对于访问同一事务中的多个事务资源是必需的。 
 * 请注意，您需要相应地配置JPA提供程序，以使其参与JTA事务。 
 *  <p>此事务管理器还支持事务内的直接数据源访问（即，与同一数据源一起使用的纯JDBC代码）。 
 * 这允许混合访问JPA的服务和使用纯JDBC的服务（无需了解JPA）！ 
 ...
 * 给定的数据源显然应该与给定的EntityManagerFactory使用的数据源匹配。 
 * 此事务管理器将自动检测用作EntityManagerFactory的连接工厂的数据源，因此通常不需要显式指定"dataSource"属性。 
 *  <p>此事务管理器通过JDBC 3.0保存点支持嵌套事务。 
 * 不过，{@link  #setNestedTransactionAllowed"nestedTransactionAllowed"}标志默认为{@code  false}，因为嵌套事务仅适用于JDBC连接，不适用于JPA EntityManager及其缓存的实体对象和相关上下文。 
 * 如果要将嵌套事务用于参与JPA事务的JDBC访问代码，则可以手动将标志设置为{@code  true}（前提是您的JDBC驱动程序支持Savepoints）。 
 *  <i>请注意，JPA本身不支持嵌套事务！ 
 * 因此，不要期望JPA访问代码在语义上参与嵌套事务。 
 * </ i> @author  Juergen Hoeller @since 2.0起
 * @see  #setEntityManagerFactory 
 * @see  #setDataSource 
 * @see  LocalEntityManagerFactoryBean 
 * @see  org.springframework.orm.jpa.support.SharedEntityManagerBean 
...
public class JpaTransactionManager extends AbstractPlatformTransactionManager
	 ...
	 *  <p>必须设置EntityManagerFactory才能使用它。 
	 *  
	 * @see  #setEntityManagerFactory
	 */
	 ...
	 * 创建一个新的JpaTransactionManager实例。 
	 *  
	 * @param  emf EntityManagerFactory来管理事务
	 */
	 ...
	 */
	/**
	 * 设置该实例应为其管理事务的EntityManagerFactory。 
	 *  <p>或者，指定目标EntityManagerFactory的持久性单元名称。 
	 * 默认情况下，将通过在包含的BeanFactory中找到一个EntityManagerFactory类型的唯一豆来检索默认的EntityManagerFactory。 
	 *  
	 * @see  #setPersistenceUnitName
	 */
	 ...
	 * 数据源应与JPA EntityManagerFactory使用的数据源匹配：例如，您可以为两者指定相同的JNDI数据源。 
	 *  <p>如果EntityManagerFactory使用已知的数据源作为其连接工厂，则将自动检测该数据源：您仍然可以显式指定数据源，但在这种情况下则不需要。 
	 *  <p>此DataSource的事务JDBC连接将提供给直接通过DataSourceUtils或JdbcTemplate访问此DataSource的应用程序代码。 
	 * 连接将从JPA EntityManager获取。 
	 *  <p>请注意，您需要为特定的JPA实现使用JPA方言，以允许将JPA事务公开为JDBC事务。 
	 ...
	 *  <@请参阅> EntityManagerFactoryInfo＃getDataSource（） .core.JdbcTemplate
	 */
	 ...
	 *  <p>如果EntityManagerFactory使用已知的JpaDialect，它将被自动检测到：您仍然可以显式指定DataSource，但在这种情况下则不需要。 
	 *  <p>方言对象可用于检索基础JDBC连接，从而允许将JPA事务公开为JDBC事务。 
	 *  
	 * @see  EntityManagerFactoryInfo＃getJpaDialect（）
	 * @see  JpaDialect＃beginTransaction 
	 * @see  JpaDialect＃getJdbcConnection
	 ...
	 */
	/**
	 * 认真初始化JPA方言，如果未设置，则为指定的EntityManagerFactory创建默认方言。 
	 * 自动检测EntityManagerFactory的数据源（如果有）。 
	 * 
	 */
	@Override
	 ...
	 */
	/**
	 * 创建用于事务的JPA EntityManager。 
	 *  <p>默认实现检查EntityManagerFactory是否为Spring代理，并首先对其进行包装。 
	 *  
	 * @see  javax.persistence.EntityManagerFactory＃createEntityManager（）
	 * @see  EntityManagerFactoryInfo＃getNativeEntityManagerFactory（）
	 */
	 ...
	 */
	/**
	 * 此实现返回"true"：JPA提交将正确处理已在全局级别标记为仅回滚的事务。 
	 * 
	 */
	@Override
	 ...
	 */
	/**
	 * JPA特定的事务定义将传递给{@link  JpaDialect＃beginTransaction}。 
	 *  @5.1起
	 */
