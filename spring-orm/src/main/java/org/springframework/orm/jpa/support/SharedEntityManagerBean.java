 ...
 */
/**
 * {@link  FactoryBean}，它公开给定EntityManagerFactory的共享JPA {@link  javax.persistence.EntityManager}引用。 
 * 通常用于{@link  org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean}创建的EntityManagerFactory，作为Java EE服务器的EntityManager参考的JNDI查找的直接替代。 
 ...
 *  <p>可以传递给期望共享EntityManager引用而不是EntityManagerFactory的DAO。 
 * 请注意，Spring的{@link  org.springframework.orm.jpa.JpaTransactionManager}始终需要EntityManagerFactory才能创建新的事务性EntityManager实例。 
...
public class SharedEntityManagerBean extends EntityManagerFactoryAccessor
	 ...
	 *  <p>默认是由EntityManagerFactoryInfo定义的EntityManager接口（如果有）。 
