 ...
 * 针对Hibernate 5.1 / 5.2 / 5.3开发。 
 *  @author  Juergen Hoeller @author  Costin Leau @since 2.0起
 * @see  HibernateJpaVendorAdapter 
 * @see  org.hibernate.Session＃setFlushMode 
 * @see  org.hibernate.Transaction＃setTimeout
 */
...
public class HibernateJpaDialect extends DefaultJpaDialect {
	 ...
	 *  <p>默认为"true"。 
	 * 如果关闭此标志，则JPA事务管理将不再支持每个事务隔离级别。 
	 * 它也不会再为只读事务调用{@code  Connection.setReadOnly（true）}。 
	 * 如果关闭此标志，则在事务处理后无需清理JDBC连接，因为不会修改任何连接设置。 
	 *  <p> <b>注意：</ b>在Spring 4.1中，只读处理的默认行为发生了变化，类似于其他Spring事务管理器，现在将只读状态传播到JDBC Connection。 
	 * 这可能会导致您立即进入只读强制措施，而以前的写访问权限已被意外容忍：请相应地修改事务声明，并在必要时删除无效的只读标记。 
	 *  @since 4.1 
	 * @see  java.sql.Connection＃setTransactionIsolation 
	 * @see  java.sql.Connection＃setReadOnly
	 */
