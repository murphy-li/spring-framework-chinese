 ...
 */
/**
 * 简单的AOP联盟{@link  MethodInterceptor}实现，为每个方法调用绑定一个新的Hibernate {@link  Session}（如果之前没有绑定）。 
 *  <p>这是一个简单的Hibernate Session范围拦截器，沿{@link  OpenSessionInViewInterceptor}行，仅用于AOP设置而不是MVC设置。 
 * 它将以刷新模式"MANUAL"打开一个新的{@link 会话}，因为该会话仅用于读取（参与事务时除外）。 
...
public class OpenSessionInterceptor implements MethodInterceptor, InitializingBean {
	 ...
	 */
	/**
	 * 为给定的SessionFactory打开一个Session。 
	 *  <p>默认实现将委托给{@link  SessionFactory＃openSession}方法，并将{@link  Session}的刷新模式设置为"MANUAL"。 
	 *  
	 * @param  sessionFactory SessionFactory使用
	 * @return  Session使用
	 * @throws  DataAccessResourceFailureException如果自5.0起就无法创建Session 
	 * @see  FlushMode＃MANUAL
	 */
	...
	/**
	 * 为给定的SessionFactory打开一个Session。 
	 *  @自5.0弃用，推荐使用{@link  #openSession（SessionFactory）}
	 */
	@Deprecated
	@Nullable
