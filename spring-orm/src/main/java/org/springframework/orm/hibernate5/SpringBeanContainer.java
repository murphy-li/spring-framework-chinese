 ...
 */
/**
 * Spring的Hibernate 5.3的{@link  BeanContainer} SPI实现，委托给Spring {@link  ConfigurableListableBeanFactory}。 
 *  <p>由{@link  LocalSessionFactoryBean＃setBeanFactory}自动配置，通过{@link  LocalSessionFactoryBuilder＃setBeanContainer}编程支持，并且可以通过JPA属性中的"hibernate.resource.beans.container"条目手动配置，例如：<pre class ="code"> <bean id ="entityManagerFactory"class ="org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean"> ... <属性名称="jpaPropertyMap"> <map> <entry key ="hibernate .resource.beans.container"> <bean class ="org.springframework.orm.hibernate5.SpringBeanContainer"/> </ entry> </ map> </ property> </ bean> </ pre>或基于Java JPA配置：<pre class ="code"> LocalContainerEntityManagerFactoryBean emfb = ... emfb.getJpaPropertyMap（）。 
 * put（AvailableSettings.BEAN_CONTAINER，new SpringBeanContainer（beanFactory））; </ pre>请注意，出于常见的JPA目的，Spring的{@link  LocalSessionFactoryBean}是{@link  org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean}的直接替代品：特别是在Hibernate 5.3中，Hibernate {@code  SessionFactory}还将本机公开JPA {@code  EntityManagerFactory}接口，并且Hibernate {@code  BeanContainer}集成将立即注册。 
 *  @author 于尔根·霍勒（Juergen Hoeller）@从5.1起BEAN_CONTAINER
 */
public final class SpringBeanContainer implements BeanContainer {
	 ...
	 */
	/**
	 * 为给定的bean工厂实例化一个新的SpringBeanContainer。 
	 *  
	 * @param  beanFactory将要委托给的Spring bean工厂
	 */
