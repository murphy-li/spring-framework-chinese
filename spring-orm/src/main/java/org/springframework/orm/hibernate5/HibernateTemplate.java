 ...
 * 遵循{@code  org.springframework.dao}异常层次结构，将HibernateExceptions自动转换为DataAccessExceptions。 
 *  <p>中心方法是{@code  execute}，它支持实现{@link  HibernateCallback}接口的Hibernate访问代码。 
 * 它提供了Hibernate Session处理，因此HibernateCallback实现和调用代码都不需要显式地关心检索/关闭Hibernate Session或处理Session生命周期异常。 
 * 对于典型的单步操作，有多种便捷方法（查找，加载，saveOrUpdate，删除）。 
 *  <p>可以在服务实现中通过使用SessionFactory引用进行直接实例化来使用，也可以在应用程序上下文中进行准备并作为bean引用提供给服务。 
 * 注意：SessionFactory应该始终在应用程序上下文中配置为bean，在第一种情况下直接提供给服务，在第二种情况下提供给准备好的模板。 
 *  <p> <b>注意：休眠访问代码也可以根据本机的休眠{@link  Session}进行编码。 
 * 因此，对于新启动的项目，请考虑针对{@link  SessionFactory＃getCurrentSession（）}采用标准的Hibernate编码风格。 
 * 或者，对Java 8 lambda代码块使用{@link  #execute（HibernateCallback）}，对回调提供的{@code  Session}使用Java 8 lambda代码块，这也会产生优美的代码，并与Hibernate Session生命周期脱钩。 
 * 与此同时，此HibernateTemplate上的其余操作已被弃用，并且主要作为现有应用程序中较旧的Hibernate 3.x / 4.x数据访问代码的迁移帮助器存在。 
 * </ b> @author  Juergen Hoeller @since 4.2 <@请参阅> #setSessionFactory <@请参阅> HibernateCallback <@请参阅>会话<@请参阅> LocalSessionFactoryBean <@请参阅HibernateTransactionManager <@请参阅> org.springframework.orm.hibernate5.support.OpenSessionInViewFilter <@请参阅> org.springframework.orm.hibernate5 .support.OpenSessionInViewInterceptor
 */
public class HibernateTemplate implements HibernateOperations, InitializingBean {
	 ...
	 * 创建一个新的HibernateTemplate实例。 
	 *  
	 * @param  sessionFactory用于创建Session的SessionFactory
	 */
	 ...
	 */
	/**
	 * 设置要为此访问器使用的所有会话激活的一个或多个Hibernate过滤器名称。 
	 *  <p>这些过滤器中的每个过滤器将在每次操作开始时启用，并在操作结束时相应地禁用。 
	 * 这将适用于新打开的会话以及现有会话（例如，在事务内）。 
	 *  
	 * @see  #enableFilters（会话）
	 * @see 会话#enableFilter（字符串）
	 */
	 ...
	 */
	/**
	 * 设置是否将本机Hibernate会话公开给HibernateCallback代码。 
	 *  <p>默认值为"false"：将返回会话代理，从而抑制{@code  close}调用并自动应用查询缓存设置和事务超时。 
	 *  
	 * @see  HibernateCallback 
	 * @see 会话
	 * @see  #setCacheQueries 
	 ...
	 */
	/**
	 * 返回是将本机Hibernate会话公开给HibernateCallback代码，还是将Session代理公开。 
	 * 
	 */
	 ...
	 */
	/**
	 * 设置是否在写操作（保存/更新/删除）的情况下检查休眠会话是否不处于只读模式。 
	 *  <p>对于尝试在只读事务中进行写操作时的快速失败行为，默认值为"true"。 
	 * 关闭此选项可允许在刷新模式为MANUAL的会话中保存/更新/删除。 
	 *  
	 * @see  #checkWriteOperationAllowed 
	 * @see  org.springframework.transaction.TransactionDefinition＃isReadOnly
	 */
	 ...
	 */
	/**
	 * 返回是否在写操作（保存/更新/删除）的情况下检查休眠会话是否处于只读模式。 
	 * 
	 */
	...
	/**
	 * 设置是否缓存此模板执行的所有查询。 
	 *  <p>如果这是"true"，则此模板创建的所有Query和Criteria对象都将标记为可缓存（包括通过find方法进行的所有查询）。 
	 *  <p>要指定用于此模板缓存的查询的查询区域，请设置"queryCacheRegion"属性。 
	 *  
	 * @see  #setQueryCacheRegion 
	 * @see  org.hibernate.Query＃setCacheable 
	 * @see 条件#setCacheable
	...
	/**
	 * 设置此模板执行的查询的缓存区域的名称。 
	 *  <p>如果指定了此选项，它将应用于该模板创建的所有Query和Criteria对象（包括通过find方法进行的所有查询）。 
	 *  <p>除非将该模板创建的查询配置为通过"cacheQueries"属性进行缓存，否则缓存区域将不会生效。 
	 *  
	 * @see  #setCacheQueries 
	 * @see  org.hibernate.Query＃setCacheRegion 
	 * @see  Criteria＃setCacheRegion
	 ...
	 */
	/**
	 * 在本地{@link 会话}中执行由给定操作对象指定的操作。 
	 *  <p>此execute变量将覆盖模板范围的{@link  #isExposeNativeSession（）"exposeNativeSession"}设置。 
	 *  
	 * @param 操作回调对象，用于指定Hibernate操作
	 * @return 操作返回的结果对象，或者{@code  null} 
	 * @throws  DataAccessException（如果发生Hibernate错误）
	 */
	@Nullable
	 ...
	 *  
	 * @param 操作回调对象，用于指定Hibernate操作
	 * @param  forceNativeSession是否强制将本机Hibernate Session公开给回调代码
	 * @return 该操作返回的结果对象，或者{@code  null} < @抛出> DataAccessException，以防发生Hibernate错误
	 */
	 ...
	 */
	/**
	 * 为给定的Hibernate会话创建一个封闭式代理。 
	 * 代理还准备返回的Query和Criteria对象。 
	 *  
	 * @param 会话Hibernate会话为
	 * @return 会话代理创建代理
	 * @see  Session＃close（）
	 * @see  #prepareQuery 
	 * @see  #prepareCriteria
	...
	/**
	 * 检查是否在给定的会话上允许写操作。 
	 *  <p>在{@code  FlushMode.MANUAL}的情况下，默认实现会引发InvalidDataAccessApiUsageException。 
	 * 可以在子类中覆盖。 
	 *  
	 * @param 会话当前休眠会话
	 * @throws  InvalidDataAccessApiUsageException如果不允许写操作
	 * @see  #setCheckWriteOperations 
	 * @see  Session＃getFlushMode（）
	 * @see  FlushMode＃MANUAL
	 */
	 ...
	 */
	/**
	 * 准备给定的查询对象，应用缓存设置和/或事务超时。 
	 *  
	 * @param  queryObject查询对象以准备
	 * @see  #setCacheQueries 
	 * @see  #setQueryCacheRegion
	 */
	@SuppressWarnings({"rawtypes", "deprecation"})
	 ...
	 * @param 值参数
	 * @throws 的值HibernateException如果被查询对象抛出
	 */
	@Deprecated
	@SuppressWarnings({"rawtypes", "deprecation"})
