 ...
 *  <p>与Servlet 3.1兼容，但可以配置为通过{@link  #setMajorVersion} / {<@link> #setMinorVersion}公开特定版本； 
 * 默认值为3.1。 
 * 请注意，对Servlet 3.1的支持是有限的：不支持Servlet，过滤器和侦听器注册方法。 
 *  JSP配置也不是。 
 * 我们通常不建议您对ServletContainerInitializers和WebApplicationInitializers进行单元测试，否则将使用这些注册方法。 
 *  <p>要在测试环境中设置完整的{@code  WebApplicationContext}，可以使用{@code  AnnotationConfigWebApplicationContext}，{<@code> XmlWebApplicationContext}或{@code  GenericWebApplicationContext}传入相应的{@code  MockServletContext}实例。 
 * 考虑使用{@code  FileSystemResourceLoader}配置{@code  MockServletContext}，以便将资源路径解释为相对文件系统位置。 
 *  @author  Rod Johnson @author  Juergen Hoeller @author  Sam Brannen @自1.0.2 
 * @see  #MockServletContext（org.springframework.core.io.ResourceLoader）
...
public class MockServletContext implements ServletContext {
	 ...
	 */
	/**
	 * 不使用基本路径和使用{@link  DefaultResourceLoader}（即类路径根作为WAR根）创建一个新的{@code  MockServletContext}。 
	 *  
	 * @see  org.springframework.core.io.DefaultResourceLoader
	 */
	 ...
	 */
	/**
	 * 使用{@link  DefaultResourceLoader}创建一个新的{@code  MockServletContext}。 
	 *  
	 ...
	 */
	/**
	 * 使用指定的{@link  ResourceLoader}和没有基本路径，创建一个新的{@code  MockServletContext}。 
	 *  
	 * @param  resourceLoader要使用的ResourceLoader（默认为null）
	 */
	 ...
	 */
	/**
	 * 使用提供的资源基础路径和资源加载器创建新的{@code  MockServletContext}。 
	 *  <p>为名为{@literal'default'}的Servlet注册一个{@link  MockRequestDispatcher}。 
	 *  
	 * @param  resourceBasePath WAR的根目录（不应以斜杠结尾）
	 * @param  resourceLoader要使用的ResourceLoader（或默认为null）
	 * @see  #registerNamedDispatcher
	 */
	 ...
	 */
	/**
	 * 在此{@code  MockServletContext}的资源基础路径之前，为给定路径构建完整的资源位置。 
	 *  
	 * @param 路径指定的路径
	 * @return 完整资源路径
	 */
	 ...
	 */
	/**
	 * 添加一个供{@link  #getMimeType（String）}使用的mime类型映射。 
	 *  
	 * @param  fileExtension文件扩展名，例如{@code  txt}，{<@code> gif} 
	 ...
	 */
	/**
	 * 注册一个{@link  RequestDispatcher}（通常为{@link  MockRequestDispatcher}）作为命名Servlet的包装。 
	 *  
	 * @param 命名包装的Servlet的名称
	 * @param  requestDispatcher封装命名的Servlet的调度程序
	 * @see  #getNamedDispatcher 
	 * @see  #unregisterNamedDispatcher
	 */
	 ...
	 */
	/**
	 * 用给定名称注销{@link  RequestDispatcher}。 
	 *  
	 * @param 命名要取消注册的调度程序的名称
	 * @see  #getNamedDispatcher 
	 * @see  #registerNamedDispatcher
	 */
	 ...
	 */
	/**
	 * 获取<em> default </ em> {@code  Servlet}的名称。 
	 *  <p>默认为{@literal'default'}。 
	 *  
	 * @see  #setDefaultServletName
	 */
	 ...
	 */
	/**
	 * 设置<em> default </ em> {@code  Servlet}的名称。 
	 *  <p>还{@link  #unregisterNamedDispatcher取消注册}当前默认值{@link  RequestDispatcher}，而{@link  #registerNamedDispatcher用{@link  MockRequestDispatcher}替换为提供的{<@code > defaultServletName}。 
	 *  
	 * @param  defaultServletName <em> default </ em> {@code  Servlet}的名称； 
	 * 从不{@code  null}或空
	 * @see  #getDefaultServletName
	 */
	 ...
	 */
	/**
	 * 此方法始终返回{@link  plain Collections＃emptyMap空地图}。 
	 *  
	 * @see  javax.servlet.ServletContext＃getServletRegistrations（）
	 */
	@Override
