 ...
 * 仅支持将普通对象绑定到字符串名称。 
 * 主要用于测试环境，也可用于独立应用程序。 
 *  <p>尽管此类可以（例如）用于在单元测试中覆盖JndiTemplate的{@code  createInitialContext}方法，但该类并不打算供应用程序直接使用。 
 * 通常，将使用SimpleNamingContextBuilder设置JVM级别的JNDI环境。 
 *  @author  Rod Johnson @author  Juergen Hoeller 
 * @see  SimpleNamingContextBuilder 
 * @see  org.springframework.jndi.JndiTemplate＃createInitialContext @deprecated从Spring Framework 5.2开始不推荐使用，以支持第三方的完整解决方案，例如<a href ="https://github.com/h-thurow/Simple-JNDI">简单JNDI </a>
 */
@Deprecated
public class SimpleNamingContext implements Context {
	 ...
	 */
	/**
	 * 使用给定的命名根，给定的名称/对象映射以及JNDI环境条目创建一个新的命名上下文。 
	 * 
	 */
	public SimpleNamingContext(
	 ...
	 * 查找具有给定名称的对象。 
	 *  <p>注意：不适用于应用程序直接使用。 
	 * 任何标准的InitialContext JNDI查找都将使用它。 
	 *  
	 * @throws  javax.naming.NameNotFoundException如果找不到对象
	 */
	@Override
	...
	/**
	 * 将给定对象绑定到给定名称。 
	 * 注意：如果设置JVM级别的JNDI环境，则不适合应用程序直接使用。 
