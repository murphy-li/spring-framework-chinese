 ...
 */
/**
 * {@code  @EnabledIf}用于表示已注释的测试类或测试方法已启用<em> </ em>，如果提供的{@link  #expression}计算为{<@code， > true}。 
 *  <p>在类级别应用时，默认情况下也会自动启用该类中的所有测试方法。 
 *  <p>有关基本示例，请参见Java文档{@link  #expression}。 
 *  <p>此注释可用作<em>元注释</ em>，以创建自定义的<em>组成的注释</ em>。 
 * 例如，可以按如下方式创建自定义{@code  @EnabledOnMac}注解。 
 *  <pre style ="code"> {@literal @} Target（{ElementType.TYPE，ElementType.METHOD}）{@literal @} Retention（RetentionPolicy.RUNTIME）{@literal @} EnabledIf（expression ="＃{systemProperties [ 'os.name']。 
 * toLowerCase（）。 
 * contains（'mac'）}"，原因="在Mac OS上启用"）public {@literal @}接口EnabledOnMac {} </ pre> @author  Sam Brannen @从5.0开始
 * @see  SpringExtension 
 * @see 禁用如果
 * @see  org.junit.jupiter.api.Disabled
 */
...
public @interface EnabledIf {
	 ...
	 */
	/**
	 * 将被评估以确定带注释的测试类或测试方法是否已<em>启用</ em>的表达式。 
	 *  <p>如果表达式的计算结果为{@link  Boolean＃TRUE}或等于{@code "true"}的{@link 字符串}（忽略大小写），则将启用测试。 
	 *  <p>表达式可以是以下任意一种。 
	 *  <ul> <li> Spring表达式语言（SpEL）表达式-例如：<pre style ="code"> @EnabledIf（"＃{systemProperties ['os.name']。 
	 * toLowerCase（）。 
	 * contains（'mac'） }"）</ pre> <li> Spring {@link  org.springframework.core.env.Environment Environment}中可用的属性的占位符-例如：<pre style ="code"> @EnabledIf（"$ {smoke.tests.enabled}"）</ pre> <li>文本文字—例如：<pre style ="code"> @EnabledIf（"true"）</ pre> </ ul> <p>注意但是，不是文本占位符</ em>（它不是动态解析属性占位符的结果）的实际值为零，因为{@code  @EnabledIf（"false"）}等效于{@code  @Disabled}和{@code  @EnabledIf（"true"）}在逻辑上是没有意义的。 
	 *  
	 * @see  #reason 
	 * @see  #loadContext 
	 * @see  #value
