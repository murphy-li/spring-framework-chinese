 ...
 */
/**
 * {@link  EventListener @EventListener}注解用于消耗{{@link> org.springframework.test.context.event.EventPublishingTestExecutionListener EventPublishingTestExecutionListener}发布的{@link  BeforeTestClassEvent}。 
 ...
 * 带有此注释的方法将在{@link  org.springframework.test.context.TestExecutionListener＃beforeTestClass}生命周期的一部分中被调用。 
 *  <p>可以选择通过SpEL表达式{@link 普通#value条件}进行事件处理-例如，{@code  @BeforeTestClass（"event.testContext.testClass.name匹配'。 
 * + IntegrationTests'"）}。 
