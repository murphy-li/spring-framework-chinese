 ...
 */
/**
 * {@code  RunBeforeTestMethodCallbacks}是自定义的JUnit {@link 语句}，它允许通过调用{@link  TestContextManager＃beforeTestMethod（提供的{@link  TestContextManager}上的对象，方法）beforeTestMethod（）}。 
...
public class RunBeforeTestMethodCallbacks extends Statement {
	 ...
	 */
	/**
	 * 构造一个新的{@code  RunBeforeTestMethodCallbacks}语句。 
	 *  
	 * @param 执行链中下一个{@code 语句}的下一个
	 * @param  testInstance当前测试实例（绝不{@code  null}）
	 * @param  testMethod即将成为测试方法的测试方法在测试实例
	 * @param  testContextManager上执行的TestContextManager，在其上调用{@code  beforeTestMethod（）}
	 */
	 ...
	 */
	/**
	 * 调用{@link  TestContextManager＃beforeTestMethod（Object，Method）}，然后评估执行链中的下一个{@link  Statement}（通常是{@link  org.junit.internal.runners.statements的实例） .RunBefores RunBefores}）。 
	 * 
	 */
	@Override
