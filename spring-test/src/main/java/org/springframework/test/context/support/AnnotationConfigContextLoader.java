 ...
 */
/**
 * {@link  AbstractGenericContextLoader}的具体实现可从组件类中加载bean定义。 
 *  <p>请参阅Java文档{{@link> org.springframework.test.context.ContextConfiguration @ContextConfiguration}，以获取<em>组件类</ em>的定义。 
 *  <p>注意：{@code  AnnotationConfigContextLoader}支持<em>组件类</ em>，而不支持旧版{@link  org.springframework.test.context.ContextLoader ContextLoader}定义的基于字符串的资源位置。 
 *  API。 
 * 因此，尽管{@code  AnnotationConfigContextLoader}扩展了{@code  AbstractGenericContextLoader}，但{@code  AnnotationConfigContextLoader}确实不支持{@code  AbstractContextLoader}定义的任何基于字符串的方法。 
 * 或{@code  AbstractGenericContextLoader}。 
 * 因此，{<@code> AnnotationConfigContextLoader}应该主要被视为{@link  org.springframework.test.context.SmartContextLoader SmartContextLoader}而不是{@link  org.springframework.test.context.ContextLoader ContextLoader}。 
 *  @author  Sam Brannen @since 3.1起
 */
public class AnnotationConfigContextLoader extends AbstractGenericContextLoader {
	 ...
	 */
	/**
	 * 在提供的{@link  ContextConfigurationAttributes}中处理<em>组件类</ em>。 
	 *  <p>如果<em>组件类</ em>为{@code  null}或为空，并且{@link  #isGenerateDefaultLocations（）}返回{@code  true}，则此{@code  SmartContextLoader}将尝试{@link  #detectDefaultConfigurationClasses检测默认配置类}。 
	 * 如果检测到默认值，将在提供的配置属性中设置{@link  ContextConfigurationAttributes＃setClasses（Class []）set}。 
	 * 否则，将不会修改提供的配置属性中的属性。 
	 *  
	 ...
	 */
	/**
	 * {@code  AnnotationConfigContextLoader}应该用作{@link  org.springframework.test.context.SmartContextLoader SmartContextLoader}，而不是旧版{@link  org.springframework.test.context.ContextLoader ContextLoader}。 
	 * 因此，不支持此方法。 
	 *  
	 ...
	 */
	/**
	 * 确保提供的{@link  MergedContextConfiguration}不包含{@link  MergedContextConfiguration＃getLocations（）位置}。 
	 *  @since 4.0.4 
	 * @see  AbstractGenericContextLoader＃validateMergedContextConfiguration
	 */
	@Override
	 ...
	 */
	/**
	 * 从提供的{@link  MergedContextConfiguration}中的类中注册提供的{@link  GenericApplicationContext context}中的类。 
	 *  <p>每个类都必须代表一个<em>组件类</ em>。 
	 *  {@link  AnnotatedBeanDefinitionReader}用于注册适当的bean定义。 
	 *  <p>请注意，此方法不会调用{@link  #createBeanDefinitionReader}，因为{@code  AnnotatedBeanDefinitionReader}不是{@link  BeanDefinitionReader}的实例。 
	 *  
	 * @param 上下文应该在其中注册组件类的上下文
	 * @param  mergedConfig应当从中检索这些类的合并配置
	 * @see  AbstractGenericContextLoader＃loadBeanDefinitions
	 */
	@Override
