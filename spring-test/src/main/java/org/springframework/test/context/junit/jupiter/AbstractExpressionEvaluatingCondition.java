 ...
 */
/**
 * {@link  ExecutionCondition}的实现的抽象基类，该实现评估通过注释配置的表达式以确定是否启用了容器或测试。 
 *  <p>表达式可以是以下任意一种。 
 *  <ul> <li> Spring表达式语言（SpEL）表达式-例如：<pre style ="code">＃{systemProperties ['os.name']。 
 * toLowerCase（）。 
 * contains（'mac'）} </ pre > <li> Spring {@link  org.springframework.core.env.Environment Environment}中可用的属性的占位符-例如：<pre style ="code"> $ {smoke.tests.enabled} </ pre> <li>文本文字，例如：<pre style ="code"> true </ pre> </ ul> @author  Sam Brannen @author  Tadaya Tsuyukubo @since 5.0 
 * @see  EnabledIf <@参见> DisabledIf
 */
abstract class AbstractExpressionEvaluatingCondition implements ExecutionCondition {
	 ...
	 */
	/**
	 * 为提供的{@link  ExtensionContext}评估在{@link  AnnotatedElement}上通过提供的注释类型配置的表达式。 
	 *  
	 * @param 注解键入要处理的注释的类型
	 * @param  expressionExtractor一个从注释中提取表达式的函数
	 * @param  reasonExtractor一个从注释中提取原因的函数
	 * @param  loadContextExtractor一个函数提取{注释
	 * @param  enabledOnTrue中的@code  loadContext}标志指示如果表达式的计算结果为{@code  true}，返回的{@code  ConditionEvaluationResult}是否应该{{@link> ConditionEvaluationResult＃enabled enabled} 
	 * @param 上下文{{@@code> ExtensionContext} 
	 * @return  {@link  ConditionEvaluationResult＃enabled enabled}（如果应启用容器或测试）； 
	 * 否则{@link  ConditionEvaluationResult＃disabled disable}
	 */
