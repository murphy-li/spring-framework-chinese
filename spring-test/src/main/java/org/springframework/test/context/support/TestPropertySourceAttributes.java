 ...
 */
/**
 * {@code  TestPropertySourceAttributes}封装通过{@link  TestPropertySource @TestPropertySource}注解声明的属性。 
 *  <p>除了封装声明的属性外，{<@code> TestPropertySourceAttributes}还强制执行配置规则。 
 *  @author  Sam Brannen @author 菲利普·韦伯（Phillip Webb）自4.1起
 * @see  TestPropertySource 
 * @see  MergedTestPropertySources
 */
class TestPropertySourceAttributes {
	 ...
	 */
	/**
	 * 确定此{@code  TestPropertySourceAttributes}实例表示的注释是否可以与提供的{@code 注释}合并。 
	 *  <p>此方法有效地检查两个注释是否在类型层次结构中的同一级别上声明（即具有相同的{@link  plain MergedAnnotation＃getAggregateIndex（）聚合索引}）。 
	 *  @since 5.2 
	 * @see  #mergeWith（MergedAnnotation）
	 */
	 ...
	 */
	/**
	 * 将此{@code  TestPropertySourceAttributes}实例与提供的{@code 注解}合并，断言两组测试属性源属性的{@link  TestPropertySource＃inheritLocations}和{<@link > TestPropertySource＃inheritProperties}标志，并且两个基础注释都在同一类上声明。 
	 *  <p>仅当{@link  #canMergeWith（MergedAnnotation）}返回{@code  true}时，才应调用此方法。 
	 *  @since 5.2 
	 * @see  #canMergeWith（MergedAnnotation）
	 */
	 ...
	 */
	/**
	 * 将所有提供的元素添加到提供的列表中，并遵守{@code  prepend}标志。 
	 *  <p>如果{@code  prepend}标志为{@code  false}，则元素将追加到列表中。 
	 *  
	 * @param 在元素是否应该添加到列表之前添加
	 * @param 列出要向其中添加元素的列表
	 * @param 元素要添加到列表中的元素
	 */
	 ...
	 */
	/**
	 * 获取声明为{@code  @TestPropertySource}的{@link  plain Class class}。 
	 *  
	 * @return 声明类； 
	 * 从不{@code  null}
	 */
	 ...
	 *  <p>注意：返回的值可能表示<em>检测到的默认值</ em>或合并的位置，这些位置与通过单个{@code  @TestPropertySource}注释声明的原始值不匹配。 
	 *  
	 * @return 资源位置； 
	 * 可能为<em>空</ em> 
	 * @see  TestPropertySource＃value 
	 ...
	 *  <p>注意：返回的值可能表示合并的属性，这些属性与通过单个{@code  @TestPropertySource}注释声明的原始值不匹配。 
	 *  
	 * @return 内联属性； 
	 * 可能为<em>空</ em> 
	 * @see  TestPropertySource＃properties
	 ...
	 *  
	 * @return  {@code  InheritedProperties}标志
	 * @see  TestPropertySource＃inheritProperties
	 */
	 ...
	 */
	/**
	 * 提供{@code  @TestPropertySource}属性和声明类的字符串表示形式。 
	 * 
	 */
	@Override
