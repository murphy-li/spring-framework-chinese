 ...
 */
/**
 * {@link  EventListener @EventListener}注解用于消耗{@link  org.springframework.test.context.event.EventPublishingTestExecutionListener EventPublishingTestExecutionListener}发布的{@link  PrepareTestInstanceEvent}。 
 ...
 * 带有此注释的方法将在{@link  org.springframework.test.context.TestExecutionListener＃prepareTestInstance}生命周期的一部分中被调用。 
 *  <p>可以选择通过SpEL表达式{@link  plain #value conditional}进行事件处理-例如，{@code  @PrepareTestInstance（"event.testContext.testClass.name与'。 
 * + IntegrationTests'匹配"）}。 
 ...
 *  @author 弗兰克·谢弗勒（Frank Scheffler）@author 山姆·布兰嫩（Sam Brannen）@从5.2开始
 * @see  PrepareTestInstanceEvent
 */
@Retention(RUNTIME)
