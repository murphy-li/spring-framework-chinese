 ...
 */
/**
 * {@link  EventListener @EventListener}注解用于消耗{@link  org.springframework.test.context.event.EventPublishingTestExecutionListener EventPublishingTestExecutionListener}发布的{@link  AfterTestClassEvent}。 
 ...
 * 带有此注释的方法将在{@link  org.springframework.test.context.TestExecutionListener＃afterTestClass}生命周期中被调用。 
 *  <p>可以选择通过SpEL表达式{@link  plain #value conditional}进行事件处理-例如，{@code  @AfterTestClass（"event.testContext.testClass.name匹配'。 
 * + IntegrationTests'"）}。 
