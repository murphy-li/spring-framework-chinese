 ...
 */
/**
 * {@link  EventListener @EventListener}注解用于消耗{@link  org.springframework.test.context.event.EventPublishingTestExecutionListener EventPublishingTestExecutionListener}发布的{@link  BeforeTestExecution}。 
 ...
 * 带有此注释的方法将作为{@link  org.springframework.test.context.TestExecutionListener＃beforeTestExecution}生命周期的一部分被调用。 
 *  <p>可以选择通过SpEL表达式{@link  plain #value conditional}进行事件处理-例如，{@code  @BeforeTestExecution（"event.testContext.testMethod.name与'test。 
 * '"匹配） }。 
