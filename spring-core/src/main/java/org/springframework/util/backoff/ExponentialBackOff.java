 ...
 */
/**
 * {@link  BackOff}的实现增加了每次重试尝试的退避时间。 
 * 当间隔达到{@link  #setMaxInterval（long）max interval}时，它不再增加。 
 * 达到{@link  #setMaxElapsedTime（long）最大经过时间}后，停止重试。 
 *  <p>示例：默认间隔为{@value #DEFAULT_INITIAL_INTERVAL} ms，默认乘数为{@value #DEFAULT_MULTIPLIER}，默认最大间隔为{@value #DEFAULT_MAX_INTERVAL}。 
 * 对于10次尝试，序列如下：<pre> request＃back off 1 2000 2 3000 3 4500 4 6750 5 10125 6 15187 7 22780 8 30000 9 30000 10 30000 </ pre> <p>请注意，默认的最大时间已过去时间为{@link  Long＃MAX_VALUE}。 
 * 使用{@link  #setMaxElapsedTime（long）}来限制实例在返回{@link  BackOffExecution＃STOP}之前应累积的最大时间长度。 
...
public class ExponentialBackOff implements BackOff {
	 ...
	 * 使用提供的设置创建实例。 
	 *  
	 * @param  initialInterval初始间隔（以毫秒为单位）
	 * @param 乘数乘数（应大于或等于1）
	 */
	 ...
	 */
	/**
	 * 调用{@link  BackOffExecution＃nextBackOff（）}之前返回的最大经过时间（以毫秒为单位）返回{@link  BackOffExecution＃STOP}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回经过的最长时间（以毫秒为单位），在此之后，对{@link  BackOffExecution＃nextBackOff（）}的调用将返回{@link  BackOffExecution＃STOP}。 
	 * 
	 */
