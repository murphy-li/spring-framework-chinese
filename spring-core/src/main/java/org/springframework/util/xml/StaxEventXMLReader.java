 ...
 * 从{@code  XMLEventReader}中使用{@code  XMLEvents}，并在SAX回调接口上调用相应的方法。 
...
class StaxEventXMLReader extends AbstractStaxXMLReader {
	 ...
	 */
	/**
	 * 构造一个{@code  StaxEventXmlReader}的新实例，该实例从给定的{@code  XMLEventReader}中读取。 
	 * 提供的事件阅读器必须处于{@code  XMLStreamConstants.START_DOCUMENT}或{@code  XMLStreamConstants.START_ELEMENT}状态。 
	 *  
	 * @param 读取器{{@@code> XMLEventReader}从
	 * @throws  IllegalStateException读取，如果读取器不在文档或元素的开头
	 */
