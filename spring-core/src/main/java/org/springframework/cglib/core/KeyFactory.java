 ...
 */
/**
 * 生成用于处理多值键的类，以用于诸如Maps和Sets之类的事情。 
 *  <equ> </ code>和<code> hashCode </ code>方法的代码遵循Joshua Bloch在<i> Effective Java </ i>中制定的规则。 
 *  <p>要生成<code> KeyFactory </ code>，您需要提供一个描述密钥结构的接口。 
 * 该接口应具有一个名为<code> newInstance </ code>的方法，该方法将返回一个<code> Object </ code>。 
 *  arguments数组可以是<i>任何内容</ i>-对象，原始值或其中一个的一维或多维数组。 
 * 例如：<p> <pre>私有接口IntStringKey {public Object newInstance（int i，String s）; } </ pre> <p>制成<code> KeyFactory </ code>后，可以通过调用接口定义的<code> newInstance </ code>方法来生成新密钥。 
 *  <p> <pre> IntStringKey factory =（IntStringKey）KeyFactory.create（IntStringKey.class）;对象key1 = factory.newInstance（4，"Hello"）;对象key2 = factory.newInstance（4，"World"）; </ pre> <p> <b>注意：</ b> <code> hashCode </ code>两个键<code> key1 </ code>和<code> key2 </ code>之间的相等性只有在< code> key1.equals（key2）</ code> <i>和</ i>密钥是由同一工厂生产的。 
 *  @version  $ Id：KeyFactory.java，v 1.26 2006/03/05 02:43:19 herbyderby Exp $
 */
...
abstract public class KeyFactory {
	 ...
	 */
	/**
	 * {@link  Type＃hashCode（）}非常昂贵，因为它会遍历完整描述符以计算哈希码。 
	 * 此定制程序使用{@link  Type＃getSort（）}作为哈希码。 
	 * 
	 */
	 ...
	 */
	/**
	 * @弃用此定制器可能会导致意外的类泄漏，因为关键对象仍然对对象和类保持强烈的引用。 
	 * 建议使用预处理方法来剥离对象并将类表示为字符串
	 */
	@Deprecated
	...
	public static class Generator extends AbstractClassGenerator {
		 ...
		 */
		/**
		 * @不建议使用{@link  #addCustomizer（KeyFactoryCustomizer）}。 
		 * 
		 */
		@Deprecated
