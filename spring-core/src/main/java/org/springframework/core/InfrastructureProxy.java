 ...
 */
/**
 * 由透明资源代理实现的接口，需要将其视为与基础资源相等，例如用于一致的查找关键字比较。 
 * 请注意，此接口确实暗示了这种特殊的语义，并且不构成通用的mixin！ 
 *  <p>在{@link  org.springframework.transaction.support.TransactionSynchronizationManager}中进行键比较时，将自动打开此类包装器。 
 *  <p>仅完全透明的代理，例如对于重定向或服务查找，应该实现此接口。 
 * 用新行为装饰目标对象的代理（例如AOP代理）在这里没有资格！ 
