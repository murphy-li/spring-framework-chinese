 ...
 */
/**
 * 扫描程序在{@link  AnnotatedElement}的注释层次结构中搜索相关的注释。 
 *  @author  Philip Webb @author  Sam Brannen @从5.2起
...
abstract class AnnotationsScanner {
	 ...
	 *  
	 * @param  context是一个可选的上下文对象，它将被传递回处理器。 
	 * 
	 * @param  source源元素进行扫描。 
	 * 
	 * @param  searchStrategy搜索策略以使用
	 * @param 处理器接收注释的处理器<@返回> {@link  AnnotationsProcessor＃finish（Object）}的结果
	 */
	@Nullable
	 ...
	 * @param  searchStrategy搜索策略以使用
	 * @param 处理器接收注释的处理器<
	 * @param> classFilter一个可选的过滤器，可用于从层次结构中完全过滤掉特定类
