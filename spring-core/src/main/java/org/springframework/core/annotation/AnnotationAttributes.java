 ...
 */
/**
 * {@link  LinkedHashMap}子类，表示由{@link  AnnotationUtils}，{<@link> AnnotatedElementUtils}以及Spring基于反射和ASM的{ @link  org.springframework.core.type.AnnotationMetadata}实现。 
 *  <p>提供"伪验证"以避免调用代码中嘈杂的Map泛型，以及以类型安全的方式查找注释属性的便捷方法。 
 *  @author  Chris Beams @author  Sam Brannen @author  Juergen Hoeller @since 3.1.1 
 * @see  AnnotationUtils＃getAnnotationAttributes 
 * @see  AnnotatedElementUtils
 */
...
public class AnnotationAttributes extends LinkedHashMap<String, Object> {
	 ...
	 */
	/**
	 * 使用给定的初始容量创建一个新的空{@link  AnnotationAttributes}实例，以优化性能。 
	 *  
	 * @param  initialCapacity基础地图的初始大小
	 */
	 ...
	 */
	/**
	 * 创建一个新的{@link  AnnotationAttributes}实例，包装提供的地图及其所有<em>键值</ em>对。 
	 *  
	 * @param 映射注释属性<em>键值</ em>对的原始来源
	 * @see  #fromMap（Map）
	 */
	 ...
	 */
	/**
	 * 为指定的{@code 注记类型}创建一个新的空{@link  AnnotationAttributes}实例。 
	 *  
	 * @param 注释类型此{@code  AnnotationAttributes}实例表示的注释类型； 
	 * 永远不会{@code  null} @从4.2开始
	 */
	 ...
	 */
	/**
	 * 为指定的{@code 注解类型}创建一个可能已经验证的新的空{@link  AnnotationAttributes}实例。 
	 *  
	 * @param 注释类型此{@code  AnnotationAttributes}实例表示的注释类型； 
	 * 如果属性自5.2起已被验证，则从不验证{@code  null} 
	 * @param 
	 */
	 ...
	 * 为指定的{@code 注记类型}创建一个新的空{@link  AnnotationAttributes}实例。 
	 *  
	 * @param 注释类型此{@code  AnnotationAttributes}实例表示的注释类型名称； 
	 * 从不{@code  null} 
	 * @param  classLoader ClassLoader尝试加载注释类型，或{@code  null}仅存储注释类型名称@since 4.3.2
	 */
	 ...
	 */
	/**
	 * 获取由此{@code  AnnotationAttributes}表示的注释的类型。 
	 *  
	 * @return 注解类型； 
	 * 如果未知，则{{@@code> null}自4.2起
	 */
	@Nullable
	 ...
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取以字符串数组形式存储在指定的{@code  attributeName}下的值。 
	 *  <p>如果在指定的{@code  attributeName}下存储的值是一个字符串，则将其包装在一个单元素数组中，然后返回。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取以布尔值形式存储在指定的{@code  attributeName}下的值。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取以数字形式存储在指定的{@code  attributeName}下的值。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取以枚举形式存储在指定的{@code  attributeName}下的值。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取存储在指定的{@code  attributeName}下的值作为一个类。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取以类数组的形式存储在指定的{@code  attributeName}下的值。 
	 *  <p>如果存储在指定的{@code  attributeName}下的值是一个类，则将其包装在单元素数组中，然后返回它。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期的类型，则永远不要{@code  null}或为空的
	 * @return 值
	 ...
	 */
	/**
	 * 获取存储在指定的{@code  attributeName}下的{@link  AnnotationAttributes}。 
	 *  <p>注意：如果需要实际的注释，请调用{@link  #getAnnotation（String，Class）}。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期类型，则从不{@code  null}或为空
	 ...
	 */
	/**
	 * 获取存储在指定的{@code  attributeName}下的类型为{@code 注解类型}的注释。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 永不{@code  null}或空的
	 * @param 注解类型为预期的注解类型； 
	 * 如果属性不存在或不是自4.2起的预期类型，则永远不要{@code  null} 
	 * @return 注释
	 * @throws  IllegalArgumentException
	 */
	 ...
	 */
	/**
	 * 获取存储在指定的{@code  attributeName}下的{@link  AnnotationAttributes}数组。 
	 *  <p>如果在指定的{@code  attributeName}下存储的值是{@code  AnnotationAttributes}的实例，则将其包装在单元素数组中，然后返回。 
	 *  <p>注意：如果期望实际的注释数组，请调用{@link  #getAnnotationArray（String，Class）}。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 如果属性不存在或不是预期类型，则从不{@code  null}或为空
	 ...
	 */
	/**
	 * 获取存储在指定的{@code  attributeName}下的{@code 注解类型}类型的数组。 
	 *  <p>如果在指定的{@code  attributeName}下存储的值是{@code  Annotation}，则将其包装在单元素数组中，然后返回。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 永不{@code  null}或空的
	 * @param 注解类型为预期的注解类型； 
	 ...
	 */
	/**
	 * 获取存储在指定的{@code  attributeName}下的值，确保该值是{@code  ExpectedType}。 
	 *  <p>如果{@code  ExpectationType}是一个数组，并且存储在指定的{@code  attributeName}下的值是期望数组类型的组件类型的单个元素，则该单个元素将被包装在返回之前，使用适当类型的单元素数组。 
	 *  
	 * @param  attributeName要获取的属性的名称； 
	 * 永远不要{@code  null}或为空的
	 * @param  ExpectedType期望的类型； 
	 * 如果属性不存在或不是预期类型，则从不{@code  null} 
	 * @return 值
	 * @throws  IllegalArgumentException
	 */
	@SuppressWarnings("unchecked")
	 ...
	 *  <p>如果该映射已经是{@code  AnnotationAttributes}实例，它将被强制转换并立即返回，而无需创建新实例。 
	 * 否则，通过将提供的映射传递到{@link  #AnnotationAttributes（Map）}构造函数，将创建一个新实例。 
	 *  
	 * @param 映射注释属性<em>键值</ em>对的原始来源
	 */
	@Nullable
