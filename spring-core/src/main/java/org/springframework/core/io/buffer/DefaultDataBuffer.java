 ...
 */
/**
 * {@link  DataBuffer}接口的默认实现，该接口在内部使用{@link  ByteBuffer}。 
 * 具有独立的读写位置。 
 * 使用{@link  DefaultDataBufferFactory}构造。 
 *  <p>受Netty的{@code  ByteBuf}启发。 
 * 进行了介绍，以使非Netty运行时（即Servlet）在类路径上不需要Netty。 
 *  @author  Arjen Poutsma @author 于尔根·霍勒（Juergen Hoeller）@author  Brian Clozel @since 5.0起
 * @see  DefaultDataBufferFactory
 */
public class DefaultDataBuffer implements DataBuffer {
	 ...
	 */
	/**
	 * 直接公开此缓冲区基于的本机{@code  ByteBuffer}并更新{@code  ByteBuffer的}位置和限制以匹配当前的{@link  #readPosition（）}和{@link  #visibleByteCount（）}。 
	 *  
	 * @return 包装的字节缓冲区
	 */
