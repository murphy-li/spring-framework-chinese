 ...
 */
/**
 * 此类封装了一些AspectJ内部知识，这些知识应在以后的发行版中推回到AspectJ项目中。 
 *  <p>它依赖于AspectJ中特定于实现的知识来破坏封装并完成AspectJ不能做的事情：查询将要执行的运行时测试的类型。 
 * 此处的代码应迁移到{@code  ShadowMatch.getVariablesInvolvedInRuntimeTest（）}或某些类似的操作。 
 *  <p>请参见<a href="https://bugs.eclipse.org/bugs/show_bug.cgi?id=151593">臭虫151593 </a> @author 艾德里安·科耶尔（Adrian Colyer）@author  Ramnivas Laddad @since 2.0
 */
