 ...
 */
/**
 * 按优先级对AspectJ建议/顾问进行排序（<i>非</ i>调用顺序）。 
 *  <p>给出两个建议{{@code> a}和{@code  b}：<ul> <li>如果{@code  a}和{@code  b}在如果在相同的方面定义了{@code  a}和{@code  b}，则具有最低顺序值的建议的优先级最高</ li> <li> {@code  a}或{@code  b}中的一个是after通知的一种形式，则在方面中最后声明的建议具有最高优先级。 
 * 如果{@code  a}和{@code  b}都不是事后建议的形式，则在方面中首先声明的建议具有最高优先级。 
 * </ li> </ ul> <p>重要提示：请注意，与普通比较器不同，返回0表示我们不在乎顺序，也不是两个元素必须相同地排序。 
...
class AspectJPrecedenceComparator implements Comparator<Advisor> {
	 ...
	 */
	/**
	 * 使用给定的Comparator创建一个AspectJPrecedenceComparator，以比较{@link  org.springframework.aop.Advisor}实例。 
	 *  @param advisorComparator用于Advisor的比较器
	 */
