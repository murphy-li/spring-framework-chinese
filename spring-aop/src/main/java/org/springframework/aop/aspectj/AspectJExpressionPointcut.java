 ...
 */
/**
 * Spring {@link  org.springframework.aop.Pointcut}实现，它使用AspectJ weaver评估切入点表达式。 
 *  <p>切入点表达式值是AspectJ表达式。 
 * 这可以引用其他切入点，并使用合成和其他操作。 
 *  <p>自然，由于这将由Spring AOP的基于代理的模型处理，因此仅支持方法执行切入点。 
 *  @author  Rob Harrop @author  Adrian Colyer @author  Rod Johnson @author  Juergen Hoeller @author  Ramnivas Laddad @author  Dave Syer @since 2.0
 */
...
public class AspectJExpressionPointcut extends AbstractExpressionPointcut
	 ...
	 *  @param DeclarationScope切入点的声明范围@param paramNames切入点的参数名称@param paramTypes切入点的参数类型
	 */
	 ...
	 */
	/**
	 * 如果已在XML中指定了切入点表达式，则用户无法将{@code 和}编写为"&&"（尽管&&可以使用）。 
	 * 我们还允许在两个切入点子表达式之间使用{@code 和}。 
	...
	public boolean matches(Method method, Class<?> targetClass, Object... args) {
		...
		try {
			 ...
			 */
			/**
			 * 做最后检查，看是否有任何这种（TYPE）类型的残基匹配。 
			 * 为此，我们使用原始方法（代理方法）的阴影来确保正确地检查了"this"。 
			 * 如果没有此检查，我们将在this（TYPE）上获得不正确的匹配，其中TYPE匹配目标类型，但不匹配"this"（就像JDK动态代理一样）。 
			 *  <p>有关原始错误，请参阅SPR-2979。 
			 * 
			 */
	 ...
	 */
	/**
	 * 针对AspectJ的特定于Spring的{@code  bean（）}切入点指示符扩展程序的处理程序。 
	 *  <p>此处理程序必须添加到需要处理{@code  bean（）} PCD的每个切入点对象。 
	 ...
	 */
	/**
	 * BeanNamePointcutDesignatorHandler的Matcher类。 
	 *  <p>此匹配器的动态匹配测试始终返回true，因为匹配决定是在代理创建时做出的。 
