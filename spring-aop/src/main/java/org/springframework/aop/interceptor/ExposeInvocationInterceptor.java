 ...
 */
/**
 * 将当前{@link  org.aopalliance.intercept.MethodInvocation}公开为线程局部对象的拦截器。 
 * 我们有时需要这样做； 
 * 例如，当切入点（例如AspectJ表达式切入点）需要了解完整的调用上下文时。 
 *  <p>除非确实必要，否则请勿使用此拦截器。 
...
public final class ExposeInvocationInterceptor implements MethodInterceptor, PriorityOrdered, Serializable {
	...
	/**
	 * 此类的Singleton顾问。 
	 * 使用Spring AOP时优先使用INSTANCE，因为它避免了创建新Advisor来包装实例的需要。 
	 * 
	 */
	 ...
	 *  
	 * @return 与当前调用关联的调用对象
	 * @throws  IllegalStateException如果没有正在进行的AOP调用，或者未将ExposeInvocationInterceptor添加到此拦截器链中
	 */
