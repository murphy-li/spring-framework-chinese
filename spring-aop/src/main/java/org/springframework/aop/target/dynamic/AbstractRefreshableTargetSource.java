 ...
 */
/**
 * 封装可刷新目标对象的抽象{@link  org.springframework.aop.TargetSource}实现。 
 * 子类可以确定是否需要刷新，以及是否需要提供新鲜的目标对象。 
 *  <p>实现{@link  Refreshable}接口，以便显式控制刷新状态。 
 *  @author 罗德·约翰逊@author  Rob Harrop @author  Juergen Hoeller @since 2.0起@
 * @see> #requiresRefresh（）
 * @see  #freshTarget（）
 */
public abstract class AbstractRefreshableTargetSource implements TargetSource, Refreshable {
	 ...
	 * 设置刷新检查之间的延迟（以毫秒为单位）。 
	 * 默认值为-1，表示完全不进行刷新检查。 
	 *  <p>请注意，只有在{@link  #requiresRefresh（）}返回{@code  true}时才会发生实际刷新。 
	 * 
	 */
	 ...
	 * 确定是否需要刷新。 
	 * 经过刷新检查延迟后，为每个刷新检查调用。 
	 *  <p>默认实现始终返回{@code  true}，每次延迟都触发刷新。 
	 * 由子类覆盖，并适当检查基础目标资源。 
	 *  
	 * @return 是否需要刷新
	 */
	...
	/**
	 * 获取一个新的目标对象。 
	 *  <p>仅在刷新检查发现需要刷新时才调用（即{@link  #requiresRefresh（）}已返回{@code  true}）。 
	 *  
	 * @return 新鲜的目标对象
	 */
