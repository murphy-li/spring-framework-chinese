 ...
 *  <p> {<@link> DefaultAopProxyFactory}将在必要时自动创建基于CGLIB的代理，例如在代理目标类的情况下（有关详细信息，请参见{@link  DefaultAopProxyFactory助理javadoc}）。 
 *  <p>如果基础（目标）类是线程安全的，则使用此类创建的代理是线程安全的。 
 *  @author  Rod Johnson @author  Rob Harrop @author  Juergen Hoeller @author  Ramnivas Laddad @author  Chris Beams @author  Dave Syer 
 * @see  org.springframework.cglib.proxy.Enhancer 
 * @see  AdvisedSupport＃setProxyTargetClass 
 * @see  DefaultAopProxyFactory
 */
...
class CglibAopProxy implements AopProxy, Serializable {
	 ...
	 * 设置构造函数参数以用于创建代理。 
	 *  
	 * @param 构造函数Args构造函数参数值
	 * @param 构造函数ArgTypes构造函数参数类型
	 */
	 ...
	 * 子类可能希望重写此方法以返回自定义的{@link  Enhancer}实现。 
	 * 
	 */
	 ...
	 */
	/**
	 * 检查所提供的{@code  Class}是否已经通过验证，如果没有通过验证。 
	 * 
	 */
	 ...
	 */
	/**
	 * 检查给定的{@code  Class}上的最终方法，以及跨ClassLoader的程序包可见的方法，并为找到的每个方法将警告写入日志。 
	 * 
	 */
	...
	/**
	 * 处理返回值。 
	 * 如有必要，包装返回的{@code  this}作为{@code  proxy}的返回值，并验证{@code  null}是否未作为原语返回。 
	 * 
	 */
	@Nullable
	 ...
	 * 方法拦截器用于没有建议链的静态目标。 
	 * 该呼叫将直接传递回目标。 
	 * 在需要公开代理并且无法确定该方法将不返回{@code  this}时使用。 
	 * 
	 */
	...
	private static class CglibMethodInvocation extends ReflectiveMethodInvocation {
		 ...
		 */
		/**
		 * 与调用公共方法时使用反射调用目标相比，与使用反射来调用目标相比，性能得到了一定的提高。 
		 * 
		 */
		@Override
	...
	private static class ProxyCallbackFilter implements CallbackFilter {
		 ...
		 */
		/**
		 * 实现CallbackFilter.accept（）以返回我们需要的回调索引。 
		 *  <p>每个代理的回调由一组常规使用的固定回调组成，然后由一组特定于在固定目标链上用于静态目标的方法专用的回调组成。 
		 *  <p>由此确定所使用的回调：<dl> <dt>对于公开的代理</ dt> <dd>公开代理需要在方法/链调用之前和之后执行代码。 
		 * 这意味着我们必须使用DynamicAdvisedInterceptor，因为所有其他拦截器都可以避免使用try / catch块</ dd> <dt>对于Object.finalize（）：</ dt> <dd>不使用此方法的替代。 
		 *  </ dd> <dt>对于equals（）：</ dt> <dd> EqualsInterceptor用于将equals（）调用重定向到此代理的特殊处理程序。 
		 * </ dd> <dt>对于Advised类上的方法：</ dt> <dd> AdvisedDispatcher用于将呼叫直接分派到目标</ dd> <dt>对于建议方法：</ dt> <dd>如果目标是静态的并且建议链被冻结，则该方法专用的FixedChainStaticTargetInterceptor用于调用建议链。 
		 * 否则，将使用DynamicAdvisedInterceptor。 
		 * </ dd> <dt>对于非建议方法：</ dt> <dd>在可以确定该方法将不返回{@code  this}或当{<@code > ProxyFactory.getExposeProxy（）}返回{@code  false}，然后使用Dispatcher。 
		 * 对于静态目标，使用StaticDispatcher； 
		 * 对于动态目标，使用DynamicUnadvisedInterceptor。 
		 * 如果方法有可能返回{@code  this}，则将StaticUnadvisedInterceptor用于静态目标-DynamicUnadvisedInterceptor已经考虑了这一点。 
		 * </ dd> </ dl>
		 */
		@Override
