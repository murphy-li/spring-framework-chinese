...
/**
 * 理解模板的表达式解析器。 
 * 表达式解析器可以将其子类化，而表达式解析器不为模板提供一流的支持。 
 *  @author 基思·唐纳德@author 于尔根·霍勒@author 安迪·克莱门特@since 3.0
 */
public abstract class TemplateAwareExpressionParser implements ExpressionParser {
	 ...
	 * 表达式字符串可以包含所有包含在"$ {......""标记中的任意数量的表达式。 
	 * 例如："foo $ {expr0} bar $ {expr1}"。 
	 * 静态文本也将作为仅返回该静态文本的表达式返回。 
	 * 结果，对所有返回的表达式求值并连接结果将生成完整的求值字符串。 
	 * 仅对找到的最外面的定界符进行解包，因此字符串"hello $ {foo $ {abc}}"将分成"hello"和"foo $ {abc}"两部分。 
	 * 这意味着支持使用$ {..}作为其功能一部分的表达式语言，而不会出现任何问题。 
	 * 解析知道嵌入式表达式的结构。 
	 * 它假定括号"（"，方括号"["和花括号"}"必须成对出现，除非它们在字符串文字中，并且字符串文字以单引号"。 
	 * "开头和结尾。 
	 * 
	 * @param  expressionString表达式字符串
	 * @return 解析的表达式
	 * @throws  ParseException当无法解析表达式时
	 */
	 ...
	 */
	/**
	 * 如果可以在提供的表达式字符串的提供位置找到指定的后缀，则返回true。 
	 *  
	 * @param  expressionString可能包含后缀的表达式字符串
	 * @param  pos检查后缀的起始位置
	 * @param 后缀后缀字符串
	 */
	 ...
	 * @param 后缀
	 * @param  expressionString表达式字符串
	 * @param  afterPrefixIndex之后，正在为其寻找匹配的后缀的最近找到的前缀位置
	 * @return 正确匹配的nextSuffix的位置； 
	 * 如果为-，则为-1没有找到
	 */
