 ...
 */
/**
 * 包含Spring Expression Language可以产生的所有消息。 
 * 每个消息都有一个种类（信息，警告，错误）和一个代码号。 
 * 可以编写测试来期望特定的代码编号而不是特定的文本，从而使消息文本更容易修改，并且测试可以在不同的语言环境中成功运行。 
 *  <p>格式化消息时，它将具有这种形式，捕获前缀和错误类型：<pre class ="code"> EL1004E：找不到类型'String'</ pre> @author 安迪·克莱门特（Andy Clement）@author  Juergen Hoeller @since 3.0
 */
public enum SpelMessage {
	 ...
	 */
	/**
	 * 产生一个完整的消息，包括前缀，并在消息上加上插入符。 
	 *  
	 * @param 插入要插入格式化消息中的插入物
	 * @return 格式化消息@4.3.5起
	 */
