 ...
 */
/**
 * 缓存方面（例如{@link  CacheInterceptor}或AspectJ方面）的基类。 
 *  <p>这使基础Spring缓存基础结构可以轻松地用于为任何方面系统实现方面。 
 *  <p>子类负责以正确的顺序调用相关方法。 
 *  <p>使用<b>策略</ b>设计模式。 
 *  {@link  CacheOperationSource}用于确定缓存操作，{<@link> KeyGenerator}将构建缓存密钥，而{@link  CacheResolver}将解析要使用的实际缓存。 
 *  <p>注意：缓存方面可序列化，但反序列化后不执行任何实际的缓存。 
 *  @author  Costin Leau @author  Juergen Hoeller @author 克里斯·比姆斯（Chris Beams）@author  Phillip Webb @author  Sam Brannen @author  Stephane Nicoll @since 3.1
 */
public abstract class CacheAspectSupport extends AbstractCacheInvoker
	 ...
	 */
	/**
	 * 使用给定的错误处理程序，密钥生成器和缓存解析器/管理器供应商配置此方面，如果供应商不可解析，则应用相应的默认值。 
	 *  @5.1起
	 */
	 ...
	 * 如果提供了多个源，则将使用{@link  CompositeCacheOperationSource}汇总它们。 
	 *  
	 * @see  #setCacheOperationSource
	 */
	 ...
	 */
	/**
	 * 返回此缓存方面委托给的默认{@link  KeyGenerator}。 
	 * 
	 */
	 ...
	 */
	/**
	 * 返回此缓存方面委托给的默认{@link  CacheResolver}。 
	 * 
	 */
	@Nullable
	 ...
	 */
	/**
	 * 返回{@link  CacheOperationMetadata}以执行指定的操作。 
	 *  <p>解析要用于该操作的{@link  CacheResolver}和{@link  KeyGenerator}。 
	 *  
	 * @param 操作操作
	 * @param 方法在其上调用操作的方法
	 * @param  targetClass目标类型
	 * @return 操作的已解析元数据
	 */
	...
	/**
	 * 返回具有指定名称和类型的bean。 
	 * 用于解析{@link  CacheOperation}中按名称引用的服务。 
	 *  
	 * @param  beanName Bean的名称，由操作
	 * @param  bean的ExpectedType类型定义。 
	 * 
	 * @return 与该名称匹配的bean 
	 * @throws  org.springframework.beans.factory.NoSuchBeanDefinitionException不存在
	 * @see  CacheOperation＃getKeyGenerator（）
	 * @see  CacheOperation＃getCacheManager（）
	 * @see  CacheOperation＃getCacheResolver（）
	 */
	 ...
	 * 如果发生异常，则将其包装在{@link  CacheOperationInvoker.ThrowableWrapper}中：可以处理或修改该异常，但必须将其<em> </ em>包装在{@link  CacheOperationInvoker.ThrowableWrapper}中也一样
	 ...
	 */
	/**
	 * 仅为通过条件的{@link  CacheableOperation}查找缓存的项目。 
	 *  
	 * @param 上下文可缓存操作
	 * @return 一个保存缓存项的{@link  Cache.ValueWrapper}，如果找不到则为{@code  null}
	 */
	@Nullable
	 ...
	 */
	/**
	 * 使用指定的结果项收集所有{@link  CacheOperation}的{@link  CachePutRequest}。 
	 *  
	 * @param 上下文要处理
	 * @param 结果的结果的上下文（永不{@code  null}）
	 * @param  put请求要更新的集合
	 */
	 ...
	 */
	/**
	 * 缓存操作的元数据不依赖于特定的调用，这使其成为缓存的理想选择。 
	 * 
	 */
