 ...
 * 允许使用{@link  org.springframework.beans.factory.BeanFactory}公开使用动态语言编写的对象。 
 *  <p>可以将每个对象的脚本指定为对包含它的资源的引用（使用'{@code  script-source}'属性），也可以在XML配置本身中内联（使用'{<@<p>默认情况下，使用这些标记创建的动态对象<strong>不可</ strong>可刷新。 
 * 要启用刷新，请使用来指定每个对象的刷新检查延迟（以毫秒为单位） "{@code 刷新检查延迟}"属性。 
...
class ScriptBeanDefinitionParser extends AbstractBeanDefinitionParser {
	 ...
	 */
	/**
	 * 创建此解析器的新实例，为提供的{@link  org.springframework.scripting.ScriptFactory}类创建bean定义。 
	 *  
	 * @param  scriptFactoryClassName要对其进行操作的ScriptFactory类
	 */
	 ...
	 */
	/**
	 * 解析动态对象元素并返回生成的bean定义。 
	 * 根据需要注册{@link  ScriptFactoryPostProcessor}。 
	 * 
	 */
	@Override
	 ...
	 * 如果两个或两个值均未指定，则记录和{@link  XmlReaderContext＃error}并返回{@code  null}。 
	 * 
	 */
	@Nullable
