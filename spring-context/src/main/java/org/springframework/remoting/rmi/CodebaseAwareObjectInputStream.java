 ...
 */
/**
 * 特殊的ObjectInputStream子类，如果没有在本地找到，则回退到指定的代码库以从中加载类。 
 * 与动态类下载的标准RMI约定相比，客户端在这里确定代码库URL，而不是服务器上的"java.rmi.server.codebase"系统属性。 
 *  <p>使用JDK的RMIClassLoader从指定的代码库中加载类。 
 * 代码库可以包含多个URL，并以空格分隔。 
 * 请注意，RMIClassLoader需要设置SecurityManager，就像在标准RMI中使用动态类下载时一样！ 
 *  （有关详细信息，请参见RMI文档。 
 * ）<p>尽管驻留在RMI程序包中，但该类<i>不</ i>用于RmiClientInterceptor，后者使用标准RMI基础结构，因此只能依靠RMI的通过"java.rmi.server.codebase"下载标准动态类。 
 *  HttpInvokerClientInterceptor使用CodebaseAwareObjectInputStream（请参见此处的"codebaseUrl"属性）。 
 *  <p>感谢Lionel Mestre建议该选项并提供了一个原型！ 
...
public class CodebaseAwareObjectInputStream extends ConfigurableObjectInputStream {
	 ...
	 */
	/**
	 * 为给定的InputStream和代码库创建一个新的CodebaseAwareObjectInputStream。 
	 *  InputStream中的
	 * @param ，以从
	 * @param  codebaseUrl读取，以从本地加载代码类URL（如果不是在本地找到的，可以加载类，可以由多个URL，用空格分隔）。 
	 * 
	 * @see  java.io.ObjectInputStream＃ObjectInputStream（java .io.InputStream）
	 */
	 ...
	 *  InputStream中的
	 * @param 可以从
	 * @param  classLoader中读取ClassLoader，用于加载本地类（可以为{@code  null}，以指示RMI的默认ClassLoader）。 
	 * @
	 * @param> codebaseUrl代码库URL来加载类从如果在本地找不到（可以由多个URL组成，以空格分隔）
	 * @see  java.io.ObjectInputStream＃ObjectInputStream（java.io.InputStream）
	 */
	 ...
	 * @param 可以从
	 * @param  classLoader中读取ClassLoader，用于加载本地类（可以为{@code  null}，以指示RMI的默认ClassLoader）
	 * @param  acceptProxyClasses是否接受代理的反序列化类（可以作为安全措施停用）
	 * @see  java.io.ObjectInputStream＃ObjectInputStream（java.io.InputStream）
	 */
	public CodebaseAwareObjectInputStream(
