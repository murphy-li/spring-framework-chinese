 ...
 */
/**
 * {@link  LoadTimeWeaver}，它使用反射将其委托给具有众所周知的转换挂钩的基础ClassLoader。 
 * 底层的ClassLoader预计将支持以下编织方法（在{@link  LoadTimeWeaver}接口中定义）：<ul> <li> {<@code> public void addTransformer（java.lang.instrument.ClassFileTransformer）} ：用于在此ClassLoader上注册给定的ClassFileTransformer <li> {<@code> public ClassLoader getThrowawayClassLoader（）}：用于获取此ClassLoader的丢弃类加载器（可选； 
 * 如果该方法不可用，则ReflectiveLoadTimeWeaver将退回到SimpleThrowawayClassLoader上） ）</ ul> <p>请注意，上述方法<i>必须</ i>驻留在可公开访问的类中，尽管该类本身不必对应用程序的类加载器可见。 
 *  <p>当将基础ClassLoader实现加载到其他类加载器本身（例如Web应用程序不可见的应用程序服务器的类加载器）中时，此LoadTimeWeaver的反射性质特别有用。 
 * 此LoadTimeWeaver适配器与基础ClassLoader之间没有直接的API依赖关系，只是一个"松散"的方法协定。 
 *  <p>这是要使用的LoadTimeWeaver使用Resin应用程序服务器3.1+版本。 
...
public class ReflectiveLoadTimeWeaver implements LoadTimeWeaver {
	 ...
	 */
	/**
	 * 为当前上下文类加载器<i>创建一个新的ReflectiveLoadTimeWeaver，它需要支持所需的编织方法</ i>。 
	 * 
	 */
	 ...
	 */
	/**
	 * 为给定的类加载器创建一个新的SimpleLoadTimeWeaver。 
	 *  
	 * @param  classLoader {@code  ClassLoader}委托进行编织（<i>必须</ i>支持所需的编织方法）。 
	 *  
	 * @throws  IllegalStateException如果提供的{@code  ClassLoader}不支持所需的编织方法
	 */
