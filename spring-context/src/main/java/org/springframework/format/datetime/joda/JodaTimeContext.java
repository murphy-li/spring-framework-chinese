 ...
 */
/**
 * 包含用户特定的Joda-Time设置的上下文，例如用户的年代（日历系统）和时区。 
 *  <p>一个{@code  null}属性值指示用户尚未指定设置。 
 *  @author 基思·唐纳德（Keith Donald）@since 3.0起@
 * @see> JodaTimeContextHolder
 */
public class JodaTimeContext {
	...
	/**
	 * 设置用户的时区。 
	 *  <p>或者，在{@link  LocaleContextHolder}上设置一个{@link  TimeZoneAwareLocaleContext}。 
	 * 如果此处未提供任何设置，则此上下文类将退回到检查语言环境上下文。 
	 *  
	 * @see  org.springframework.context.i18n.LocaleContextHolder＃getTimeZone（）
	 * @see  org.springframework.context.i18n.LocaleContextHolder＃setLocaleContext
	 */
	 ...
	 */
	/**
	 * 使用将此上下文的设置应用于基本{@code  formatter}来获取DateTimeFormatter。 
	 *  
	 * @param 格式化程序建立默认格式化规则的基本格式化程序，通常与上下文无关
