 ...
 */
/**
 * {@link  GenericApplicationListener}适配器，将事件的处理委派给带有注释的{@link  EventListener}方法。 
 *  <p>委派给{@link  #processEvent（ApplicationEvent）}，以使子类有机会偏离默认值。 
 * 如有必要，请解开{@link  PayloadApplicationEvent}的内容，以允许方法声明定义任何任意事件类型。 
 * 如果定义了条件，则在调用基础方法之前先对其进行评估。 
 *  @author  Stephane Nicoll @author  Juergen Hoeller @author  Sam Brannen @始于4.2
 */
public class ApplicationListenerMethodAdapter implements GenericApplicationListener {
	 ...
	 */
	/**
	 * 解析要用于指定的{@link  ApplicationEvent}的方法参数。 
	 *  <p>这些参数将用于调用此实例处理的方法。 
	 * 可以返回{@code  null}表示无法解析任何合适的参数，因此，不应为指定事件完全调用该方法。 
	 * 
	 */
	@Nullable
	...
	/**
	 * 返回条件使用。 
	 *  <p>匹配{@link  EventListener}注解的{@code  condition}属性，或者匹配使用{@code  @EventListener}进行元注释的组合注解中的任何匹配属性。 
	 * 
	 */
	@Nullable
	 ...
	 */
	/**
	 * 将其他详细信息（例如Bean类型和方法签名）添加到给定的错误消息中。 
	 *  
	 * @param 消息错误消息，将HandlerMethod详细信息附加到
	 */
