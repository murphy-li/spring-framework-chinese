 ...
 */
/**
 * 用于实时bean的适配器查看公开信息，从本地{@code  ApplicationContext}（具有本地{@code  LiveBeansView} bean定义）或所有已注册的ApplicationContext（由{驱动）构建当前bean及其依赖关系的快照。 
 *  @value #MBEAN_DOMAIN_PROPERTY_NAME}环境属性）。 
 *  <p>注意：该功能仍处于beta中，主要设计用于Spring Tool Suite 3.1及更高版本。 
 *  @author  Juergen Hoeller @author  Stephane Nicoll @since 3.2起
 * @see  #getSnapshotAsJson（）
 * @see  org.springframework.web.context.support.LiveBeansViewServlet
 */
public class LiveBeansView implements LiveBeansViewMBean, ApplicationContextAware {
	 ...
	 */
	/**
	 * 生成当前bean及其依赖项的JSON快照，通过{@link  #findApplicationContexts（）}查找所有活动的ApplicationContext，然后委派给{@link  #generateJson（java.util.Set）}。 
	 * 
	 */
	@Override
	 ...
	 */
	/**
	 * 查找当前应用程序的所有适用ApplicationContexts。 
	 *  <p>如果尚未为此LiveBeansView设置特定的ApplicationContext，则调用此方法。 
	 *  
	 * @return  ApplicationContexts的集合
	 */
	 ...
	 */
	/**
	 * 实际上在给定的ApplicationContexts中生成bean的JSON快照。 
	 *  <p>此实现不使用任何JSON解析库，以避免第三方库依赖。 
	 * 它生成了一个上下文描述对象数组，每个对象都包含一个上下文和父属性以及带有嵌套Bean描述对象的bean属性。 
	 * 每个bean对象都包含一个bean，作用域，类型和资源属性，以及一个具有当前bean所依赖的嵌套的bean名称数组的依赖项属性。 
	 *  
	 * @param 上下文ApplicationContexts的集合
	 * @return  JSON文档
	 */
	 ...
	 */
	/**
	 * 确定给定bean定义的资源描述，并对其应用基本的JSON转义（反斜杠，双引号）。 
	 *  
	 * @param  bd bean定义为
	 * @return  JSON转义的资源描述构建资源描述
	 */
	@Nullable
