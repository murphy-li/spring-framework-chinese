 ...
 */
/**
 * {@link 插件}，该插件应用{@code "japicmp-gradle-plugin"}并为所有子项目创建任务，将公共API一张一张地进行比较，并在{@code "build /报告/ api-diff / $ OLDVERSION_to_ $ NEWVERSION /"}。 
 *  <p> {<@code>"./gradlew apiDiff -PbaselineVersion = 5.1.0.RELEASE"}将为所有模块输出基线版本和当前版本之间的API差异报告。 
 * 您可以使用{@code "./gradlew：spring-core：apiDiff -PbaselineVersion = 5.1.0.RELEASE"}将报告限制为单个模块。 
 *  @author 布莱恩·克洛泽尔
 */
